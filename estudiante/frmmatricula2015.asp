...
<!--#include file="cronogramapensiones.asp"-->
<%
...
if (rsMensajes.BOF = false and rsMensajes.EOF=false)  then
	...
	response.Write("<style type='text/css'> #basic-modal-content {display:none;}</style></head>")
	response.write("<body><h3>PROCESO DE MATRICULA " & descripcion_cac & " </h3>")
...
dim rshabilitado
dim  rsrequisitoalumno 

cn.abrirconexion	
set rsrequisitoalumno=cn.consultar("ConsultarRequisitosMatricula_v2_2015","FO",codigo_alu, codigo_pes, codigo_cac)
cn.cerrarconexion  

credaprobados=rsrequisitoalumno.fields("TotalCredAprobados") 	
cicloalumno = 1
...
if tieneMatricula = 1  and codigo_mat = 0 then
	response.redirect("mensajes2015.asp?proceso=EMAT")	
end if 
...
<html class="no-js">
	<head>
		...
		<script type="text/javascript" src="scriptMatricula2015/validarhorario.js?v=<% response.Write DateDiff("s", "12/31/1969 00:00:00", Now) %>"></script>         
        <script type="text/javascript" src="scriptMatricula2015/validarciclocurso.js?v=<% response.Write DateDiff("s", "12/31/1969 00:00:00", Now) %>"></script>         
        <script type="text/javascript" src="scriptMatricula2015/validarmatricula.js?v=<% response.Write DateDiff("s", "12/31/1969 00:00:00", Now) %>"></script>                 
...
<form id="frmFicha" name="frmFicha" method="post" action="procesarmatricula2015.asp?accion=<%=accion%>&codigo_mat=<%=codigo_mat%>">
...
<table border="0" cellpadding="0" cellspacing="0" style="BORDER-COLLAPSE: collapse; border-color:#111111; height:75%" width="100%">
	...
	<tr>
		...
		<b>» Matrícula Ordinaria: </b> del <% response.Write(session("iniciomat")) %> al <% response.Write(session("finmat")) %><br/>
		<b>» Matrícula Extraordinaria: </b> del <% response.Write(session("iniciomatex")) %> al <% response.Write(session("finmatex")) %><br/>
		<b>» Agregados y Retiros: </b> <% response.Write(session("agregadoretiro"))%> <br/>
		</td>
	</tr>
				...
				<tr>
					<td class="bordeinf" align="left" width="65%" height="5%">
					  <font color="#0000FF">Créditos máx. para Matricularse: <b><% response.write(session("CredMax")) %> </b>
							| Total de créditos aprobados : <%=credaprobados%>
					  </font>&nbsp;														
							</td>
				</tr>
				<tr>
					...
					if (rsCursos.BOF And rsCursos.EOF) then
						response.write "<div id=""curso" & descripciontipo & """><p class=""usatsugerencia"">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;No se han Programado Cursos para este ciclo académico " & descripcion_cac  & ", según su Plan de Estudios. Consultar con la Dirección de su Escuela Profesional</p></div>"
					Else					    
	...
	<tr>
		...
		<div id="listadiv" style="height: 100%" class="NoImprimir">
			...
			Do while not rsCursos.EOF
		  		i=i+1
			  	electivo=0
			  	TieneComplementario=rsCursos("TieneComplementario")
			...
			<tr valign="top" style="display:none" id="codigo_cur<%=rsCursos("codigo_cur")%>">
				...
				<table style="border-collapse:collapse" width="95%" class="bordesup">
...
<%  end if %>
	...
	<tr class="azul">
			<td height="5%" class="pestanarevez" bgcolor="#FFFFCC" width="50%" style="font-size:14px;">
						Total Créditos Inscritos: <span id="tdTotal"><%=session("credMat")%></span>
						| Total Cursos Inscritos: <span id="tdCursos"><%=session("cantCursos")%></span> <font color='red'>*</font>
			</td>

			<td align="right" height="5%" width="50%" class="pestanarevez" style="font-size:14px;" background="../images/fa.gif">
					<%if session("tipo_cac")="E" then%>
	...
	<table style="border: 6px double #C0C0C0; width:100%;">
        <tr>
            ...
            <td class="style1" align="right" style="color: #0033CC; font-weight: bold">Cronograma de pago de pensión:</td>
        </tr>
        <tr>
...
<input type="hidden" name="TieneComplementario" id="TieneComplementario" value="<%=TieneComplementario%>" />	
