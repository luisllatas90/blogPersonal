...
function CalcularcursosElegidos()
{
	...
	if ($("#txttipo_cac").val() == "E") {
	//if (frmFicha.txttipo_cac.value == "E") {
	    PagoCursos = PagoCursos * 5
	    //Calcular pago por inscripción
	    lblInscripcion.innerHTML = redondear(((cursosMarcados - cantCursosMat) * 40), 2)

	    if (eleccompeleccionado > 0) {
	        lblInscripcion.innerHTML = parseInt(lblInscripcion.innerHTML) + redondear(((eleccompeleccionado) * 25), 2)    
	    }
	}
	else {
	    PagoCursos = (PagoCursos * 5)
	...
	if (valor > 0 && frmFicha.TieneMatricula.value == 0) {
	    $("#lblCuota").html(redondear(parseFloat(PagoCursos / valor), 2));
...
/* Funcion que verifica los cursos marcados en el formulario de matricula */
function Actualizar(idCheck,limitecrd,tipo)
{
	...
	cantCursosMat = $("#NroCuotas").val();	/* Capturamos el numero de cuoatas */	
	credMat = $("#credMat").val();			/* Capturamos el numero de Créditos matriculados */	
	
	if ($("input:checkbox[name=chkcursoshabiles]").size() == 0) {
	    $(document).ready(function() {
	        $("input[name=chkcursoshabiles]:checked").each(function() {
	    ...
	    total = credMat
	    $(document).ready(function() {
	        $("input:checkbox[name=chkcursoshabiles]:checked").each(function() {
	            if ($(this).attr("type") == "checkbox") {
	                if ($(this).attr("cc") == $(idCheck).attr("cc")) {
	                    contadorGH = contadorGH + 1
	                }

	                if ($(this).attr("tcomp") == "I" && $(idCheck).attr("tcomp") == "I") {
                    ...
                    /*
	                if ($(this).attr("tc") == "CC") {
	                    PrecioCalculado = parseFloat(PrecioCalculado) + parseFloat($(this).attr("preciocalculadocurso"))
	                } else {
	                    PrecioCalculado = parseFloat(PrecioCalculado) + (parseFloat($(this).attr("preciocalculadocurso")) * 5)
	                }
	                */
	            }
	        });
	...
	if (contadorGH>1){
	   alert (ArrMensajes[1])
	   total = parseInt(total) - parseInt($(idCheck).val())
	   $(idCheck).attr('checked', false)	   
	   return 0
	}

	if (parseInt(total) > parseInt(limitecrd) && $("#txtcodigo_cpf").val() != 24) {
	   alert("Ud. solo se puede matricular hasta en " + limitecrd + " créditos\n\n Si aún no ha cumplido el límite de créditos\n Se le sugiere elija otro curso de menor creditaje.")
	   $(idCheck).attr('checked', false)
	   return 0
	}

	if ($("#txttipo_cac").val() == "N") {
	    /*3. Validar adelanto de cursos. Debe matricularse en ciclo inferiores*/
	    ValidaCurso = ValidarCicloCursoInferior($(idCheck));
	    //if (ValidarCicloCursoInferior($(idCheck)) == 0) {
	    if (ValidaCurso == 0) {
	            ...
	            $(document).ready(function() {
	                $("input:checkbox[name=chkcursoshabiles]").each(function() {
	                    if ($(this).attr("cp") == ValidaCurso) {
	                        ...
	                        //$("#chkcursoshabiles" + $(idCheck).attr("cp")).attr("checked", true);
	                        //alert(ValidaCurso + "   |   " + $(this).attr("cp") + "  |  " + $(this.id).attr("cp"))
	                    }
	                });
	            });
	...
	$("#chkcursoUnico" + $(idCheck).attr("cc")).attr("checked", $(idCheck).prop("checked"));

	//Inicia Agregado por Carlo 27.06.2015
	$(document).ready(function() {
	    $("input:checkbox[name=chkcursoshabiles]:checked").each(function() {
	        if ($(this).attr("tc") == "CC" && $(this).val() == 0) {
	            PrecioCalculado = parseFloat(PrecioCalculado) + parseFloat($(this).attr("preciocalculadocurso"))
	        } else {
	            PrecioCalculado = parseFloat(PrecioCalculado) + (parseFloat($(this).attr("preciocalculadocurso")) * 5)
	        }	        
	...
	//Fin Agregado por Carlo 27.06.2015

	lblPrecioCurso.innerHTML = PrecioCalculado
	tdTotal.innerHTML = total
	tdCursos.innerHTML = $("input:checkbox[name=chkcursoshabiles]:checked").size();

	//Agregado por edgard
	var lblPC = ""; // lblPrecioCurso
	lblPC = $('#lblPrecioCurso').html();
	CalcularCuota(5, lblPC);
...
function ActualizaPreRequisito() {
    pagina = "procesarmatricula22015.asp"
    //alert(pagina)
...
function EnviarFichaMatricula()
{
		...
		var HayComplementarioMatriculado = $("#txtExigirCompl").val(); //Permite validar si se exige o no el complementario
		//var cicloalumno=frmFicha.txtcicloalumno.value
		var cicloalumno = $("#txtcicloalumno").val();
		var cuotas
		cuotas=0
		...
		*/
		...
		$("input:checkbox[name=chkcursoshabiles]").each(function() {
		    if (($(this).attr("tc") == "CO" || $(this).attr("cc") == "CC")
		        && $(this).val() == "0" && $(this).attr("ciclo") <= cicloalumno) {
		        HayComplementarioLlevar = parseInt(HayComplementarioLlevar) + 1
		    }
		    ...
		    if ($(this).prop("checked")) {
		        ...
		        totalmarcados = parseInt(totalmarcados) + 1

		        //El usuario ha marcado más complementarios
		        if (($(this).attr("tc") == "CO" || $(this).attr("tc") == "CC") && $(this).val() == "0") {
		            HayComplementarioMarcados = parseInt(HayComplementarioMarcados) + 1 
		        }
		    }
		});
			...
			if (HayComplementarioLlevar>0 && HayComplementarioMarcados==0 && HayComplementarioMatriculado==0){
				...
				{ //Mostrar mensaje de confirmación y enviar datos
					if (confirm(ArrMensajes[4])==true){
						...
						$("#CursosProgramados").attr("value", arrCP);
						//frmFicha.VecesDesprobados.value = arrVD
						$("#VecesDesprobados").attr("value", arrVD);
						//frmFicha.submit()
						$("#frmFicha").submit()
					}
				}else
...
function modificarmatricula(modo,ID)
{
	...
	if (modo=='A'){
		location.href="cargando.asp?pagina=agregarcursomatricula&codigo_mat=" + ID
	}
	else{
...
function CalcularCuota(valor, precioCredito) {
    ...
    $("#lblCuota").html(parseFloat((precioCredito / valor)));
