...
Sub CrearMenu(ByVal codigo_apl,ByVal codigo_tfu,ByVal codigoRaiz_Men,ByVal j)
        ...
        for i=1 to rsMenu.recordcount
			...
			if rsMenu("total_men")>0 then
				...
				'**Ocultar menu padre: Procesos en linea para postgrado: YPEREZ 02/11/11 y educación continua : yperez 20.02.12
			...
			elseif rsMenu("enlace_men")<>"" or IsNull(rsMenu("enlace_men"))=false  then
				...
				'**Ocultar algunos menus hijo para postgrado YPEREZ 02/11/11 y educación continua : yperez 20.02.12
...
<% if session("foto_alu")=0 then %>
    <script>    
        <!-- alert('OBLIGATORIO: TOMA DE FOTOGRAFÍA PARA CARNET DEL 01 AL 15 DE MARZO EN AULA 104 DE 10:00 AM A 01:00 PM');'-->
    </script>	    
...
<%IF session("codigo_usu")<>13379 then%>
	<%
		...
		if session("codigouniver_alu") <> "" then
			...
			'posible motivo: ClscifradoNet dañada en server-test.
			'####(YPEREZ)###
			   'Dim objNEnc
...
<%  'Mostrar Alerta de Pagos
	...
	<% 
	'[ MUESTRA VENTANA PARA ACTUALIZACIÓN DE DATOS - ALUMNI - 2016-06-23 MVILCHEZ]
