...
function CalcularcursosElegidos()
{
	...
	if (chk.length==undefined){
		...
		for (i=0; i<chk.length;i++){
			...
			if (Control.checked==true && eval(Control.value)>0){
				/*PagoCursos+= Control.value * eval(Control.preciocurso)*/
	...
	*/
	if (frmFicha.txtcodigo_cpf.value==24 && frmFicha.txttipo_cac.value=="N"){
		PagoCursos=1200
	}
	...
	if (cursosMarcados>0 || (cursosobligatorios==0 && eleccompeleccionado>0))
		{
...
function Actualizar(idCheck,limitecrd,tipo)
{
	...
	/*4. Validar Horario en Escuelas NO MODULARES MARCADAS
	if (idCheck.checked && frmFicha.txtcodigo_cpf.value!=20 && frmFicha.txtcodigo_cpf.value!=22 && frmFicha.txtcodigo_cpf.value!=23){
	    ValidarHorarioCurso(idCheck)
	}*/

	/*5. Validar que en verano sólo deben matricularse en 3 cursos curriculares*/
