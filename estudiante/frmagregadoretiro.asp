...
'### consultar mensajes bloqueos ###
	...
	if (rsMensajes.BOF = false and rsMensajes.EOF = false) or (rsBloqueoAYR.BOF = false and rsBloqueoAYR.EOF = false)  then
		...
		If (rsMatricula.BOF AND rsMatricula.EOF) then
			...
			response.redirect ("mensajes.asp?proceso=FM")
		Else
			codigoAcceso=2
...
.mensaje {
	...
	<table id="tblmatriculados" border="1" cellpadding="3" cellspacing="0" style="border-collapse: collapse;" bordercolor="gray" width="100%">
			<tr class="etabla">
				...
				<td width="15%" height="14" colspan="2">Código</td>
			...
			%>
			  <tr onMouseOver="Resaltar(1,this,'S')" onMouseOut="Resaltar(0,this,'S')">
				...
				<%
				    ...
				    if rsMatricula("PermitirRetiro")="S" and session("tipo_Cac") = "N" then
			...
			<tr class="usatencabezadotabla">
			<td colspan="9" style="font-size:8"> <%if session("tipo_cac") = "E" then %>(No incluye costo de matricula o inscripción) <%end if %></td>
		...
		<br>
		<h4 align="center" class="azul"><a href="../librerianet/academico/adminestadocuenta.aspx?id=<%=session("codigo_alu")%>">Haga click aquí, para visualizar su monto a pagar.</a></h4>
	</body>
</HTML>
		<%
		end if 
