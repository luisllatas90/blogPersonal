...
Partial Class GestionCurricular_FrmAdicionarInstrumentosContenido_modular
...
#Region "Eventos"
    ...
    Protected Sub gvResultados_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles gvResultados.RowCommand
        Try
            ...
            If e.CommandName.Equals("EditarGrupo") Then
                Dim codigo_gru As Integer, codigo_sgr As Integer

                codigo_gru = Me.gvResultados.DataKeys(index).Values("codigo_gru")
                codigo_uni = Me.gvResultados.DataKeys(index).Values("codigo_uni")
                codigo_sgr = Me.gvResultados.DataKeys(index).Values("codigo_sgr")
                Session("codigo_uni") = Me.gvResultados.DataKeys(index).Values("codigo_uni")
                Session("codigo_sgr") = Me.gvResultados.DataKeys(index).Values("codigo_sgr")

                Me.hdCodigoGru.Value = codigo_gru
                Call mt_CargarDatosEvaluacion(codigo_gru, codigo_sgr)
                ...
                If ddl IsNot Nothing Then
                    If ddl.SelectedIndex > 0 Then
                        If ddl2.SelectedValue > 0 Then
                            ...
                            obj.Ejecutar("COM_AgregarEvaluacionCurso", codigo_cup, ddl.SelectedValue, Me.hdCodigoGru.Value, cod_user, ddl.SelectedItem.Text, ddl2.SelectedValue, Session("codigo_sgr")) ' 20191227 - ENevado
                            obj.CerrarConexion()
                            Call mt_CargarDatosEvaluacion(Me.hdCodigoGru.Value, Session("codigo_sgr"))
                        Else
    ...
    Protected Sub gvInstrumento_RowDataBound(ByVal sender As Object, ByVal e As GridViewRowEventArgs) Handles gvInstrumento.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            If ddl IsNot Nothing Then
                dt = fc_CargarDatosInstrumento(Session("codigo_uni"), Session("codigo_sgr"))
                ddl.DataSource = dt
            ...
            If ddl2 IsNot Nothing Then
                dt = fc_CargarFechas(codigo_cup, Me.hdCodigoGru.Value, Session("codigo_sgr"))
                ddl2.DataSource = dt2
        ...
        If e.Row.RowType = DataControlRowType.Footer Then
            ...
            If ddln IsNot Nothing Then
                dtn = fc_CargarDatosInstrumento(Session("codigo_uni"), Session("codigo_sgr"))
                ddln.DataSource = dtn
            ...
            If ddln2 IsNot Nothing Then
                dtn2 = fc_CargarFechas(codigo_cup, Me.hdCodigoGru.Value, Session("codigo_sgr"))
...
#Region "Metodos"
    ...
    Private Sub mt_CargarDatos(ByVal codigo_dis As String, ByVal codigo_uni As String)
        ...
        Try
            ...
            obj.AbrirConexion()
            dt = obj.TraerDataTable("COM_ListarContenidoAsignatura", codigo_dis, codigo_uni, codigo_cup, "SG")
            obj.CerrarConexion()
            ...
            If dt.Rows.Count > 0 Then
                Call mt_AgruparFilas(Me.gvResultados.Rows, 0, 3)
            End If
        Catch ex As Exception
    ...
    Private Sub mt_CargarDatosEvaluacion(ByVal codigo_gru As String, ByVal codigo_sgr As String)
        Dim dtContenido As Data.DataTable = fc_GetEvaluacion(codigo_gru, Session("codigo_cup"), codigo_sgr)
        Try
...
#Region "Funciones"
    ...
    Public Function fc_GetEvaluacion(ByVal codigo_gru As String, ByVal cod_cup As Integer, ByVal codigo_sgr As String) As Data.DataTable
        Dim obj As New ClsConectarDatos
        Dim dt As New Data.DataTable
        obj.CadenaConexion = ConfigurationManager.ConnectionStrings("CNXBDUSAT").ToString
        ...
        obj.AbrirConexion()
        dt = obj.TraerDataTable("COM_ListarEvaluacionCurso", codigo_gru, cod_user, cod_cup, codigo_sgr)
        obj.CerrarConexion()

        Return dt
    End Function

    Private Function fc_CargarDatosInstrumento(ByVal codigo_uni As Integer, ByVal codigo_sgr As Integer) As Data.DataTable
        Dim obj As New ClsConectarDatos
        Dim dt As New Data.DataTable("data")
        obj.CadenaConexion = ConfigurationManager.ConnectionStrings("CNXBDUSAT").ToString
        ...
        Try
            obj.AbrirConexion()
            dt = obj.TraerDataTable("COM_ListarInstrumentos", codigo_dis, -1, codigo_cup, codigo_sgr)
            obj.CerrarConexion()
        Catch ex As Exception
            Throw ex
        End Try
    ...
    Private Function fc_CargarFechas(ByVal codigo_cup As Integer, ByVal codigo_gru As Integer, ByVal codigo_sgr As Integer) As Data.DataTable
        Dim obj As New ClsConectarDatos
        Dim dt As New Data.DataTable("data")
        obj.CadenaConexion = ConfigurationManager.ConnectionStrings("CNXBDUSAT").ToString
        Try
            obj.AbrirConexion()
            dt = obj.TraerDataTable("COM_ListarFechaSesion", -1, -1, codigo_cup, codigo_gru, "F", codigo_sgr)
            obj.CerrarConexion()
            Return dt
        Catch ex As Exception
            Throw ex
