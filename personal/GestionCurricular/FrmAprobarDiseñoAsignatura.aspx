        ...
        $(document).ready(function() {
            $('#btnBuscar').click(function() {
                ...
                if ($('#ddlPlanCurricular').val() == '' || $('#ddlPlanCurricular').val() == '-1') {
                    alert("Seleccione la carrera profesional para listar las Asignaturas");
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-8">
                                <asp:DropDownList ID="ddlEstado" runat="server" CssClass="form-control input-sm"
                                    ...
                                    <asp:ListItem Value="E">CON DISEÑO ENVIADO</asp:ListItem>
                                </asp:DropDownList>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-md-12">
                        <div class="table-responsive">
                            <asp:GridView ID="gvResultados" runat="server" Width="99%" AutoGenerateColumns="false"
                                AllowPaging="True" PageSize="20" ShowHeader="true" DataKeyNames="codigo_Cur,nombre_Cur,codigo_dis,fecha_apr,codigo_Pes,IdArchivo_Anexo, estado_apr, modular_pcu"
                                CssClass="table table-bordered">
                                <Columns>
                                    <asp:BoundField DataField="codigo_Cur" HeaderText="Código" Visible="false" HeaderStyle-Width="0%" />
                                    <asp:BoundField DataField="nombre_Cur" HeaderText="Nombre del Curso" HeaderStyle-Width="35%" />
                                    <asp:TemplateField ItemStyle-HorizontalAlign="Center" HeaderText="Diseño Aprobado"
                                        HeaderStyle-Width="10%">
                                        <ItemTemplate>
                                            <asp:CheckBox ID="chkDiseño" runat="server" Checked='<%# IIf(Eval("estado_dis")="A", true, false) %>' Enabled="false" />
                                        </ItemTemplate>
                                        <ItemStyle HorizontalAlign="Center"></ItemStyle>
                                    </asp:TemplateField>
                                    <asp:BoundField DataField="aprobado_por" HeaderText="Aprobado por" HeaderStyle-Width="25%" />
                                    <asp:BoundField DataField="fecha_apr" HeaderText="Fecha de Aprobación" HeaderStyle-Width="15%" />
                                    <asp:TemplateField ItemStyle-HorizontalAlign="Center" HeaderText="Acciones" HeaderStyle-Width="15%">
                                        <ItemTemplate>
                                            <asp:LinkButton ID="btnVer" runat="server" CommandName="Ver" CommandArgument="<%# CType(Container,GridViewRow).RowIndex %>"
                                                CssClass="btn btn-info btn-sm" OnClientClick="return confirm('¿Desea visualizar el silabo?');">
                                            ...
                                            <asp:LinkButton ID="btnAprobar" runat="server" CommandName="Aprobar" Text='<i class="fa fa-check"></i>'
                                                ...
                                                Enabled='<%# Eval("tiene_dis").toString() %>' Visible='<%# IIf( Eval("estado_apr") = 0, IIF(Eval("tieneCronograma") > 0, True, False), False) %>'>
                                            </asp:LinkButton>
                                            <asp:LinkButton ID="btnObservar" runat="server" CommandName="Observar" Text='<i class="fa fa-unlink"></i>'
                                                ...
                                                CssClass="btn btn-danger btn-sm" Visible='<%# IIf(Eval("estado_apr")=0,True,False) %>'
                                                OnClientClick="return confirm('¿Desea generar una observación del diseño de la asignatura?');">
                                            </asp:LinkButton>
                                            <asp:LinkButton ID="btnDesaprobar" runat="server" CommandName="Desaprobar" Text='<i class="fa fa-calendar-times"></i>'
                                                ToolTip="Desaprobar diseño" CommandArgument="<%# CType(Container,GridViewRow).RowIndex %>"
                                                CssClass="btn btn-warning btn-sm" Visible='<%# IIf(Eval("estado_apr")=1,True,False) %>'
                                                OnClientClick="return confirm('¿Esta seguro de desaprobar el diseño de la asignatura?');">
                                            </asp:LinkButton>
                                        </ItemTemplate>
                                    </asp:TemplateField>
                                </Columns>
