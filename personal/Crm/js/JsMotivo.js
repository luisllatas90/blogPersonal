$(document).ready(function() {
    fnLoading(true);
    var dt = fnCreateDataTableBasic('tMotivo', 2, 'asc');
    ope = fnOperacion(1);
    //console.log(ope);
    fnBuscarMotivo(false);

    rpta = fnvalidaSession()
    //alert(rpta)
    if (rpta == true) {
        $('#btnListar').click(fnBuscarMotivo);
...
function fnBuscarMotivo(sw) {
    rpta = fnvalidaSession()
    if (rpta == true) {
        if (sw) { fnLoading(true); }
        //fnLoading(true)

        $("form#frmBuscarMotivo input[id=action]").remove();
        $('#frmBuscarMotivo').append('<input type="hidden" id="action" name="action" value="' + ope.lst + '" />');
        var form = $("#frmBuscarMotivo").serializeArray();
        //console.log(form);
        $("form#frmBuscarMotivo input[id=action]").remove();

        $.ajax({
            type: "POST",
            url: "../DataJson/crm/Motivo.aspx",
            data: form,
            dataType: "json",
            cache: false,
            success: function(data) {
                ...
                fnDestroyDataTableDetalle('tMotivo');
                $('#tbMotivo').html(tb);
                fnResetDataTableBasic('tMotivo', 2, 'asc');
...
function fnDelete(cod) {
    aDataR = {
        cod: cod,
        mensaje: '¿Desea Eliminar el Motivo?'
    }
