...
Partial Class Egresado_DetOfertas
    ...
    Private Sub AsignaDatos(ByVal oferta As Integer)
        ...
        If (dtDatos.Rows.Count > 0) Then
            ...
            Me.lblVisible.Text = IIf(dtDatos.Rows(0).Item("visible_ofe").ToString = "True", "Sí", "No")
            Me.lblSector.Text = dtDatos.Rows(0).Item("nombre_sec")
            Me.lblWeb.Text = dtDatos.Rows(0).Item("web_ofe").ToString
            Me.lblInicio.Text = dtDatos.Rows(0).Item("fechaInicioAnuncio").ToString
            Me.lblFin.Text = dtDatos.Rows(0).Item("fechaFinAnuncio").ToString
            Me.lblDpto.Text = dtDatos.Rows(0).Item("nombre_Dep").ToString
            Me.lblmodo.Text = IIf(dtDatos.Rows(0).Item("modopostular_ofe").ToString = " ", "No definido", IIf(dtDatos.Rows(0).Item("modopostular_ofe").ToString = "C", "Correo", "Web"))
            Me.lblEstado.Text = dtDatos.Rows(0).Item("estado_ofe").ToString
