...
var TimeGrid = Grid.extend({
	...
	slatRowHtml: function() {
		...
		/****/
		var slotNormal2 = this.slotDuration.asMinutes() % 15 === 0;

		var slotTime2 = moment.duration(+this.minTime); // wish there was .clone() for durations
		
		var slotDate2; // will be on the view's first day, but we only care about its time
		var minutes2;
		
		/***/

		// Calculate the time for each slot
		slotTime2.add(this.slotDuration);
		slotDate2 = this.start.clone().time(slotTime2);
				...
				'<td class="fc-axis fc-time ' + view.widgetContentClass + '" ' + view.axisStyleAttr() + '>' +
					((!slotNormal || !minutes) ? // if irregular slot duration, or on the hour, then display the time
						'<span>' + // for matchCellWidths
							htmlEscape(slotDate.format(this.axisFormat)) + '-' + htmlEscape(slotDate2.format(this.axisFormat)) +
						'</span>' :
						''
						) +
				'</td>';
...
var momComputableOptions = {
	...
	dayOfMonthFormat: function(momOptions, fcOptions) {
		...
		if (fcOptions.isRTL) {
		    ...
		    format = 'ddd '; // for LTR, add day-of-week to beginning
		}
		return format;
	},
