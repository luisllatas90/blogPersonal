...
Partial Class DataJson_Boletas_SelectBoletasPago_ajax
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        'Response.Write("<script>alert('" & codigo_per & "');</script>")
        ...
        Catch ex As Exception
            ...
            'Data.Add("codigo_per", codigo_per)
    ...
    Sub DatosPersona()
        ...
        tb = obj.TraerDataTable("dbo.USP_LISTAR_PERSONA_BOLETA_V2", "1", Session("id_per"))
    ...
    Sub SubirArchivo(ByVal CodigoPer As Integer)
        ...
        list.Add("CodigoPlla", tb.Rows(0)("Codigo"))
        list.Add("TipoPlla", tb.Rows(0)("CodigoTplla"))
        list.Add("Mes", tb.Rows(0)("Mes"))
        ...
        Dim envelope As String = wsCloud.SoapEnvelopeGenerarBoleta(list)
            ...
            Dim result As String = wsCloud.PeticionRequestSoap("http://serverdev/campusvirtual/ArchivosCompartidos/SharedFiles.asmx", envelope, "http://usat.edu.pe/GenerateTicket", Session("perlogin"))
            ...
            Dim Data As ResultMessage = ResultMessage(result)

            ' Response.Write(cif.EncrytedString64(Data.StatusBody.Code))
            Dim tblRes As New Data.DataTable
            If Data.Status = "OK" Then
                obj.AbrirConexion()
                tblRes = obj.TraerDataTable("dbo.USP_BOLETAPAGOPLANILLA", 0, CodigoPer, CodigoPlla, tb.Rows(0)("Periodo"), tb.Rows(0)("Mes"), Data.StatusBody.Code, "", Session("perlogin").ToString().Trim, HostName, IpRemoto, "R")
                obj.CerrarConexion()
	 ...
	 Catch ex As Exception
            ...
            Dim Data As New Dictionary(Of String, Object)()
            Data.Add("Message", ex.Message & " SUBIR ARCHIVO--- " & ex.StackTrace)
    ...
    Sub GenerarHojaCts(Optional ByVal codigoPer As String = "")
        ' CodigoPer = Request.Form("PersonaId")
        'Response.Write(Request.Form("PersonaId"))
        ' Response.Write(Desencriptar("PERU"))
        ' Dim b As New BinaryReader(post.InputStream)
        ...
        Try
            ...
            'tb = obj.TraerDataTable("dbo.USP_LISTARPLANILLA_PERSONA_test", "UN", CodigoPlla, 0, Session("id_per"))
            tb = obj.TraerDataTable("dbo.USP_LISTARPLANILLA_PERSONA", "UN", CodigoPlla, 0, Session("id_per"))
            obj.CerrarConexion()
            ...
            list.Add("ImpTc", "3.25")
            list.Add("CodigoPer", codigoPer.ToString())
            'list.Add("CodigoPer", "684")
            list.Add("Fecha", tb.Rows(0)("FechaIni"))
            list.Add("TablaId", 3) ' 7
            list.Add("codigoplanilla", CodigoPlla.ToString())
            list.Add("Usuario", Session("perlogin").ToString().Trim)
            'Response.Write("Codigo_plla : " & CodigoPlla & " CodigoPer: " & CodigoPer & " TipoPlla : " & tb.Rows(0)("CodigoTplla") & " Mes : " & tb.Rows(0)("Mes") & " Periodo : " & tb.Rows(0)("Periodo") & " Fecha : " & tb.Rows(0)("FechaIni"))
            Dim envelope As String = wsCloud.SoapEnvelopeGenerarCTS(list)
            'Response.Write(envelope)
            ''Dim result As String = wsCloud.PeticionRequestSoap("http://localhost/campusvirtual/ArchivosCompartidos/SharedFiles.asmx", envelope, "http://usat.edu.pe/GenerateCTS", Session("perlogin"))
            ''Dim result As String = wsCloud.PeticionRequestSoap("http://serverqa/campusvirtual/ArchivosCompartidos/SharedFiles.asmx", envelope, "http://usat.edu.pe/GenerateCTS", Session("perlogin"))
            Dim result As String = wsCloud.PeticionRequestSoap("http://serverdev/campusvirtual/ArchivosCompartidos/SharedFiles.asmx", envelope, "http://usat.edu.pe/GenerateCTS", Session("perlogin"))
    ...
    Private Sub DescargarArchivo()
        ...
        Try
            ...
            If Not Session("perlogin") Is Nothing Then
                ...
                'result = wsCloud.PeticionRequestSoap("http://serverqa/campusvirtual/ArchivosCompartidos/SharedFiles.asmx", envelope, "http://usat.edu.pe/DownloadFile", Usuario)
    ...
    Function ResultMessageCTS(ByVal cadXml As String) As ResultMessage
        ...
        'Dim Fecha As XmlNode = xml.DocumentElement.SelectSingleNode("/soap:Envelope/soap:Body/xm:GenerateCTSResponse/xm:GenerateCTSResult/xm:StatusBody/xm:Fecha", nsMgr)
        '  Dim mNombre = xml.ReadElementString("nombre")
        ' Return res.InnerXml '.InnerText
        '   Response.Write("dd" + res.InnerText)
        Dim cif As New ClsCRM
        ...
        result.StatusBody.Code = Code.InnerXml
        result.StatusBody.Message = Message.InnerXml
        'result.StatusBody.Fecha = Fecha.InnerXml
