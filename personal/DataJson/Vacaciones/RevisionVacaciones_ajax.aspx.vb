...
Partial Class DataJson_Vacaciones_Vacaciones_ajax
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Dim Periodo As String = Request.Form("Periodo")
            ...
            Select Case FPost
                ...
                Case "ListarVacaciones"
                    If Periodo = "TODOS" Then
                        Periodo = "0"
                    End If
                    ListarVacaciones(CInt(Periodo), CInt(codigo_per))
                Case "Generar"
                    'SubirArchivo(CInt(codigo_per))
                    SubirArchivo(CInt(codigo_dvac))
                    'Case "GenerarCTS"
                    '    GenerarHojaCts(CInt(codigo_per))
                Case "DescargarArchivo"
                    DescargarArchivo()
                Case "Periodo"   'Se activa el periodo
                    PeriodoContable()
                Case "DatosPersona"
                    DatosPersona()
            End Select
    ...
    Sub ListarVacaciones(ByVal Periodo As Integer, ByVal codigo_usu As Integer)
        ...
        tb = obj.TraerDataTable("dbo.USP_LISTARVACACIONES_PERSONA_V1", "CT", Periodo, codigo_usu, Session("id_per")) 'Cambié 0 por Periodo
        obj.CerrarConexion()
    ...
    Sub PeriodoContable()
        Try
            ...
            obj.CadenaConexion = ConfigurationManager.ConnectionStrings("CNXBDUSAT").ConnectionString
            obj.AbrirConexion()
            tb = obj.TraerDataTable("dbo.CONTA_PROCESOCONTABLE_MMVAC") 'Este e sun Nuevo Procedimiento
            obj.CerrarConexion()
    ...
    Sub DatosPersona()
        ...
        tb = obj.TraerDataTable("dbo.USP_LISTAR_PERSONA_VACACIONES_V1", "1", Session("id_per"))
    ...
    Sub SubirArchivo(ByVal Codigo_dvac As Integer)
        ...
        Try
            ...
            list.Add("CodigoDvac", CInt(Codigo_dvac)) 'CInt(tb.Rows(0)("Codigo_dvac"))) 'CInt(Codigo_dvac))
            ...
            Dim envelope As String = wsCloud.SoapEnvelopeGenerarVacaciones(list)
            ...
            Dim result As String = wsCloud.PeticionRequestSoap("http://serverdev/campusvirtual/ArchivosCompartidos/SharedFiles.asmx", envelope, "http://usat.edu.pe/GenerateVacaciones", Session("perlogin"))

            'Response.Write(result)
            Dim Data As ResultMessage = ResultMessageVacaciones(result)
            ...
            If Data.Status = "OK" Then
                obj.AbrirConexion()
                tblRes = obj.TraerDataTable("dbo.USP_MemoVacaciones", 0, tb.Rows(0)("Codigo_Per"), Codigo_dvac, Data.StatusBody.Code, "", Session("perlogin").ToString().Trim, HostName, IpRemoto, "R")
                obj.CerrarConexion()
    ...
    Function ResultMessageVacaciones(ByVal cadXml As String) As ResultMessage
        ...
        Dim Fecha As XmlNode = xml.DocumentElement.SelectSingleNode("/soap:Envelope/soap:Body/xm:GenerateVacacionesResponse/xm:GenerateVacacionesResult/xm:StatusBody/xm:Fecha", nsMgr)
        '  Dim mNombre = xml.ReadElementString("nombre")
        ' Return res.InnerXml '.InnerText
        '   Response.Write("dd" + res.InnerText)
        Dim cif As New ClsCRM
