...
Imports System.Data
Partial Class DataJson_tutoria_TipoEvaluacion
    ...
    Private Sub ListaTipoEvaluacion(ByVal tipo As String, ByVal codigo As Integer)
        Dim serializer As System.Web.Script.Serialization.JavaScriptSerializer = New System.Web.Script.Serialization.JavaScriptSerializer()
        Dim JSONresult As String = ""
        Dim list As New List(Of Dictionary(Of String, Object))()
        Try
            ...
            tb = obj.ListaTipoEvaluacion(tipo, codigo)

            If tb.Rows.Count > 0 Then
                For i As Integer = 0 To tb.Rows.Count - 1
                    Dim data As New Dictionary(Of String, Object)()
                    data.Add("cCod", obj.EncrytedString64(tb.Rows(i).Item("codigo_te")))
                    data.Add("cDescripcion", tb.Rows(i).Item("descripcion_te"))
                    data.Add("cOpe", tb.Rows(i).Item("operacion"))
                    data.Add("cSuma", tb.Rows(i).Item("aplica_suma"))
                    data.Add("cPeso", tb.Rows(i).Item("aplica_peso"))
                    data.Add("cProm", tb.Rows(i).Item("aplica_promedio"))

                    If tipo = "E" Then
                        data.Add("cEstado", tb.Rows(i).Item("estado_te"))
                    End If
                    list.Add(data)
                Next
            End If
