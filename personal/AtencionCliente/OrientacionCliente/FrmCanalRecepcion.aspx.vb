...
Partial Class OrientacionCliente_FrmCanalRecepcion
    ...
    Protected Sub gvLista_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles gvLista.RowCommand
        ...
        Try
            If (e.CommandName = "Editar") Then
                Me.Lista.Visible = False
                Me.DivMantenimiento.Visible = True
                Me.hdc.Value = Me.gvLista.DataKeys(e.CommandArgument).Values("codigo_ooc")
                Me.txtDescripcion.Text = Me.gvLista.DataKeys(e.CommandArgument).Values("nombre_ooc")
            End If
    ...
    Private Sub ActualizarTipo(ByVal codigo As Integer, ByVal descripcion As String, ByVal cod_encuesta As Integer, ByVal usuario As Integer)
        Dim obj As New ClsConectarDatos
        obj.CadenaConexion = ConfigurationManager.ConnectionStrings("CNXBDUSAT").ConnectionString
        obj.AbrirConexion()
        Dim dt As New Data.DataTable
        dt = obj.TraerDataTable("[OAC_ActualizarOrigenOrientacion]", codigo, descripcion, cod_encuesta, usuario)
        obj.CerrarConexion()
    ...
    Protected Sub lbGuardar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lbGuardar.Click
        ...
        If Me.txtDescripcion.Text.Trim = "" Then
            Me.ScriptManager1.RegisterStartupScript(Me.Page, Me.GetType(), "alert2", "fnMensaje('error','Ingrese nombre del canal de recepción')", True)
        Else
            ActualizarTipo(Me.hdc.Value, Me.txtDescripcion.Text, Me.hdcv.Value, Session("id_per"))
        End If
    End Sub
    ...
    Private Sub EliminarTipo(ByVal codigo As Integer, ByVal usuario As Integer)
        ...
        obj.AbrirConexion()
        Dim dt As New Data.DataTable
        dt = obj.TraerDataTable("[OAC_EliminarOrigenOrientacion]", codigo, usuario)
        obj.CerrarConexion()
        If dt.Rows(0).Item("Respuesta") = 1 Then
            Listar()
