$(document).ready(function() {
    fnLoading(true);
    var dt = fnCreateDataTableBasic('tTipoActividad', 2, 'asc');
    ope = fnOperacion(1);
    ////console.log(ope)
    fnBuscarTipoActividad();
    rpta = fnvalidaSession()
    //alert(rpta)
    if (rpta == true) {
        $('#btnListar').click(fnBuscarTipoActividad);
...
function fnBuscarTipoActividad(sw) {
    rpta = fnvalidaSession()
    if (rpta == true) {
        if (sw) { fnLoading(true); }
        //fnLoading(true)
        $("form#frmBuscarTipoActividad input[id=action]").remove();
        $('#frmBuscarTipoActividad').append('<input type="hidden" id="action" name="action" value="' + ope.lst + '" />');
        var form = $("#frmBuscarTipoActividad").serializeArray();
        $("form#frmBuscarTipoActividad input[id=action]").remove();
        //        //console.log(form);
        $.ajax({
            type: "POST",
            url: "../DataJson/tutoria/TipoActividad.aspx",
            data: form,
            dataType: "json",
            cache: false,
            success: function(data) {
                ...
                fnDestroyDataTableDetalle('tTipoActividad');
                $('#tbTipoActividad').html(tb);
                fnResetDataTableBasic('tTipoActividad', 2, 'asc');
...
function fnDelete(cod) {
    aDataR = {
        cod: cod,
        mensaje: '¿Desea Eliminar el Tipo de Actividad?'
    }
