$(document).ready(function() {
    fnLoading(true);
    var dt = fnCreateDataTableBasic('tTipoProblema', 2, 'asc');
    ope = fnOperacion(1);
    ////console.log(ope)
    fnBuscarTipoProblema();
    rpta = fnvalidaSession()
    //alert(rpta)
    if (rpta == true) {
        $('#btnListar').click(fnBuscarTipoProblema);
...
function fnBuscarTipoProblema(sw) {
    rpta = fnvalidaSession()
    if (rpta == true) {
        if (sw) { fnLoading(true); }
        //fnLoading(true)
        $("form#frmBuscarTipoProblema input[id=action]").remove();
        $('#frmBuscarTipoProblema').append('<input type="hidden" id="action" name="action" value="' + ope.lst + '" />');
        var form = $("#frmBuscarTipoProblema").serializeArray();
        $("form#frmBuscarTipoProblema input[id=action]").remove();
        //        //console.log(form);
        $.ajax({
            type: "POST",
            url: "../DataJson/tutoria/TipoProblema.aspx",
            data: form,
            dataType: "json",
            cache: false,
            success: function(data) {
                ...
                fnDestroyDataTableDetalle('tTipoProblema');
                $('#tbTipoProblema').html(tb);
                fnResetDataTableBasic('tTipoProblema', 2, 'asc');
...
function fnDelete(cod) {
    aDataR = {
        cod: cod,
        mensaje: '¿Desea Eliminar el Tipo de Problema?'
    }
