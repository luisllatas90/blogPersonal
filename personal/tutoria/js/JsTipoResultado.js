$(document).ready(function() {
    fnLoading(true);
    var dt = fnCreateDataTableBasic('tTipoResultado', 2, 'asc');
    ope = fnOperacion(1);
    ////console.log(ope)
    fnBuscarTipoResultado();
    rpta = fnvalidaSession()
    //alert(rpta)
    if (rpta == true) {
        $('#btnListar').click(fnBuscarTipoResultado);
...
function fnBuscarTipoResultado(sw) {
    rpta = fnvalidaSession()
    if (rpta == true) {
        if (sw) { fnLoading(true); }
        //fnLoading(true)
        $("form#frmBuscarTipoResultado input[id=action]").remove();
        $('#frmBuscarTipoResultado').append('<input type="hidden" id="action" name="action" value="' + ope.lst + '" />');
        var form = $("#frmBuscarTipoResultado").serializeArray();
        $("form#frmBuscarTipoResultado input[id=action]").remove();
        //        //console.log(form);
        $.ajax({
            type: "POST",
            url: "../DataJson/tutoria/TipoResultado.aspx",
            data: form,
            dataType: "json",
            cache: false,
            success: function(data) {
                ...
                fnDestroyDataTableDetalle('tTipoResultado');
                $('#tbTipoResultado').html(tb);
                fnResetDataTableBasic('tTipoResultado', 2, 'asc');
...
function fnDelete(cod) {
    aDataR = {
        cod: cod,
        mensaje: '¿Desea Eliminar el Tipo de Resultado?'
    }
