...
#Region "Eventos"
    ...
    Protected Sub gvListaDocByFirmar_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles gvListaDocByFirmar.RowCommand
        Try
            ...
            ''''''****************************************************************************** para traer el nombre del alumno si es que hay codigo del alumno
            'Dim md_sol As New d_SolicitaDocumentacion
            'Dim dtAlum As New Data.DataTable
            'dtAlum = md_sol.ListarSolicitaDocumentacion("ALU", codigo_sol, 0, "", 0)
            'If dtAlum.Rows.Count > 0 Then
            '    With dtAlum
            '        nomAluFile = .Rows(0).Item("apellidoPat_Alu") & "_" & .Rows(0).Item("apellidoMat_Alu") & "_" & .Rows(0).Item("nombres_Alu")
            '    End With
            'Else
            '    nomAluFile = ""
            'End If
                ...
                Case "firmar"
                    ...
                    If fileUpload.HasFile Then
                        ...
                        Else '' todo documento (Td)
                            ...
                            ' ************* para los documentos requiere aprobar una etapa del trámite ************************************
                            '----- trae las abreviaturas de los documentos por el codigo_cda
                            If mt_TraeAbreviaturasDocumento(codigo_cda) = "RESO-SUST-PGRA" Then 'resolución de sustentación de posgrado
                                '--- aprueba etapa del trámite
                                ...
                                '23/10/2020 cambio
                            ElseIf mt_TraeAbreviaturasDocumento(codigo_cda) = "FICH-FNOT-ACAD" Then
                                ...
                                'finalizo la etapa del trámite
                                '*************************************************************************************************************************
                                Dim rpt_tramite As String
                                rpt_tramite = mt_ApruebaEtapaTramiteByAdjunto(referencia01, idArchivoCompartido) '''' En este caso referencia 01 as codigo_dta
                                If rpt_tramite <> "" Then
                                    Call mt_ShowMessage(rpt_tramite, MessageType.error)
                                End If

                                'Envio correo con el documento adjunto
                                '*************************************************************************************************************************
                                Call mt_enviarCorreoAdjunto(codigo_sol, ruta_shf)
                                ' fin 23/102020
                            End If
...
#Region "Procedimientos y funciones"
    ...
    Private Function mt_ApruebaEtapaTramiteByAdjunto(ByVal codigo_dta As String, ByVal idArchivoCompartido As Integer) As String
        ...
        If codigo_dta <> 0 Or codigo_dta <> "" Then
            Dim dtTramite As New Data.DataTable
            dtTramite = mt_ActualizarEtapaTramiteByAdjunto(codigo_dta, "1", "A", "7", idArchivoCompartido)  'le he puesto 7 en duro hasta que cambien la configuracion del trámite
            If dtTramite.Rows.Count > 0 Then ' SI HAY RESPUESTA DEL CAMBIO DE ETAPA
