...
Partial Class frmprogramacioncademicaeve
    ...
    Public EnFechaLimite As Boolean = False
    ...
    Sub cargarFechaLimite()
        ...
        obj.CadenaConexion = ConfigurationManager.ConnectionStrings("CNXBDUSAT").ConnectionString
        obj.AbrirConexion()
        FechaLimite = CDate(obj.TraerDataTable("HORARIOPE_ConsultarAmbienteConfig", 1).Rows(0)(1).ToString)
        Me.lblLimite.Text = "Fecha límite para solicitar ambiente: " & FechaLimite
        obj.CerrarConexion()

        If Request.QueryString("mod") = 5 Then
            Me.lblLimite.Visible = False
        Else
            Me.lblLimite.Visible = True
        End If
    ...
    Protected Sub gridHorario_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gridHorario.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
           ...
           'No permitir solicitar ambiente si está fuera de fecha límite 
            If CDate(e.Row.Cells(2).Text) > CDate(FechaLimite.ToString("dd/MM/yyyy")) Then
                e.Row.Cells(10).Text = "-"
            End If
