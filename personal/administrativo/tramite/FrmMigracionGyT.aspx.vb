...
Partial Class administrativo_tramite_FrmMigracionGyT
...
#Region "Procedimiento"
    ...
    Private Sub CargaDatos()
        If fnValidarFiltroBuscar() Then
            ...
            Try
                ...
                If dt.rows.count > 0 Then
                    Me.btnMigrar.visible = True
                    Me.cpf.value = dt.rows(0).item("glosaCorrelativo_trl")
                    Me.ctdg.value = dt.rows(0).item("spmigracion")
                Else
                    Me.btnMigrar.visible = False
                    Me.cpf.value = ""
                    Me.ctdg.value = ""
    ...
    Private Sub MigrarTramite()
        'Response.Write(Me.ctdg.value & " " & Me.cpf.value)
        Dim obj As New ClsConectarDatos
        Dim dt As New Data.DataTable
        obj.CadenaConexion = ConfigurationManager.ConnectionStrings("CNXBDUSAT").ConnectionString
        'TRL-2020-14207
        ...
        Try
            ...
            If Me.ctdg.value <> "" And Me.cpf.value <> "" Then
                obj.AbrirConexion()
                dt = obj.TraerDataTable("TRL_" & Me.ctdg.value, Me.cpf.value)
                obj.CerrarConexion()

                If dt.rows.count > 0 Then
                    rpta = dt.rows(0).item("RPTA")
                    ShowMessage("Tramite migrado con éxito: ", MessageType.Warning)
                    CargaDatos()
                Else
                    ShowMessage("Ocurrió un problema al migrar el tramite: ", MessageType.Error)
                End If
            End If
            ' Response.Write("rpta: " & rpta)
    ...
    Private Function fnValidarFiltroBuscar() As Boolean
        Try
            If Me.txtnroTramite.Text.Trim <> "" Then
                Return True
            Else
