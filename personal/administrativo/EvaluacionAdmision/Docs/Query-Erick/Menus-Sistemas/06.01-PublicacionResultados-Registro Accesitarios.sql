...
DECLARE @codigo_Men AS INT = 0 , 
		@descripcion_Men AS NVARCHAR(510) = 'Registro Accesitarios' , --NOMBRE DEL MENU
		@enlace_Men AS NVARCHAR(510) = '../personal/administrativo/EvaluacionAdmision/PublicacionResultados/frmAccesitarios.aspx?mod=1' , --RUTA DEL FORMULARIO
		...
		@orden_men AS INT = 1 ,
		@variable_men AS NVARCHAR(100) = 'menu61' ,
		@formulario_men AS NVARCHAR(50) = '' ,
		@codigo_Tfu AS INT = 1 --PERFIL DE USUARIO		
		
		
SELECT @codigoRaiz_Men = codigo_Men FROM dbo.MenuAplicacion(NOLOCK) where codigo_Apl = 32 and descripcion_Men = 'Publicar Resultados'
