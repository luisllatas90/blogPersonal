...
$(document).ready(function () {
    ...
    $mdlMensajeClienteConfirmar.on('hide.bs.modal', function (e) {
        ...
        atenuarBoton($btnAccionPorConfirmar.attr('id'), true);
    });
    initFakeListener('btnFakeConfirmar', 'btnConfirmar');
...
function initFilterPlugins() {
    ...
    initSelectPicker('cmbFiltroTipoEvaluacion', {
        liveSearch: true,
        size: 8
    });
    initSelectPicker('cmbFiltroEvaluacion', {
        liveSearch: true,
        size: 8
    });
    initSelectPicker('cmbFiltroCarreraProfesional', {
        liveSearch: true,
        size: 8
    });
    initSelectPicker('cmbFiltroModalidadIngreso', {
        liveSearch: true,
        size: 8,
...
function initListPlugins() {
    $('#cllPuntajesMinimos').collapse();
}
...
function initFakeListener(fakeId, id) {
    $('body').on('click', '[id*="' + fakeId + '"]', function (e) {
        ...
        atenuarBoton($(this).attr('id'), false);
        ...
        switch (id) {
            case 'btnConfirmar':
                msg = '¿Realmente desea realizar esta operación? Al confirmar los resultados se procederá a cambiar la condición de ingreso de los postulantes según los filtro seleccionados';
                break;
        }
...
function exportar(id, type, fn, dl) {
    ...
    return dl ?
        ...
        XLSX.writeFile(wb, fn || ('RESULTADOS.' + (type || 'xlsx')));
}