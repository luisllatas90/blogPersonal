...
function initFiltrosPlugins() {  
    ...
    initSelectPicker('cmbFiltroCentroCostos', {
        liveSearch: true,
        size: 8
    });
}
...
function initFormPlugins() {
    initSelectPicker('cmbPersonal', {
        liveSearch: true,
        size: 10
    });

    initSelectPicker('cmbCompetencia', {});
}
...
function initFakeListener(fakeId, id) {
    $('body').on('click', 'button[id*="' + fakeId + '"]', function (e) {
        $btnAccionPorConfirmar = $(this);

        hideGrvTooltips();
        atenuarBoton($(this).attr('id'));

        var $realButton = $(this).closest('td').find('button[id*="' + id + '"]');
