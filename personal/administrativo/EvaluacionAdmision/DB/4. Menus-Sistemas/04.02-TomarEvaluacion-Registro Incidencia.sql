...
DECLARE @codigo_Men AS INT = 0 , 
		@descripcion_Men AS NVARCHAR(510) = 'Registro de Incidencias' , --NOMBRE DEL MENU
		@enlace_Men AS NVARCHAR(510) = '../personal/administrativo/EvaluacionAdmision/TomarEvaluacion/frmRegistroIncidentes.aspx?mod=1' , --RUTA DEL FORMULARIO
		...
		@orden_men AS INT = 2 ,
		@variable_men AS NVARCHAR(100) = 'menu42' ,
		@formulario_men AS NVARCHAR(50) = '' ,
		@codigo_Tfu AS INT = 1 --PERFIL DE USUARIO		
		
		
SELECT @codigoRaiz_Men = codigo_Men FROM dbo.MenuAplicacion(NOLOCK) where codigo_Apl = 32 and descripcion_Men = 'Tomar Evaluación'
