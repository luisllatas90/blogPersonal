...
If Not(rsPersonal.BOF and rsPersonal.EOF) then
	...
	Set rsdatoscurso=ObjCnx.Consultar("EAD_ConsultarCursoEvaluacionDocente","FO", cdbl(session("codigo_Usu")))
	...
	IF (sw = 1)  THEN
			...
			id = ObjCif.cifrado(trim(left(session("Ident_Usu"),8) & session("codigo_Usu")), "EncuestaDocente")
			response.redirect "academico/encuesta/EvaluacionAlumnoDocente/EvaluacionDocente_Docente.aspx?id=" & id  & "&mo=L" 
	end if
	'##########################
