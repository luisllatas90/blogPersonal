...
if fechainicio="" then fechainicio= dateadd("m",-4,date)
if fechafin="" then fechafin=date

Set obj= Server.CreateObject("PryUSAT.clsAccesoDatos")
	obj.AbrirConexion
		if (codigo_tfu=1 or codigo_tfu=35) then
			Set rsServicio= obj.Consultar("ConsultarServicioConcepto","FO","LI",0)
		else
			Set rsServicio=obj.Consultar("ConsultarAccesoRecurso","FO",6,codigo_usu,0,0)
		end if
		
		if codigo_sco<>"-2" then
			Set rsMoneda=obj.Consultar("ConsultarServicioConcepto","FO","CO",codigo_sco)
			Set rsClientes=obj.Consultar("ConsultarDeudasXCobrar","FO",0,fechainicio,fechafin,codigo_sco,0,"","",0,"","")		
...
<script type="text/javascript" language="javascript">
	...
	{
		pagina="../administrativo/controlpagos/vstdeudascobrar.asp?codigo_sco=" + cbocodigo_sco.value + "&fechainicio=" + txtFechaInicio.value + "&fechafin=" + txtFechaFin.value + "&moneda=S"
...
<table cellpadding="3" style="width: 100%" class="contornotabla">
	<tr>
		...
		<td colspan="5">
		<%call llenarlista("cbocodigo_sco","",rsServicio,"codigo_sco","nombre",codigo_sco,"Seleccione el servicio","","")%>
		</td>
	</tr>
	<tr>
...
if codigo_sco<>"-2" then
	...
	<%else
		ArrEncabezados=Array("Fecha","Código","Cliente","Area/Escuela","cantidad", "Cargos","Abonos","Saldos")
		ArrCampos=Array("fecha","cod. resp.","cliente","carrera","cantidad","cargos","abonos","diferencia")
		ArrCeldas=Array("15%","10%","30%","15%","10%", "10%","10%","10%")
	...
	<table bgcolor="white" bordercolor="silver" border="1" cellpadding="3" style="width: 100%;border-collapse:collapse">
		...
		Do while not rsClientes.EOF
			i=i+1
			cantidad=iif(isnull(rsClientes("cantidad"))=true,1,rsClientes("cantidad"))
		...
		<tr <%=clase%> onMouseOver="Resaltar(1,this,'S')" onMouseOut="Resaltar(0,this,'S')">
			...
			<td align="center"><%=rsClientes("cantidad")%>&nbsp;</td>
		...
		<tr class="usatTablaInfo">
			...
			<td align="right" style="font-weight: 700"><%=cantEntradas%>&nbsp;</td>
...
<p class="usattitulousuario">Resumen:
<% 

    if codigo_Sco = 952 then
        response.write cstr(cantEntradas) & " Entradas vendidas"
 end if%>
</p>
<table style="width: 50%" class="contornotabla">
	<tr>
		<td style="width: 90%">Deudas que se han cancelado todo el cargo asignado</td>
		<td width="10%">: <%=a%></td>
	</tr>
	<tr>
		<td style="width: 90%">Deudas que les falta cancelar el cargo asignado</td>
		<td width="10%">: <%=f%>&nbsp;</td>
	</tr>
	<tr>
