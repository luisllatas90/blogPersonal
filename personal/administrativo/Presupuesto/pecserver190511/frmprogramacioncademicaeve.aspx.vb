...
Partial Class frmprogramacioncademicaeve
    ...
    Private Sub ConsultarFechasCronograma()
        Dim DatCronograma As New Data.DataTable
        Dim obj As New ClsConectarDatos
        obj.CadenaConexion = ConfigurationManager.ConnectionStrings("CNXBDUSAT").ConnectionString
        obj.AbrirConexion()
        DatCronograma = obj.TraerDataTable("ACAD_ConsultarCronogramaxTipo", "PROGCAR", dpCodigo_cac.selectedvalue, Request.QueryString("mod"))
        obj.CerrarConexion()
        obj = Nothing

        If DatCronograma.Rows.Count > 0 Then
            Me.lblIntervalo.Text = "al"
            Me.lblMsjFechas.Text = "Fechas definidas para la programación: "
            Me.lblFechaIni.Text = DatCronograma.Rows(0).Item("fechaIni_Cro")
            Me.LblFechaFin.Text = DatCronograma.Rows(0).Item("fechaFin_Cro")
        Else
            Me.lblIntervalo.Text = ""
            Me.lblMsjFechas.Text = ""
            Me.lblFechaIni.Text = ""
            Me.LblFechaFin.Text = ""
        End If
    End Sub
