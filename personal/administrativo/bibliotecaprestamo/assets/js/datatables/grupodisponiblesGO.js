...
$(document).ready(function() {
            ...
            if (data.length > 0) {
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    if (data[i].vtc == "I") {
                        ...
                        tabla += '<br><sup style="font-weight:bold;">Tu plan de estudios te exige llevar ' + msjel + '</sup>';
                                ...
                                if (!(data[i].vvec > 0 && data[i].vCic == 1 && data[i].soloPrimerCiclo_cup == 'True')) {
                                    if (i < (data.length - 1)) {
                                        ...
                                        if (data[i].codigo_cup != data[i + 1].codigo_cup) {
                                            ...
                                            sOut += '<tr style="color:' + _null + '" valign="top">';
                                            ...
                                            jQuery.each(data, function(j, hor) {
                                                if (hor.codigo_cup == data[i].codigo_cup) {
                                                    sOut += '<tr valign="top">';
                                                    sOut += '<td align="left" width="50%"><input type="hidden" id="txtdia' + hor.codigo_cup + '[' + c + ']" name="txtdia' + hor.codigo_cup + '[' + c + ']" value="' + hor.dia_Lho + '"/><input type="hidden" id="txtfecIni' + hor.codigo_cup + '[' + c + ']" name="txtfecIni' + hor.codigo_cup + '[' + c + ']" value="' + hor.nombre_Hor + '"/><input type="hidden" id="txtfecFin' + hor.codigo_cup + '[' + c + ']" name="txtfecFin' + hor.codigo_cup + '[' + c + ']" value="' + hor.horaFin_Lho + '"/>' + hor.Horas + '</td>';
                                                    sOut += '<td align="left" width="50%">' + hor.ambiente + '</td>';
    ...
    function fnDataHorario() {        
      ...
      /*  for (var idx in arrCursos) {
            ...
            cr = cruceHorarioSession(key.cup);
    ...
    function eventoCursoCargaHorario(cod, nom, ccur, dia, f1, f2, ccup, vcs, accion, cm, rem, vtc) {
        ...
        if (accion == "A") {
            ...
            cr = cruceHorarioSession(ccup);
            CalendarAgregaEvento(ccup, '', '', false, cr);

        }
        else {
            delete arrCursos[dia + f1 + ccup];
            delete arrData[ccup];
            cr = cruceHorarioSession(0);
            CalendarAgregaEvento(0, '', '', false, cr);
        }
    ...
    function fnValidaCursoInferiorJSON(c, e) {
        ...
        if (!fnValidaSeleccionElectivo(c)) {
            ...
            if (!fnValidaSeleccionComplementario(c)) {
                ...
                fnMensaje('warning', 'Debes seleccionar un curso complementario (cómputo)');
                return false;
            } else if (!fnValidaSeleccionIdioma(c)) {
                fnMensaje('warning', 'Debes curso complementario (Idioma)');
                return false;
                ...
                if (!fnValidaSeleccionInferior(c)) {
                    ...
                    //if (fnValidaSeleccionInferiorIdioma(c)) {
                    // fnMensaje('warning', '2.Debes curso complementario (Idioma)');
                    //   return false;
                    // } else {
                    return true;
                    //   }
    ...
    function fnValidaSeleccionIdioma(cur) {
        //if (aDataCnf[0].rIdi) {
        //    return true;
        //} else {
        var sw = true;
        ...
        // swSI = fnBuscarData(cur, "sIdi", "");
        //console.log("fnValidaSeleccionIdioma" + swSI);
        // sw = swSI;
        //  console.log(swSI);
        // if (swSI) {
        //  console.log('selcciona idioma');
        for (i = 0; i < aData.length; i++) {
            sThisVal = (aData[i].chk ? "1" : "0");


            //if (aData[i].vtc == 'I' && parseInt(aData[i].vnidi) > parseInt(aData[i].vidiap) && parseInt(aData[i].vCic) < parseInt(cicSel) && parseInt(aData[i].stt) == 1) { // && sThisVal == "0" && ) {
            if (aData[i].vtc == 'I' && parseInt(aData[i].vCic) < parseInt(cicSel) && parseInt(aData[i].stt) == 1 && aData[i].sIdi == 1) { // && sThisVal == "0" && ) {

                if (sThisVal == "0") {

                    if (fnValidaSeleccionIdiomaCicloChk(aData[i].vCic)) {
                        sw = false;
                        i = aData.length;
                    }
                }
    ...
    function fnValidaSeleccionIdiomaCicloChk(c) {
        var sThisVal = "";
        var sw = true;

        for (i = 0; i < aData.length; i++) {
            if (parseInt(aData[i].vCic) == parseInt(c) && aData[i].vtc == 'I') {
                sThisVal = (aData[i].chk ? "1" : "0");
    ...
    function fnRetiro(dm, c) {
        ...
        if (fnValidaRetiro(dm, c)) {
            ...
            $.ajax({
                ...
                contentType: "application/json; charset=utf-8",
                url: "procesar.aspx",
                data: { "param0": "retMatGO", "param1": dm, "r": r, "m": m, "t": t },
                dataType: "json",
                success: function(data) {

                    if (data[0].R == "OK") {
                        fnMensaje("success", data[0].Mensaje);
                        f_Menu('matriculaGO.aspx');
                    }
                    else {
    ...
    function fnValidaRetiro(dm, c) {
        ...
        if (!fnValidaRetiroComplementario(c)) {
            ...
            if (parseInt(tc) < 12) {
                fnMensaje('warning', 'No puedes retirarte de este curso. Cr&eacute;ditos mínimos para llevar en ciclo regular: 12');
                return false;
            } else {
    ...
    function fnValidaSeleccionInferiorIdioma(cur) {
        var sw = true;
        sw = parseInt(fnBuscarData(cur, "sIdi", ""));
    ...
    function fnValidaSeleccionElectivo(cur) {
        ...
        var nel = fnElectivoSel();

        var total = parseInt(aDataCnf[0].elecAprob) + nel;

        if (total >= parseInt(aDataCnf[0].elecPes)) {
            aDataCnf[0].rElec = true;
        } else {
            aDataCnf[0].rElec = false;
        }
    ...
    function fnElectivoSel() {
        var s = 0;
        var i;
        var sw = false;
        for (i = 0; i < aData.length; i++) {
            if (aData[i].electivo_cur == 'True') {

                sThisVal = (aData[i].chk ? "1" : "0");
                if (sThisVal == "1") {

                    if (fnElectivoMat(aData[i].codigo_cur)) {
                        s++;
                    }
                }
            }
    ...
    function fnValidaSeleccionElectivoCicloChk(c) {
        ...
        for (j = 0; j < aData.length; j++) {
            if (parseInt(aData[j].vCic) == parseInt(c) && aData[j].electivo_cur == 'True') {
                Elec = parseInt(aData[j].vnel - parseInt(aData[j].velap));
                sThisVal = (aData[j].chk ? "1" : "0");
                if (sThisVal == "1") {
    ...
    try{
        ...
        if (selCurso.length == 0) {
            ...
            if (selCred > 0) {
                ...
                if (parseInt(aDataCnf[0].maxCred) > 0 && sumcredSel > aDataCnf[0].maxCred) {
                    $('#h6CostoCuota2').html(aDataCnf[0].montoCredpen.toFixed(2));
                    $('#h6CostoCiclo2').html((aDataCnf[0].montoCredpen * cuotas).toFixed(2));
                    $('#h6CostoCuota').hide();
                    $('#h6CostoCiclo').hide();
    ...
    function cruceHorarioSession(cup) {
        var _cup = "";
        var sw;
        try{
            ...
            for (var i in arrCursos) {
                obj = arrCursos[i];
             // console.log(obj);
             // console.log(cup + ' !=  ' + obj.cup);
                _cup += obj.cup + ',';
                
            }

            console.log(cup + ' !=  ' + _cup); 
            
            sw = DevuelveFechaCalendarioSession(cup, _cup);
    ...
    function DevuelveFechaCalendarioSession(cup, _cup) {
        var cr=[];
        ...
        $.ajax({
            ...
            contentType: "application/json; charset=utf-8",
            url: "procesar.aspx",
            data: { "param0": "chses", "param2": cup, "param3": _cup },
            dataType: "json",
            async: false,
            success: function(data) {
                var i;
    ...
    function fnGuardar() {
        ...
        $.ajax({
            ...
            contentType: "application/json; charset=utf-8",
            url: "procesar.aspx",
            data: { "param0": "regMatgo", "param1": obj, "M": m, "O": o, "T": t },
            dataType: "json",
            success: function(data) {
                 console.log(data);
                if (data[0].R == "C") {
