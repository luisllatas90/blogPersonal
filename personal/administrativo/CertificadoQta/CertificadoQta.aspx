<%@ Page Language="VB" AutoEventWireup="false" CodeFile="CertificadoQta.aspx.vb" Inherits="administrativo_CertificadoQta_CertificadoQta" %>
...
//                      url: "../../DataJson/CertificadoQta/SelectCertificadoQta_ajax.aspx",
...
//              }
              ...
              function Persona() {
                  $.ajax({
                      type: "POST",
                      // contentType: "application/json; charset=utf-8",
                      url: "../../DataJson/CertificadoQta/CertificadoQta_ajax.aspx",
              ...
              function ListarCertQuinta() {
                ...
                $.ajax({
                    ...
                    data: { "Funcion": "ListarCertQuinta", "PersonaId": $("#Persona").val() },
                    dataType: "json",
                    cache: false,
                    success: function(data) {
                        var row = 1;                        
                        ...
                        jQuery.each(data, function(i, val) {
                            ...
                            var generado = val.VacGenerado;
                            var btn = "";
                            if (generado == "1") {
                                btn = '<a href="#" id=' + val.IdArchivosCompartidos + '   name=' + val.IdArchivosCompartidos + ' onclick="DescargarArchivo(\'' + val.IdArchivosCompartidos + '\');" class="btn btn-primary btn-icon-primary btn-icon-block btn-icon-blockleft" data-toggle="modal" data-target="#ModalPrint" data-whatever="@getbootstrap"><i  class="ion  ion-ios-printer-outline"></i><span>Descargar Certificado Renta de Quinta</span></a>';
                            } else {
                            btn = '<a href="#"  id=' + val.Codigo + ' name=' + val.Codigo + ' onclick="GenerarCertificadoQta(\'' + val.Codigo + '\',this);" class="btn btn-green btn-icon-green btn-icon-block btn-icon-blockleft" data-toggle="modal" data-target="#exampleModal" data-whatever="@getbootstrap"><i  class="ion ion-checkmark"></i><span>Aceptar Certificado Renta de Quinta &nbsp;&nbsp;&nbsp;</span></a>';
                            }

                            sOut += '<tr>';
                            //sOut += '<td>' + val.codigo_dvac + ' ' + '</td>';
                            sOut += '<td>' + val.FechaIni + ' ' + '</td>';
                            sOut += '<td>' + val.FechaFin + ' ' + '</td>';
                            //sOut += '<td>' + val.NroDias + ' ' + '</td>';
                            sOut += '<td>' + val.NroMemo + ' ' + '</td>';
                            sOut += '<td>';
            ...
            function GenerarCertificadoQta(codigo, name) {
                $('.piluku-preloader').removeClass('hidden');
                var flag = false;
                    ...
                    success: function(data) {
                        ...
                        if (data[0].Status == "OK") {
                            ...
                            if (navigator.userAgent.indexOf("NET") > -1) {
                                ...
                                window.open("../../DataJson/CertificadoQta/DescargarArchivo.aspx?Id=" + IdArchivo, 'ta', "");
                                                          } else {
        ...
        <div class="panel panel-piluku">
            <div class="panel-heading">
                <h3 class="panel-title">
                    Certificado de Rentas de Quinta Categoría</h3>
             </div>
                        ...
                        <form  method="post" >
                            ...
                            <div class="col-md-10 col-sm-10 col-xs-10"  style="float:left">
                                <div class="form-group">
                                    <label class="col-sm-2 col-xs-2 control-label"><b>Trabajador :</b></label>
                                    ...
                                    <table class="display dataTable cell-border" id="tablaRendiciones">
                                        <thead>
                                            <tr role="row">		
                                                ...
                                                <th style="width:8%">Período Inicial</th>
                                                <th style="width:8%">Período Final</th>
                                                <th style="width:8%"> </th> 
