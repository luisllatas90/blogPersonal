<%@ Page Language="VB" AutoEventWireup="false" CodeFile="registroActivo_v3.aspx.vb" Inherits="administrativo_activofijo_registroActivo_v3" %>
...
//        alert($('#txtNroPedido').val());
    ...
    function fnRegistrarSerie(fila, c_det, activo, u_egreso, u_art, serie) {
        //alert("fila: " + fila);
        //alert("c_det: " + c_det)
        //alert("activo: " + activo);
        ...
        $('#txtSerie').val(serie);
        $('#hd_uArt').val(u_art);
        $('#hd_uEgr').val(u_egreso);

        $("input#param0").val("LstDetalleRegistroAF_v2");
        var form = $('#frmRegistroActivo').serialize();
        $.ajax({
            type: "GET",
            contentType: "application/json; charset=utf-8",
            ...
            data: { "param0": "LstDetalleRegistroAF_v2", "param1": $('#txtNroPedido').val(), "hd_uArt": $('#hd_uArt').val(), "hd_uEgr": $('#hd_uEgr').val() },
            dataType: "json",
            cache: false,
            async: false,
            success: function(data) {
                if (data.length > 0) {
                    ...
                    //console.log(data);
                    aDataDR = data;
                    $('#hdSerie').val(aDataDR[fila].c_ser);
                    $('#txtSerie').val(aDataDR[fila].d_ser);
                    document.getElementById('txtSerie').disabled = true;
    ...
    function fnRegistrarSerie_bk(fila, c_det, activo, u_egreso, u_art, serie) {
        //alert("fila: " + fila);
        //alert("c_det: " + c_det)
        //alert("activo: " + activo);
        //alert("F: " + fila + " D:" + c_det + " A:" + activo);
            ...
            if ($('#hd_sw').val() == 1) {
                ...
                if ($('#txtCSerie\\[' + cont + '\\]').val() == '-' && $('#txtNroComp\\[' + cont + '\\]').val() > 0) {
                    ...
                    if ($('#txtCSerie\\[' + cont + '\\]').val() == '-' && $('#txtNroComp\\[' + cont + '\\]').val() == '-') {
                        //alert($('#txtCSerie\\[' + cont + '\\]').val() + " - " + $('#txtNroComp\\[' + cont + '\\]').val());
                        //$('#txtSerie').val("");
                        document.getElementById("btnGuardarCrt").disabled = true;
                        //document.getElementById('txtSerie').disabled = false;
                        //document.getElementById('divBtnActualizaSerie').style.display = 'block';
            ...
            success: function(data) {
                ...
                if (data.length > 0) {
                    for (var i = 0; i < aDataAC.length; i++) {
                        ...
                        t += '<td id=' + aDataAC[i].c_hart + '>' + aDataAC[i].d_com + '</td>';
                        t += '</tr>';
                    }
                }
    ...
    function fnCrtComp(c_art) {
        //alert(document.getElementById(c_art).innerText);
        var nombreart = document.getElementById(c_art).innerText;
        $.ajax({
            type: "GET",
            contentType: "application/json; charset=utf-8",
            url: "../../DataJson/activofijo/processactivofijo.aspx",
            data: { "param0": "lstRegistroActivo", "param1": $('#param1').val(), "tipo": "DP", "param2": $('#param2').val(), "hdIEgreso": $('#hdIEgreso').val() },
            dataType: "json",
            async: false,
            success: function(data) {
                //console.log("-------");
    ...
    function focusEgresado(fila, j, cdet, txtSerieF) {
        if (txtSerieF.value.length > 0) {
            ...
            $('#hdNumeroEs').val(fila);
            $('#hdAccion').val("ACT");
            $('#txtSerie').val(txtSerieF.value);
            //alert("Focus EGR: " + $('#txtSerie').val());
            $("input#param0").val("gActualizarSerieEgreso");
            var form = $('#frmRegistroActivo').serialize();
            $.ajax({
                type: "GET",
                ...
                url: "../../DataJson/activofijo/processactivofijo.aspx",
                //data: form,
                data: { "param0": "gActualizarSerieEgreso", "txtNroPedido": $('#txtNroPedido').val(), "hdIdEgresoS": $('#hdIdEgresoS').val(), "hdArt": $('#hdArt').val(), "hdUnid": $('#hdUnid').val(), "hdNumeroEs": $('#hdNumeroEs').val(), "hdAccion": $('#hdAccion').val(), "txtSerie": $('#txtSerie').val() },
                dataType: "json",
                success: function(data) {
                    //console.log("------");
                    //console.log(data);
                    ...
                    //document.getElementById(txtSerie).disabled = true;
                    $(txtSerieF).attr('disabled', 'disabled');
                    //$(txtSerie).attr('disabled', true);
                    //$('#tbdetalleaf .1').attr('disabled', true);
                    document.execCommand('ClearAuthenticationCache');
                    //location.reload();
                    //ActualizarTablaDetalle();
                    $('.piluku-preloader').addClass('hidden');
...
<form id="frmRegistroActivo" name="frmRegistroActivo" action="#"  > 
    ...
    <input type="hidden" id="hd_uArt" name="hd_uArt" value="" runat="server" />
    <input type="hidden" id="hd_uEgr" name="hd_uEgr" value="" runat="server" />
    <input type="hidden" id="hd_sw" name="hd_sw" value="" />
    ...
    <div class="panel panel-piluku">
	    ...
	    <div class="panel-body">								
	        <div class="col-md-12">
                ...
                <div class="row">			            
                <div class="form-group">
                    <script type="text/javascript" src='assets/js/datatables/activofijo_v2.js'></script>
...
<%--			        <div class="row">
                    ...
                    <div id='divCrtCom' style='display:none;'>
                        ...
                        <div id="DivsinCrtCom" style="display:none">
                            <center><label style="color:Red">Sin Caracaterísticas</label></center>
                        </div>
                        <br />
