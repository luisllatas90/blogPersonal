<%@ Page Language="VB" AutoEventWireup="false" CodeFile="VerVacacionesPersonal.aspx.vb" Inherits="administrativo_Vacaciones_Vacaciones" %>
    ...
    <script src="../../academico/assets/js/PDFObject/pdfobject.min.js" type="text/javascript"></script>
          ...
          <script type="text/javascript">
              $(document).ready(function() {
                  ...
                  //});
                  Periodo(); //se activa esto para ver los periodos
                  Persona();
              ...
              function Periodo() {
                  $.ajax({
                      ...
                      url: "../../DataJson/Vacaciones/RevisionVacaciones_ajax.aspx",
              ...
              function Persona() {
                  $.ajax({
                      ...
                      success: function(data) {
                          var row = 1;
                          console.log(data); // añado esta línea, Vacaciones.aspx no lo tiene
                          var sOut = '';
                          $("#Persona").html("");
                          jQuery.each(data, function(i, val) {
              ...
              function ListarVacaciones() {
                ...
                $.ajax({
                    ...
                    data: { "Periodo": $("#Periodo").val(), "Funcion": "ListarVacaciones", "PersonaId": $("#Persona").val() },
                    dataType: "json",
                    cache: false,
                    success: function(data) {
                        var row = 1;                        
                        ...
                        jQuery.each(data, function(i, val) {
                            ...
                            var btn = "";
                            if (generado == "1") {
                                btn = '<a href="#" id=' + val.IdArchivosCompartidos + '   name=' + val.IdArchivosCompartidos + ' onclick="DescargarArchivo(\'' + val.IdArchivosCompartidos + '\');" class="btn btn-primary btn-icon-primary btn-icon-block btn-icon-blockleft" data-toggle="modal" data-target="#ModalPrint" data-whatever="@getbootstrap"><i  class="ion  ion-ios-printer-outline"></i><span>Descargar Memorandums de Vacaciones</span></a>';
                            } else {
                                btn = '<a href="#"  id=' + val.Codigo + ' name=' + val.Codigo + ' onclick="GenerarVacaciones(\'' + val.Codigo + '\',this);" class="btn btn-green btn-icon-green btn-icon-block btn-icon-blockleft" data-toggle="modal" data-target="#exampleModal" data-whatever="@getbootstrap"><i  class="ion ion-checkmark"></i><span>Aceptar Memorandums de Vacaciones &nbsp;&nbsp;&nbsp;</span></a>';
                            }
            ...
            function GenerarVacaciones(codigo, name) {
                $('.piluku-preloader').removeClass('hidden');
                var flag = false;
                    ...
                    success: function(data) {
                        ...
                        if (data[0].Status == "OK") {
                            ...
                            if (navigator.userAgent.indexOf("NET") > -1) {
                                ...
                                window.open("../../DataJson/Vacaciones/DescargarArchivo.aspx?Id=" + IdArchivo, 'ta', "");
                                                          } else {
