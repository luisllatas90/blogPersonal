...
eval ("frminstitucion." + objeto + ".readOnly=false")
	...
	function EnviarResponsable(codigo_prp,codigo_per,codigo_cco,informe){
		...
		codigo_cco1=frmResponsable.cbocentrodecostos.value	
//S		alert(codigo_cco1)
		document.all.cargar.style.visibility="visible"
		location.href="calificar_propuesta.asp?accion=guardar&codigo_prp="+codigo_prp+"&codigo_per="+codigo_per+"&codigo_cco="+codigo_cco1+"&informe="+informe //  (instancia B) proponente que guarda como borrador |  (instancia P) proponente que envìa como director
		//frmResponsable.submit()
		window.opener.location.reload()
	}

	function DerivarResponsable(codigo_prp,codigo_per,codigo_cco,informe){
		//alert(informe)
		codigo_per=frmResponsable.cboPersonal.value
		codigo_cco1=frmResponsable.cbocentrodecostos.value	
		document.all.cargar.style.visibility="visible"
		location.href="calificar_propuesta.asp?accion=derivar&codigo_prp="+codigo_prp+"&codigo_per="+codigo_per+"&codigo_cco="+codigo_cco1+"&informe="+informe //  (instancia B) proponente que guarda como borrador |  (instancia P) proponente que envìa como director
		//frmResponsable.submit()
		window.opener.location.reload()
	}
	function Observar(codigo_prp,codigo_per,codigo_cco,informe){
		//alert(informe)
		codigo_per=frmResponsable.cboPersonal.value
		codigo_cco1=frmResponsable.cbocentrodecostos.value	
		document.all.cargar.style.visibility="visible"
		location.href="calificar_propuesta.asp?accion=observar&codigo_prp="+codigo_prp+"&codigo_per="+codigo_per+"&codigo_cco="+codigo_cco1+"&informe="+informe //  (instancia B) proponente que guarda como borrador |  (instancia P) proponente que envìa como director
		//frmResponsable.submit()
		window.opener.location.reload()
	}	
	function Denegar(codigo_prp,codigo_per,codigo_cco,informe){
		//alert(informe)
		codigo_per=frmResponsable.cboPersonal.value
		codigo_cco1=frmResponsable.cbocentrodecostos.value	
		document.all.cargar.style.visibility="visible"
		location.href="calificar_propuesta.asp?accion=denegar&codigo_prp="+codigo_prp+"&codigo_per="+codigo_per+"&codigo_cco="+codigo_cco1+"&informe="+informe //  (instancia B) proponente que guarda como borrador |  (instancia P) proponente que envìa como director
		//frmResponsable.submit()
		window.opener.location.reload()
	}		
    ...
    Set objProp=Server.CreateObject("PryUSAT.clsAccesoDatos")
   	objProp.AbrirConexionTrans
		if informe_prp="IN" then
			objProp.Ejecutar "ModificarEstadoPropuesta",false,"ap",codigo_prp,"A"
...
if Request.QueryString("accion")="derivar" then
	codigo_apr=Request.QueryString("codigo_apr")
    Set objProp=Server.CreateObject("PryUSAT.clsAccesoDatos")
   	objProp.AbrirConexion
			objProp.Ejecutar "ModificarEstadoPropuesta",false,"ap",codigo_prp,"D"
...
if Request.QueryString("accion")="observar" then
	codigo_apr=Request.QueryString("codigo_apr")
    Set objProp=Server.CreateObject("PryUSAT.clsAccesoDatos")
   	objProp.AbrirConexion
			objProp.Ejecutar "ModificarEstadoPropuesta",false,"ap",codigo_prp,"O"
	objProp.CerrarConexion
	set objProP=nothing
	Response.Write("<script>window.close()</script>")
...
if Request.QueryString("accion")="denegar" then
	codigo_apr=Request.QueryString("codigo_apr")
    Set objProp=Server.CreateObject("PryUSAT.clsAccesoDatos")
   	objProp.AbrirConexion
			objProp.Ejecutar "ModificarEstadoPropuesta",false,"ap",codigo_prp,"R"
	objProp.CerrarConexion
	set objProP=nothing
	Response.Write("<script>window.close()</script>")
    ...
    <tr>
      <td bgcolor="#F0F0F0" class="bordeinf"><table  width="95%" height="100%" border="0" align="center" cellpadding="0" cellspacing="5">
        <tr>
		  ...
		  <input name="Submit" type="button" onClick="EnviarResponsable('<%=codigo_prp%>','<%=codigo_per%>','<%=centrocosto%>','<%=informe_prp%>')" class="conforme1" value="         Aprobar"> 
		  
		  <input name="Submit2" type="button" onClick="Observar('<%=codigo_prp%>','<%=codigo_per%>','<%=centrocosto%>','<%=informe_prp%>')" class="editar1" value="          Observar">

          <input name="Submit3" type="button" onClick="Denegar('<%=codigo_prp%>','<%=codigo_per%>','<%=centrocosto%>','<%=informe_prp%>')" class="noconforme1" value="         Denegar">

          <input name="Submit42" type="button" onClick="DerivarResponsable('<%=codigo_prp%>','<%=codigo_per%>','<%=centrocosto%>','<%=informe_prp%>')" class="enviarpropuesta" value="          Derivar" style="visibility:hidden"></td>
	    	<%	    
		  	Set objProp=Server.CreateObject("PryUSAT.clsAccesoDatos")
	     	objProp.AbrirConexion
