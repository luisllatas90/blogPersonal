		...
		<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">
			  <tr>
				...
				<td width="10%"><input name="cmdver" disabled="disabled" type="button" class="ver2" id="cmdver" onClick="AbrirPopUp('registrapropuesta.asp?modifica=2&codigo_prp=' + txtelegido.value.substring(4,txtelegido.value.length) + '&instancia_prp=<%=instancia%>','430','600')" value="         Ver" style="visibility:hidden"></td>
					...
					<td align="right">&nbsp;</td>
					<td width="10%"><input onClick="darveredicto('N','<%=Request.QueryString("menu")%>','<%=Request.QueryString("instancia")%>','<%=Request.QueryString("estado")%>','<%=Request.QueryString("informe")%>','<%=Request.QueryString("inf")%>')" name="cmdnoconforme" type="submit" disabled="disabled" class="noconforme1" id="cmdnoconforme" value="       No conforme" style="visibility:hidden"></td>
					...
					<td><input onClick="darveredicto('O','<%=Request.QueryString("menu")%>','<%=Request.QueryString("instancia")%>','<%=Request.QueryString("estado")%>','<%=Request.QueryString("informe")%>','<%=Request.QueryString("inf")%>')" name="cmdobservar" type="submit" disabled="disabled" class="editar1" id="cmdobservar" value="       Observar"></td>
			  </tr>
		</table>
	</td>
  </tr>
    ...
    <td height="34%" valign="top"><input name="hidden" type="HIDDEN" id="txtelegido">
	    <table width="100%" height="100%"  border="0" cellpadding="0" cellspacing="0">
		  <tr>
			...
			<form id="frmpropuesta" name="frmpropuesta" method="post" action="contenido.asp?estado=<%=estado%>&instancia=<%=instancia%>&menu=<%=menu%>">
<%	
