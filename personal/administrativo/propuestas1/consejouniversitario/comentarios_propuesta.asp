...
//	response.write(leido)
		...
		%>
					...
					CargarComentarios coment1,codigo_per
		...
		<iframe name="fraDetalleComentario" id="fradetallecomentario"  height="100%" width="100%" frameborder="0" scrolling="AUTO" src="detalle_comentario.asp?codigo_cop=<%=codigo_cop%>&codigo_prp=<%=codigo_prp%>&codigo_ipr=<%=codigo_ipr%>&coment1=<%=coment1%>&leido=<%=leido%>&codigo_per=<%=codigo_per%>">		</iframe></td>
      </tr>
    </table>
	</td>
  </tr>
...
Public Sub CargarComentarios(byval codigorespuesta_cop,COD)
dim objCom1  
dim RsComentarios
 //consulta mi recordset
 		Set objCom1=Server.CreateObject("PryUSAT.clsAccesoDatos")
		objCom1.AbrirConexiontrans
		set RsComentarios=objCom1.Consultar("ConsultarComentarios","FO","PR",codigorespuesta_cop,COD)
    ...
    <tr>
      <td colspan="2" align="left"><strong>Enviado el: </strong>
		...
		<TD>
				...
				<%//fin				
				call CargarComentarios(RsComentarios("codigo_cop"),COD)				
			else			
			//inicio
			...
			%>
			  <table cellspacing="1" width="100%" <%if   RsLeido.BOF=true then %> bgcolor="#FFFFD9" <% end if%> width="100%"  border="0" cellpadding="0" cellspacing="0" class="contornotabla" id="fila<%=RsComentarios(0)%>"  <%if trim(RsComentarios(0))=trim(codigo_cop) then %> bgcolor="#0099FF" <%end if%> onClick="LeerComentario(<%=RsComentarios(0)%>,<%=codigo_prp%>,<%=codigo_ipr%>,<%=coment1%>,<%=leido%>,<%=codigo_per%>)" style="cursor:hand"> 
    ...
    <tr>
	  ...
	  <strong>Asunto: </strong><%response.write(RsComentarios("asunto_cop"))%>
	  <%
	  ''Response.Write(RsComentarios(0))
