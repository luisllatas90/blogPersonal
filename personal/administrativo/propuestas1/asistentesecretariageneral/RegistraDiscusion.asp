...
<html>
<head>
<title>Registro de Discusiones</title>
...
var discusion=frminstitucion.txtacuerdo.value
	if (discusion==""){
...
//	alert (codigo_prp)
//	alert (codigo_rec)	
	location.href="procesar.asp?codigo_prp=" + codigo_prp + "&codigo_rec=" + codigo_rec + "&discusion=" + discusion  + "&accion=registraDiscusion"	
	}
}
function Actualizar(codigo_dpr,codigo_prp){
//	alert (codigo_apr)
	var texto=eval ("frminstitucion.TextArea_" + codigo_dpr + ".value")
//	alert (texto)
	location.href="registraDiscusion.asp?codigo_dpr="+codigo_dpr+"&texto="+texto+"&guardar=si&codigo_prp="+codigo_prp
	eval ("frminstitucion.guardar_" + codigo_dpr + ".style.visibility='hidden'")
}

function Eliminar(codigo_dpr,codigo_prp){
//	alert (codigo_apr)
//	alert (codigo_prp)	
	location.href="registradiscusion.asp?codigo_dpr="+codigo_dpr+"&eliminar=si&codigo_prp="+codigo_prp
}
...
if guardar="si" then
	codigo_dpr=Request.QueryString("codigo_dpr")
	texto=Request.QueryString("texto")	
    Set objProp=Server.CreateObject("PryUSAT.clsAccesoDatos")
   	objProp.AbrirConexion
	objProp.Ejecutar "ActualizarDiscusionPropuesta",false,codigo_dpr,texto
...
if eliminar="si" then
    ...
    Set objProp=Server.CreateObject("PryUSAT.clsAccesoDatos")
   	objProp.AbrirConexion
	objProp.Ejecutar "EliminarDiscusionPropuesta",false,codigo_dpr
   	objProp.CerrarConexion
	set objProP=nothing
end if
%>
<form action="procesar.asp?accion=" method="post" name="frminstitucion" id="frminstitucion">
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	%>
          <td valign="top">
		  <span class="Estilo5"><span class="Estilo7">Discusi&oacute;n</span> referente a la Propuesta:</span>
		  <font size="2"color="#990000"><strong> <%=propuesta("nombre_prp")%> 
		  </strong></font>
		  </td>
          </tr>
      </table></td>
    ...
    <tr>
      <td valign="top"><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
        ...
        <tr>
          <td align="center"><span class="Estilo4">Escriba la discuisi&oacute;n referente a la propuesta y haga clilc en &quot;Agregar&quot; </span></td>
        ...
        <tr bgcolor="#FFFFFF">
          <td colspan="3"><table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
            <tr>
              <td colspan="5"><span class="Estilo4">Discusi&oacute;n:</span>
                  <%	    Set objProp=Server.CreateObject("PryUSAT.clsAccesoDatos")
	     	objProp.AbrirConexiontrans
			set ACUERDOS=objProp.Consultar("ConsultarDiscusionPropuesta","FO","PR",codigo_prp)
	    	objProp.CerrarConexiontrans
			set objProP=nothing
	%>              </td>
            </tr>
		  ...
		  %>
			...
			<%FILA= LEN(ACUERDOS("descripcion_dpr"))/70+2%>
			  <textarea name="TextArea_<%=ACUERDOS("codigo_dpr")%>"  id="TextArea_<%=ACUERDOS("codigo_dpr")%>" rows="<%=FILA%>" class="Cajas2" readonly="readonly" style="border:none; overflow:hidden" ><%response.write(ACUERDOS("descripcion_dpr"))%></textarea>			  </td>
              <td width="3%" align="center" valign="top"><img src="../../../../images/menus/editar.gif" alt="Editar" width="18" height="13" border="0" style="cursor:hand" onClick="modificar('TextArea_<%=ACUERDOS("codigo_dpr")%>','guardar_<%=ACUERDOS("codigo_dpr")%>')"></td>
              <td width="3%" align="center" valign="top"> <img id="guardar_<%=ACUERDOS("codigo_dpr")%>" src="../../../../images/menus/guardar.gif" alt="Guardar" width="16" height="16" border="0" style="cursor:hand; visibility:hidden" onClick="Actualizar('<%=ACUERDOS("codigo_dpr")%>','<%=codigo_prp%>')" ></td>
              <td width="3%" align="center" valign="top">
			  <img src="../../../../images/menus/noconforme_small.gif" alt="Eliminar" width="16" height="16"  border="0" style="cursor:hand" onClick="Eliminar('<%=ACUERDOS("codigo_dpr")%>','<%=codigo_prp%>')">
