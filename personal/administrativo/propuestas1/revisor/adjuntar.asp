...
function SeleccionarFila1()
{
	...
	//}
		...
		function eliminar(codigo_prp,archivo,codigo_dap){
		    var codigo_cop = frmSubir.txtcodigo_cop.value
			var archivo = frmSubir.txtnombrearchivo.value
			if (frmSubir.txtnombrearchivo.value!="")	{
			//alert(codigo_cop)
			location.href="guardararchivo.asp?accion=eliminar&codigo_prp=" + codigo_prp + "&archivo=" + archivo + "&codigo_dap=" + codigo_dap + "&codigo_cop=" + codigo_cop				
			}
			else
			{
...
%>
		...
		<form id="frmSubir" action="guardararchivo.asp?codigo_prp=<%=codigo_prp%>&codigo_dap=<%=codigo_dap%>&codigo_cop=<%=codigo_cop%>" method="post" enctype="multipart/form-data">
                        ...
                        <td width="10%" align="right" valign="top"><table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">
                          <tr>
                            <td valign="top"><table width="100%" border="0" cellspacing="5" cellpadding="0">
                              ...
                              <tr>
                                <td align="right"><input type="button" name="cmdEnviar2" onClick="eliminar(<%=codigo_prp%>,this.value,<%=codigo_dap%>)" value="Quitar" class="remove_prp" style="width: 85" <%''if modifica=2 then%> <%''end if%>></td>
                              </tr>
