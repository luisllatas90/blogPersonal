...
if accion="enviar" then '' envía una propuesta a la instancia siguiente
	...
	Response.Redirect("contenido.asp?menu=" + menu + "&instancia=" + instancia + "&estado=" + estado + "&informe=" + informe + "&inf=" + inf + "&estado_prop=P")

end if


if accion="aconsejo" then '' envía una propuesta a la instancia siguiente
	veredicto=Request.QueryString("veredicto")
	codigo_prp=Request.QueryString("codigo_prp")
	menu=Request.QueryString("menu")
	instancia=Request.QueryString("instancia")
	...
	estado_prop=Request.Form("estado_prop")
	codigo_per=session("codigo_Usu")
	objPropuesta.Ejecutar "PRP_EnviarDeFacultadaConsejo",false,codigo_prp
