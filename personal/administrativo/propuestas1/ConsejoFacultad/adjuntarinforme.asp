...
function SeleccionarFila1()
{
	...
	//}
		...
		function eliminar(codigo_prp,archivo,codigo_cop){
			...
			//alert(codigo_cop)
			location.href="guardarinforme.asp?accion=eliminar&codigo_prp=" + codigo_prp + "&archivo=" + archivo			
			}
			else
			{
...
%>
		...
		<form id="frmSubir" action="guardarinforme.asp?codigo_prp=<%=codigo_prp%>" method="post" enctype="multipart/form-data">
...
//			  		do while not rsArchivosPrp.eof
			  if rsInformePrp("Descripcioninforme_prp") <> "" then%>
                            <TR height="25" onMouseOver="Resaltar(1,this,'S')" onMouseOut="Resaltar(0,this,'S')" class="Sel" typ="Sel" onClick="ponervalortext('<%=rsInformePrp("nombreinforme_prp")%>')" >
                              <td  width="5%" align="left" valign="middle">&nbsp;</td>
                              <td width="15%" align="center"><a href="../../../../filespropuestas/<%=codigo_prp%>/<%=rsInformePrp("nombreinforme_prp")%>" target="_blank"> <img src="../../../../images/ext/<%=right(rsInformePrp("nombreinforme_prp"),3)%>.gif" width="16" height="16"  border="0"> </a> </td>
                              <td width="80%" align="left"><%response.write(rsInformePrp("Descripcioninforme_prp"))%></td>
                            </TR>
              <%end if
                        ...
                        <td width="10%" align="right" valign="top"><table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">
                          <tr>
                            <td valign="top"><table width="100%" border="0" cellspacing="5" cellpadding="0">
                              <tr>
                                <td align="right"><input type="button" name="cmdEnviar" onClick="ValidarSilabos()" value=" Agregar" class="attach_prp" style="width: 85" <%if rsInformePrp("Descripcioninforme_prp")<>"" then%> disabled="disabled" <%end if%>></td>
                              </tr>
                              <tr>
                                <td align="right"><input type="button" name="cmdEnviar2" onClick="eliminar(<%=codigo_prp%>,this.value,<%=codigo_prp%>)" value="Quitar" class="remove_prp" style="width: 85" <%if modifica=2 then%> disabled="disabled" <%end if%>></td>
                              </tr>
