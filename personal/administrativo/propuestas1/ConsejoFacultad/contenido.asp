...
showModalDialog("../../../abrirfuncionVolver.asp?codigo_apl=7&descripcion_apl=SistemaPropuestas&estilo_apl=P",window,"dialogWidth:400px;dialogHeight:250px;status:no;help:no;center:yes")
	...
	//alert (pendiente)
		if (pendiente!='A'){
			...
			document.all.cmdEnviarConsejo.disabled=false			
		}
	}
	...
	function EnviarAConsejo(menu,instancia,estado,informe, inf){
	var codigo_prp=document.all.txtelegido.value
	

	calificar=confirm ("¿Desea Enviar la propuesta a Consejo Universitario?")
		if (calificar==true){
			frmpropuesta.action="procesar.asp?accion=aconsejo&codigo_prp="+codigo_prp.substring(4,codigo_prp.length)+"&menu="+menu+"&instancia="+instancia+"&estado="+estado+"&informe="+informe+"&inf="+inf
			frmpropuesta.submit()
		}
	}
  ...
  <tr>
    <td width="100%" height="6%">
		<table width="100%" height="100%" border="0" cellpadding="0" cellspacing="0">
			  <tr>
				<td align="center"><input style="visibility:hidden" name="Submit2" <%if Request.QueryString("inf") ="SI"then%>style="visibility:hidden" <%end if%> type="button" class="nuevo1"  value="       Nuevo"  onClick="AbrirPopUp('registrapropuesta.asp','580','750','false','yes',0,0,'yes')"></td>
				<td align="center"><input name="cmdmodificar" type="button" class="modificar_1" id="cmdmodificar" value="       Modificar" disabled="disabled" onClick="AbrirPopUp('registrapropuesta.asp?modificacion=1&codigo_prp=' + txtelegido.value.substring(4,txtelegido.value.length) + '&instancia_prp=<%=instancia%>','580','750')" style="visibility:hidden">
				
				<td align="center"><input name="cmdver" disabled="disabled" type="button" class="ver2" id="cmdver" onClick="AbrirPopUp('registrapropuesta.asp?modifica=2&codigo_prp=' + txtelegido.value.substring(4,txtelegido.value.length) + '&instancia_prp=<%=instancia%>','430','600')" value="         Ver" style="visibility:hidden"></td>
				<td align="center">&nbsp;</td>
				<td align="center" ><input name="Submit3" type="button" class="cambiouser" onClick="AbrirMenu()" value="       Cambiar Funci&oacute;n"></td>
					<td align="center"><% 
Set objDec=Server.CreateObject("PryUSAT.clsAccesoDatos")
	objDec.AbrirConexion	
	set RsDec =objDec.Consultar ("PRP_ConsultarEsDecano","FO", session("codigo_usu"))
	objDec.CerrarConexion
	%>
  <input name="cmdEnviarConsejo" type="button" disabled="disabled" class="enviarpropuesta" id="cmdEnviarConsejo" value="      Enviar a Consejo" <%if RsDec.RecordCount=0 then %> style="visibility:hidden" <%end if%> onClick="EnviarAConsejo('<%=Request.QueryString("menu")%>','<%=Request.QueryString("instancia")%>','<%=Request.QueryString("estado")%>','<%=Request.QueryString("informe")%>','<%=Request.QueryString("inf")%>')" >
                </td>
					<td align="center">&nbsp;</td>
					<td align="center"><input onClick="darveredicto('C','<%=Request.QueryString("menu")%>','<%=Request.QueryString("instancia")%>','<%=Request.QueryString("estado")%>','<%=Request.QueryString("informe")%>','<%=Request.QueryString("inf")%>')" name="cmdconforme" type="submit" disabled="disabled" class="conforme1" id="cmdconforme" value="       Conforme"></td>
					<td align="center"><input onClick="darveredicto('O','<%=Request.QueryString("menu")%>','<%=Request.QueryString("instancia")%>','<%=Request.QueryString("estado")%>','<%=Request.QueryString("informe")%>','<%=Request.QueryString("inf")%>')" name="cmdobservar" type="submit" disabled="disabled" class="editar1" id="cmdobservar" value="       Observar"></td>
...
<%	
		...
		'	else
			set propuesta=objCC.Consultar("ConsultarPropuestasRevision","FO","CF",instancia,estado,codigo_per,nombre_prp,informe,estado_prop)
		'	end if
'propuesta=objCC.Consultar("ConsultarPropuestasRevision","FO","PA",instancia,estado,codigo_per,nombre_prp,informe,estado_prop)
                      ...
                      <td colspan="6"><div id="listadiv" style="height:100%" class="NoImprimir">
                          <table table class="sortable" id="unique_id" width="100%" border="0" cellpadding="0" cellspacing="0" name="rsTable" id=rsTable  cols=5 >
							...
							<%if estado_prop<>"P" then%>		
			                    <tr id="fila<%=propuesta(0)%>" height="17"  onMouseOver="Resaltar(1,this,'S')" onMouseOut="Resaltar(0,this,'S')" class="Sel" Typ="Sel" onClick="<%if inf="SI" then%>ResaltarfilaDetalle('',this,'ConsultarAcuerdos.asp?codigo_prp=<%=propuesta(0)%>&instancia=<%=Request.QueryString("instancia")%>')<%else%>ResaltarfilaDetalle('',this,'ConsultarAcuerdos.asp?codigo_prp=<%=propuesta(0)%>&instancia=<%=Request.QueryString("instancia")%>')<%end if%>;ResaltarPestana_1('0','','');activamodifica('<%=instancia%>');ActivaBotones('<%=instancia%>','<%=estado%>','<%=informe%>')">
            				<%else%>               
							 ...
							 'else%>
							  ...
							  <%IF RsFechas.RecordCount=0 then
							  	Response.Write("-")
							   		...
							   		rESPONSE.Write(IIF(ISNULL(RsFechas(1)),"-",RsFechas(1)))
							   end if%> </font> </strong>
							 <%'end if%>
  ...
  <tr>
	...
	<table cellspacing="0" cellpadding="0" style="border-collapse: collapse" bordercolor="#111111" width="100%" height="100%">
      <tr>
		...
		<%if inf="SI" then%>
        	<td <%if estado_prop<>"P" then%> class="pestanabloqueada"<%else%>class="pestanaresaltada_1"  <%end if%> id="tab" align="center" width="20%" height="5%" <%if estado_prop<>"P" then%> onClick="ResaltarPestana_1('1','','');AbrirPropuesta('datos_informe.asp')" <%else%> onClick="ResaltarPestana_1('0','','');AbrirPropuesta('datos_informe.asp')" <%end if%> >Datos Generales</td>
		<%else%>
        	<td <%if estado_prop<>"P" then%> class="pestanabloqueada"<%else%>class="pestanaresaltada_1"   <%end if%> id="tab" align="center" width="20%" height="5%" <%if estado_prop<>"P" then%> onClick="ResaltarPestana_1('1','','');AbrirPropuesta('datos_propuesta.asp')" <%else%> onClick="ResaltarPestana_1('0','','');AbrirPropuesta('datos_propuesta.asp')" <%end if%>>Datos Generales</td>
		<%end if%>		

        <td width="1%" height="5%" class="bordeinf">&nbsp;</td>
        <td class="pestanabloqueada" id="tab" align="center" width="20%" height="5%" <%if estado_prop<>"P" then%> onClick="ResaltarPestana_1('2','','');AbrirPropuesta('revisores_propuesta.asp')"<%else%> onClick="ResaltarPestana_1('1','','');AbrirPropuesta('revisores_propuesta.asp')"<%end if%>>Revisores</td>
        <td width="1%" height="5%" class="bordeinf">&nbsp;</td>
		<td class="pestanabloqueada" id="tab" align="center" width="20%" height="5%" <%if estado_prop<>"P" then%> onClick="ResaltarPestana_1('3','','');AbrirPropuesta('comentarios_propuesta.asp')"<%else%> onClick="ResaltarPestana_1('2','','');AbrirPropuesta('comentarios_propuesta.asp')"<%end if%>>Observaciones</td>
