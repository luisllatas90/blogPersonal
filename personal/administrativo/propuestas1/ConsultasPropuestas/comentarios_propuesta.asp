...
<table width="100%" height="100%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    <td align="left" valign="top"><table width="100%" height="100%" border="0" cellpadding="0" cellspacing="3">
      ...
      <tr>
        ...
        <td align="center" valign="top">
		<iframe name="fraDetalleComentario" id="fradetallecomentario"  height="100%" width="100%" frameborder="0" scrolling="no" src="detalle_comentario.asp?codigo_prp=<%=codigo_prp%>">		</iframe></td>
      </tr>
    </table></td>
  </tr>
</table>
 ...
 //consulta mi recordset
		...
		<%
			if RsComentarios("HIJOS")>0 then
				...
				%>
				  <table cellspacing="1" width="100%" <%if   RsLeido.BOF=true then %> bgcolor="#FFFFD9" <% end if%> width="100%"  border="0" cellpadding="0" cellspacing="0" class="contornotabla" id="fila<%=RsComentarios(0)%>" onClick="document.frames['fradetallecomentario'].location='detalle_comentario.asp?codigo_cop=<%=RsComentarios(0)%>&codigo_prp=<%=codigo_prp%>&codigo_ipr=<%=codigo_ipr%>&coment1=<%=coment1%>&leido=<%=leido%>&codigo_per=<%=codigo_per%>' " style="cursor:hand"> 
    <tr>
    ...
    <tr>
      <td colspan="2" align="left"><strong>Enviado el: </strong>
        <%response.write(RsComentarios("fecha_cop"))%></td>
			...
			%>
			  <table  cellspacing="1"<%if   RsLeido.BOF=true then%> bgcolor="#FFFFD9" <% end if%> width="100%" style="cursor:hand" border="0" cellpadding="0" cellspacing="0" class="contornotabla" align="right" onClick="document.frames['fradetallecomentario'].location='detalle_comentario.asp?codigo_cop=<%=RsComentarios(0)%>&codigo_prp=<%=codigo_prp%>&codigo_ipr=<%=codigo_ipr%>&coment1=<%=coment1%>&leido=<%=leido%>&codigo_per=<%=codigo_per%>'" style="cursor:hand">
    <tr>
      <td   width="75%" align="left" valign="middle"><strong>De: </strong><%response.write(RsComentarios("nombre"))%></td>
      <td width="25%" align="center" valign="middle">	  
	  	  	  <% 
