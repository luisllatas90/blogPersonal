...
sub buscaralumno(byval pag,ByVal ruta)
...
<script language="Javascript">
	function enviarConsulta()
	{
		...
		if (cu.value==""){
			...
			location.href="<%=ruta%>clsbuscaralumno.asp?codigouniver_alu=" + cu.value + "&rutaactual=<%=ruta%>&pagina=<%=pag%>"
		}
	}
</script>
...
sub planalumnoescuela(byval evento,byval codigo_alu,byval varcodigo_pes,byval varseleccion,byval textolista)
...
sub escuelaprofesional(byval evento,byval varseleccion,byval textolista)
	...
	if trim(evento)<>"" then  evento="onChange=""" & evento & """ "
  	...
  	Set objEscuela=Server.CreateObject("PryUSAT.clsDatCarreraProfesional")
    Set rsEscuela= objEscuela.ConsultarCarreraProfesional("RS","PG","684")
	%>   
