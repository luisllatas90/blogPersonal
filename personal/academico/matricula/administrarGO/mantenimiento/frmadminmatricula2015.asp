...
if codigo_alu<>"" and trim(modo)="resultado" then
	...
	Set rsBloqueo=ObjBloqueo.Consultar("ConsultarAccesoMatriculaPorAsesorBloqueoAlu_2015","FO",codigo_alu,codigo_cac,"administrar/mantenimiento",session("codigo_tfu"))
	ObjBloqueo.CerrarConexion
	
	
	if(rsBloqueo("EstadoMatricula") <> "") then
	    set pagina=rsBloqueo("EstadoMatricula")		
	...
	Obj.AbrirConexion
	Set rsMatricula=Obj.Consultar("ConsultarAccesoMatriculaPorAsesor_2015","FO",codigo_alu,codigo_cac,0,"administrarGO/mantenimiento",session("codigo_tfu"))
	'Set rsCronograma=Obj.Consultar("ConsultarCronograma","FO", "ME", session("codigo_cac"))
	Set rsCronograma=Obj.Consultar("ACAD_ConsultarCronogramaGO","FO", "ME", codigo_cac)
...
<table border="0" cellpadding="2" cellspacing="0" style="border-collapse: collapse" bordercolor="#111111" width="100%" height="<%=alto%>">
    ...
    <tr>
      ...
      <td width="50%" style="height: 4%">
     	<%call buscaralumno("matricula/administrarGO/mantenimiento/frmadminmatricula2015.asp","../../../",-1)%>
      </td>
    </tr>
    <%else   %>
    <tr>
				...
				<tr bgcolor="#FFFFCC">
					...
					<%if pagina="" then
	                    if Not(rsCronograma.BOF AND rsCronograma.EOF) then
						    if session("Ultimamatricula")<>descripcion_cac then%>
						        <input onclick="modificarmatricula('N','<%=session("codigo_pes")%>')" type="button" value="     Nueva Matrícula <%=descripcion_cac%>" name="cmdAgregarMatricula" class="agregar2" style="width: 140px" />
					<%	    end if
					    else %>
