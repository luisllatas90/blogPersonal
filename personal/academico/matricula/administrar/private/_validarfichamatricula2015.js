...
arrMensajes[1] = "Ud. solo puede elegir un Grupo de Horario del Curso a Matricularse";
		...
		switch (eval(frmFicha.txtcodigo_cpf.value)) {
		    case 24:
		        ...
		        if (creditos < 13) // medicina
		            ...
		            document.all.lblPrecioCiclo.innerHTML = redondear(1600 * 5,2)
		    }		  
		    break    
		    case 31:
		        if (creditos < 13) // odontologia
		            ...
		            document.all.lblPrecioCiclo.innerHTML = 750 * 5
		        break
		    default:
		        document.all.lblPrecioCiclo.innerHTML = redondear( parseFloat(frmFicha.precioCredito.value) * CreditosPension * 5, 2)
		} 
...
{
	var pagina=""
	//alert('nuvea matricula modo2015: '+modo+" ID: "+ID);
	//return false;
	...
	if (modo=='R'){ //Para retiro de asinaturas
	    if (parseInt($("#cbocodigo_mar").val()) > 0) {
	        if (ValidarRetiroCursoInferior(ID) == 1) {
	            ...
	            alert("No puede retirar un alumno de ciclos inferiores");
	        }
	    } else {
...
function EnviarFichaMatricula() {
    ...
    cuotas = 5
    //cuotas = $("input:hidden[name=NroCuotas]").val()
...
function CalcularCuota(valor, precioCredito) {
   // frmFicha.lblCuota.innerHTML = redondear(parseFloat(precioCredito / valor), 2)
   $('#lblCuota').html(redondear(parseFloat(precioCredito / valor), 2));
