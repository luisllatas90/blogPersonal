...
if trim(codigo_pes)<>"-2" then   
    ...
    Set objSituacion=Server.CreateObject("PryUSAT.clsAccesoDatos")	
    objSituacion.AbrirConexion
    Set rsSituacion= objSituacion.Consultar("ConsultarSituacionAlumno","FO","ALU", codigo_alu,codigo_cac)	        
	...
	if(Right(descripcion_cac, 1) = "0") then
	    Set rsCursosProgramados= Obj.Consultar("ConsultarCursoProgramadoPorAsesorVerano","FO","1",codigo_alu,codigo_pes,codigo_cac,session("codigo_tfu"),session("CicloActual"))
	else	
	    if rsSituacion.RecordCount > 0 then	        
	        Set rsCursosProgramados= Obj.Consultar("ConsultarCursoProgramadoAlumnosSeparados","FO",codigo_alu,codigo_pes,codigo_cac,session("codigo_tfu"))	    
	    else	         
	...
	'Si tiene cursos desaprobados más de 2 veces
	hdDesaprobado = "N"
	if(rstotales.fields("CredDesaprobados2") > 0) then
	    hdDesaprobado = "S"
	end if
	...
	if Not(rsRequisitos.BOF and rsRequisitos.EOF) then
		...
		'Para Alumnos PA (llevan cursos por 3ra vez) 05.03.19 Yperez / Esaavedra
		'Excepto los PA que son por haber desaprob 3 cursos o más
		'if rsRequisitos("condicionciclo_anterior")="P" or rsRequisitos("condicionciclo_anterior")="C" then
		if rsRequisitos("condicion_PA15")="S" then
		    maxCreditos = 15
		    'maxCreditos = 11
		else
            if(CInt(codigo_tfu) = 9) then
...
<title>Elija los cursos que desea agregar a la <%  if Session("tipo_cac") = "E" then 
		            response.write("inscripci&oacute;n")
		        else
		            response.write("matr&iacute;cula")
		        end if
		    %> <%=descripcion_cac%></title>
...
<script type="text/javascript" language="JavaScript" src="private/validarfichamatricula_v12015.js?X=<% response.Write DateDiff("s", "12/31/1969 00:00:00", Now) %>"></script>
...
<input name="tfu" id="tfu" type="hidden" value="<%=session("codigo_tfu")%>"/>
...
<% if bloqueado <> "Si" then %>    
    ...
    <input type="hidden" value="<% response.write(rsRequisitos("CicloRefMat")) %>" name="lblCicloActual" id="lblCicloActual"/>
...
-->
  ...
  <tr style="height:5%">  
    ...
    <input type="hidden" id="txtMaxCred" name="txtMaxCred" value="<%=maxCreditos%>" />
  	Cantidad de créditos aprobados : <b><%=credaprobados%></b> | 
    Máximo de Créditos a seleccionar :  <b><%=maxCreditos%></b> |
    ...
    <br />
    Créditos Totales: <b><span id="creditosMat"><% response.Write(creditosMatriculados) 'session("credMat") %> </span></b> | 
	Créditos Seleccionados: <b><span id="totalcrd">0 </span></b> |
	Cursos seleccionados: <b><span id="totalcur">0 </span></b>	|
	Ciclo Referencia: <b><span id="cicloRefMat"> <% response.write(rsRequisitos("CicloRefMat")) %> </span></b> |
	<%  
		
	    if Session("tipo_Cac") = "E" then	
	       ' response.Write("AAA1: " & rsRequisitos("Nivelado"))
	        nivelacion = "No Nivelado"    	        
	        if CBool(rsRequisitos("Nivelado")) = true then
	            nivelacion = "Nivelado"
	        end if	        
	        
	        response.Write " Condicion de Inscripción: <b><span id='spanNivelacion'>" & nivelacion & "</span></b>"
	    end if	    
	    'if(Right(descripcion_cac, 1) = "0") then 
	    ...
	    '    end if        
	    'end if 	
	    response.Write "<span id='maxNivelacion' style='display:none;'>" & session("max_nivelacion") & "</span>"
	    response.Write "<span id='maxAdelantar'  style='display:none;'>" & session("max_adelantar") & "</span>"
	%>
	<!-- | Tipo matricula : <b><span id="idTipoMatricula">0 </span></b>	-->
	<input id="creditosmatriculados" name="creditosmatriculados" value="<% response.Write(creditosMatriculados) %>" type="hidden" />	
	<input id="hdDesaprobado" name="hdDesaprobado" value="<% response.Write(hdDesaprobado) %>" type="hidden" />	
	</td>
  </tr>
  <tr>
					...
					<tr valign="top" style="display:none" id="codigo_cur<%=rsCursosProgramados("codigo_cur")%>">
						...
						<%	
							...
							if activar=true then							
							    ...
							    if codigo_tfu = 9 or codigo_tfu = 1 then 'Agregado Epena : Solo puede saltar idiomas:  9	DIRECTOR DE ESCUELA	
							%>
							            ...
							            id='Checkbox1' vd='<%=rsCursosProgramados("NroVecesDes")%>' 
							            obl='<%=rsCursosProgramados("Obligatorio")%>' dei ='<%= DebeIdioma %>' dec = '<%= DebeComplementario %>' 
							            tipocomp = '<%=rsCursosProgramados("tipocomplementario_cur") %>' obligaComp = '0'/>
							
							<%							
							    else 'Agregado Epena
							        obligaComp = 0
							        if Session("tipo_cac") = "N" then
							            obligaComp = rsCursosProgramados("obligaComp")
							        end if
							...
							%>
							            ...
							            obl='<%=rsCursosProgramados("Obligatorio")%>' dei ='<%= DebeIdioma %>' dec = '<%= DebeComplementario %>' 
							            tipocomp = '<%=rsCursosProgramados("tipocomplementario_cur") %>' obligaComp = '<%=obligaComp %>'
							            />
							<%
							
							    end if  'Agregado Epena
		...
		<td colspan="4" bgcolor="#FFFFFF" style="border-color: #000000">
		    <div style="position:relative">
		        <div style="position:absolute; top:3px; left: 0px">
		            <% 
                        'response.write ("Cuota 1: 30/01/2020" & "</br>")
	                    'response.write ("Cuota 2: 26/02/2020" & "</br>")	                    
                    %>
		        </div>
		        ...
		        <div style="position:absolute; left:70%">
                    <input type="button" value="       Cancelar" name="cmdCancelar1" 
                        ...
                        class="noconforme1"/>
                    <input type="button" value="     Guardar <%  if Session("tipo_cac") = "E" then 
		            response.write("inscripci&oacute;n")
		        else
		            response.write("matr&iacute;cula")
		        end if
		    %>" disabled="false" name="cmdAgregar" id="cmdAgregar" class="conforme1" onclick="EnviarFichaMatricula(<%=codigo_alu%>,<%=codigo_pes%>,<%=codigo_cac%>)"/>
                </div>  
		    </div>		    
		</td>				
	</tr>
