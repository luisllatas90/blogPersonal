	...
	if accion="matriculasegura" then
		...
		%>
			<script type="text/jscript" language="javascript">
				...
				window.location.href = 'frmagregarcurso2015.asp?accion=<%=accion%>&codigo_pes=<%=codigo_pes%>&codigo_cac=<%=codigo_cac%>&esnuevamatricula=<%=Request.form("txtesnuevamatricula")%>&cruce=1';
...
'*************************************************************************************
	if accion="agregarcursomatricula" then	    
        ...
        if(Verificacodigo_alu > -1) then
	        ...
	        %>
		        <script type="text/jscript" language="javascript">
		            window.open("frmcruces.asp?codigo_alu=<%=codigo_alu%>&codigo_pes=<%=codigo_pes%>&codigo_cac=<%=codigo_cac%>&cursosprogramados=<%=cursosprogramados%>", "", "status:no;resizable:yes;dialogWidth:60;dialogHeight:20");
		            window.location.href = 'frmagregarcurso2015.asp?accion=<%=accion%>&codigo_pes=<%=codigo_pes%>&codigo_cac=<%=codigo_cac%>&esnuevamatricula=<%=Request.form("txtesnuevamatricula")%>&cruce=1';
			        //window.history.back(-1);
		        </script>
	        <%else
...
'*************************************************************************************
	...
	if accion="retirarcursomatricula" then
		...
		Set objmatricula= Server.CreateObject("PryUSAT.clsAccesoDatos")
		objMatricula.AbrirConexion
			mensaje=objMatricula.Ejecutar("RetirarCursoMatriculaAdmin",true,"A",codigo_dma,usuario,codigo_mar,obs,null)			
		objmatricula.CerrarConexion
		Set objmatricula=nothing
		tituloboton="Regresar"
