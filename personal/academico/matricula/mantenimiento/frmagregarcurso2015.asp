...
if trim(codigo_pes)<>"-2" then
	...
	Set rsCursosProgramados= Obj.Consultar("ConsultarCursoProgramadoPorAdmin","FO","ADM",codigo_alu,codigo_pes,codigo_cac,session("codigo_tfu"))	
	...
	if Not(rsRequisitos.BOF and rsRequisitos.EOF) then
	    ...
	    CicloRefMat = rsRequisitos.fields("CicloRefMat")
	    Nivelado = rsRequisitos.fields("Nivelado")
	end if
end if
%>
<html>
...
<table id="tblmensaje" border="0" cellpadding="0" cellspacing="0" style="border-collapse: collapse;display:none" bordercolor="#111111" width="100%" height="100%" class="contornotabla">
	...
	<img border="0" src="../../../../images/cargando.gif" width="209" height="20" alt="Cargando"/>
	</td>
	</tr>
</table>
...
<table cellpadding="3" cellspacing="0" style="border-collapse: collapse; border: 0px solid #C0C0C0; " bordercolor="#111111" width="100%" <%=alto%>>
	...
	<tr height="5%">
		<td colspan="3" width="100%" align="right" >
            ...
            'end if
           %>           
           <input type="hidden" id="hdMaxCredPermitidos" name="hdMaxCredPermitidos" value="<% response.write(maxCreditos) %>" />
           ...
           <br /> 
            ...
            <% if Cint(codigo_tfu) = 9 or Cint(codigo_tfu) = 85 or Cint(codigo_tfu) = 181 then %>
    ...
    <td width="100%" colspan="3" height="90%" valign="top">
	    <table border="1" cellpadding="2" cellspacing="0" style="border-collapse: collapse" bordercolor="#808080" width="100%" height="100%" bgcolor="white">
			...
			<tr bgcolor="#F0F0F0" align="right">
				...
				Nº Cursos seleccionados :<b><span id="totalcur">&nbsp;</span></b><br />
				Total de Cursos Programados :<b><%=k%></b> | Tipo matricula : <b><span id="idTipoMatricula"></span></b>	| 
				Ciclo Referencia: | <% response.write(CicloRefMat) %>
				Condición Inscripción: | <% response.write(Nivelado) %>
				<span style="color:Red" id="AvisoBloqueo"></span>	
		    	</td>
		    </tr>
		    <tr>
