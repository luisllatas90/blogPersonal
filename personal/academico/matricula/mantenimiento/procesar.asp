...
codigo_alu=request.querystring("codigo_alu")
	...
	if accion="actualizartrasladoalumno" then
			...
			set obj =Server.CreateObject("PryUSAT.clsAccesoDatos")
				obj.AbrirConexion
				rsCiclo = obj.consultar("ConsultarCicloAcademico", "FO", "CV", 1)
				call Obj.Ejecutar("ActualizarPlanEscuelaAlumno",false,"TI",cicloIng_Alu,codigo_alu,codigo_pes,session("usuario_bit"))
				'response.write cicloIng_Alu & "," & codigo_alu & "," & codigo_pes & "," & session("usuario_bit")
				call obj.Ejecutar("MAT_ActualizarPrerequisitos", false, codigo_alu, codigo_pes, rsCiclo("codigo_cac"))
				obj.CerrarConexion
			Set obj=nothing
			
			call limpiarsesionalumno
			response.redirect("../../clsbuscaralumno.asp?codigouniver_alu=" & codigouniver_alu & "&pagina=matricula/mantenimiento/frmtraslados.asp")
	end if
	...
	if accion="anularmatricula" then
		...
		Set obj=Server.CreateObject("PryUSAT.clsAccesoDatos")
			obj.AbrirConexion
			mensaje=Obj.Ejecutar("AnularMatricula",true,tipo, cint(codigo_cac),codigo_alu,session("Usuario_bit"),obs,null)
			obj.CerrarConexion
		Set obj=nothing
	    ...
	    'if not mensaje<>"" then
		'	response.write "a = " & mensaje
	    'else	

				
		if Err.number <> 0	then
			'response.Write("Error: " & Err.Description  & "<br/> Source: " & Err.Source  & " mensaje=" & mensaje) 
			response.write "<script>alert('No se puede procesar el retiro');location.href='frmanularmatricula.asp'</script>"
		end if		
...
'if(Err.number <> 0) then
 '   response.Write("Error: " & Err.Description  & "<br/> Source: " & Err.Source & "<br/> Numero: " & Err.Number )       
  '  response.Write("tipo="& tipo & " cac =" & codigo_cac  & " alu = " & codigo_alu & " usuario_bit = " &  session("Usuario_bit")  & " obs = " & obs & " mensaje = " & mensaje )
'end if
