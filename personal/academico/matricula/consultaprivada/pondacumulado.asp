...
if cicloIng_Alu="" then cicloIng_Alu="-2"
	...
	'	Set rsEscuela=Obj.ConsultarCarreraProfesional("RS","RE","",0)
	'Set Obj=nothing
...
<p class="usattitulo">Ponderado acumulado por Escuela Profesional y ciclos de 
ingreso</p>
<table border="0" <%=alto%> cellpadding="3" cellspacing="0" style="border-collapse: collapse" bordercolor="#111111" width="100%">
  <tr>
    <td width="25%" height="5%" colspan="2" style="width: 90%" class="usatsugerencia">&nbsp;&nbsp;&nbsp;&nbsp; 
	Seleccione la Escuela Profesional y el ciclo de ingreso del estudiante y 
	luego especifique hasta que ciclo académico desea acumular el promedio 
	ponderado</td>
  </tr>
  <tr>
    <td width="25%" height="5%">Escuela Profesional</td>
    <td width="65%" height="5%"><%call llenarlista("cboescuela","actualizarlista('pondacumulado.asp?codigo_cpf=' + this.value)",rsEscuela,"codigo_cpf","nombre_cpf",codigo_cpf,">>Seleccionar Escuela Profesional<<","","")%></td>
  </tr>
  <%if codigo_cpf<>"-2" then%>
  <tr>
    <td width="25%" height="5%">Ciclo Ingreso</td>
  ...
  <tr>
   <td colspan="2" width="100%" height="80%" valign="top">
	...
	Set Obj= Server.CreateObject("PryUSAT.clsDatMatricula")
		Set rsAlumnos=Obj.ConsultarMatricula("RS","8",codigo_cpf,cicloIng_Alu,codigo_Cac)
	Set Obj=nothing
  ...
  <tr>
  <td colspan="2" width="100%" class="usatsugerencia">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Haga clic en el 
	estudiante para visualizar su historial de notas</td></tr>
  <%end if
  end if%>
</table>
