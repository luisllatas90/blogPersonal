...
'*************************************************************************************
	if accion="matriculasegura" then
		...
		<%else
			...
			mensaje = objMatricula.Ejecutar("AgregarMatriculaWeb_v1_2015",True,"A",codigo_alu,codigo_pes,codigo_cac,"M","Matricula por Asesor",CursosProgramados,VecesDesprobados,"N","M","MAT",usuario,IP,1,"",cuotas,tipomotivo, descripcionmotivo, null)			
	...
	if accion="agregarcursomatricula" then
		...
		'*************************************************************************************
			...
			mensaje=objMatricula.Ejecutar("AgregarMatriculaWeb_v1_2015",True,"A",codigo_alu,codigo_pes,codigo_cac,"N","Matricula por Asesor",CursosProgramados,VecesDesprobados,"N","M","AGR",usuario,IP,1, null, cuotas, tipomotivo, descripcionmotivo,null)
...
'*************************************************************************************
	...
	if accion="retirarcursomatricula" then	    
		...
		Set objmatricula= Server.CreateObject("PryUSAT.clsAccesoDatos")
		objMatricula.AbrirConexion		    
			mensaje=objMatricula.Ejecutar("RetirarCursoMatriculaComp",true,"A",codigo_dma,usuario,codigo_mar,"",null)
		objmatricula.CerrarConexion
		Set objmatricula=nothing
		'tituloboton="Regresar"
	...
	<tr height="5%">
		...
		<% 
            ...
            if(tipoMotivo = "C" or tipoMotivo = "P") then
                ...
                'rutacarta = "location.href='frmadminmatricula2015.asp'"
                'rutacarta = mensaje(0)      
            else
                'rutacarta = mensaje(0)      
            end if	    
		%>
		<input style="width:170px" class="conforme1" name="cmdRegresar" type="button" value="<%=tituloboton%>" onclick="<%=Replace(rutacarta,"administrar","administrarcomplementario")%>" />
		</td>
	</tr>
</table>
<%end if%>
