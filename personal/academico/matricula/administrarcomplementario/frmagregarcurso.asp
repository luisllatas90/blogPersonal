...
if trim(codigo_pes)<>"-2" then
	...
	obj.AbrirConexion		
	Set rsCursosProgramados= Obj.Consultar("ConsultarCursoProgramadoPorAsesor","FO",1,codigo_alu,codigo_pes,codigo_cac,11)
	...
	if Not(rsRequisitos.BOF and rsRequisitos.EOF) then
	    ...
	    if(tipo_motivo = "C") then
	        msj = "<td width='18%' style='background-color:Yellow; color:red; border: 1px solid black; padding:5px; font-weight:bold;'>Matr&iacute;cula Condicional</td>"
	    end if
	    
	    if(tipo_motivo = "P") then
	        msj = "<td width='18%' style='background-color:Yellow; color:red; border: 1px solid black; padding:5px; font-weight:bold;'>Problemas acad&eacute;micos</td>"
	    end if
	    
	    if(tipo_motivo = "O") then
	        msj = "<td width='18%' style='background-color:Yellow; color:red; border: 1px solid black; padding:5px; font-weight:bold;'>En observaci&oacute;n</td>"
	    end if
	end if
...
<script type="text/javascript" language="JavaScript" src="private/validarfichamatricula_v1.js?v=<% response.Write DateDiff("s", "12/31/1969 00:00:00", Now) %>"></script>
...
<form name="frmFicha" id="frmFicha" method="post" action="procesarmatricula.asp?accion=<%=accion%>&codigo_mat=<%=rsRequisitos.fields("codigo_mat")%>">
...
-->
  ...
  <tr>
    <td style="width:10%">Motivo de Agregado: 
	    <%  Set obj=Server.CreateObject("PryUSAT.clsAccesoDatos")
	        Obj.AbrirConexion
  ...
  <!--Cambio: form Informacion Adicional, se cambio de lugar -->
 	...
 	<td bgcolor="#E1F1FB" bordercolor="#000000"><b><u>Información adicional</u></b></td>
	<td colspan="3" width="80%" bgcolor="#E1F1FB" style="border-color: #000000">&nbsp;</td>        
	<tr>
		<td bgcolor="#FFFFFF" style="border-color: #000000">Observacion : (Max 500 caracteres)</td>
		<td colspan="3" width="100%" bgcolor="#FFFFFF" style="border-color:#000000">
        <input id="txtMotivoAnterior"  type="text" disabled="disabled" value="<%=motivo%>"
                style="width: 100%; background-color: #F0F0F0;" />
         <textarea class="cajas2" rows="4" id="txtMotivo" name="txtMotivo" cols="20" > </textarea>
         </td>	
	</tr>
    <!--fin cambio -->
  ...
  <tr>
    <td width="100%" colspan="2" height="3%" valign="top">
        * Si es ingresante de la Escuela de Medicina el precio del ciclo académico es: S/. 
        8000
    </td>
...
<%  if(session("tipo_Cac") <> "E") then %>
    ...
    %>
    <input type="hidden" name="NroCuotas" id="Hidden1" value="5"/>	
<%  else %>
