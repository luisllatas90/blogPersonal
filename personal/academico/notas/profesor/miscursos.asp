...
Set obj=Server.CreateObject("PryUSAT.clsAccesoDatos")
	obj.AbrirConexion
		Set rsCiclo=obj.Consultar("ConsultarCicloAcademico","FO","TN",codigo_per) ' Tipo TO >> TN
		Set rsCarga=Obj.Consultar("NOT_ConsultarRegistroNotas","FO",3,codigo_cac,codigo_per)
...
<%
  	...
  	<%else
  	    'if (codigo_cac<>"" and codigo_per<>"-2" and cint(codigo_cac)<=cint(session("codigo_cac"))) then 'Comentado x yperez 30.01.2018 - No pueden poner notas a cursos del ciclo superior al actual, por ejemplo Prof que ya no tiene ciclo de verano.
	  ...
	  <tr onMouseOver="Resaltar(1,this,'S')" onMouseOut="Resaltar(0,this,'S')">
	   <td width="5%" align="center"><%=i%></td>
		...
		<%if rsCarga("codigo_aut")=0 and rsCarga("estadonota_cup")<>"P" then%>
			<font color="darkblue">Realizado</font>
		<%elseif rsCarga("codigo_aut")>0 and rsCarga("estadonota_cup")<>"P" then%>
			<font color="darkorange">Pendiente con autorización</font>
        <%elseif rsCarga("matriculados")=0 then%>
        	<font color="darkblue">-</font>		
		<%else%>
			<font color="darkorange">Pendiente</font>
		<%end if%>
	    </td>
