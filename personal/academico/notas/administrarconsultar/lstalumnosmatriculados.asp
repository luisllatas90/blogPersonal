...
set objRec=Server.createObject("PryUSAT.clsAccesoDatos")
	objRec.AbrirConexion
		...
		if not (rsRec.BOF and rsRec.EOF) then				    		    
		    Set rsFec = objRec.Consultar("ACAD_VerificaPermisosExamen","FO", codigo_cac, codigo_cup)
...
Set objRec=nothing
    ...
    if Not(rsRegistro.BOF and rsRegistro.EOF) then
	    ...
	    notamax=20
	    if Left(rsRegistro("grupohor_cup"),4) = "REC-" then
	        notamax=14
	    end if
	    'response.Write "notamax: " & notamax
    end if
...
%>
    ...
    <script type="text/javascript" language="JavaScript" src="../private/validarnotas.js?x=2"></script>
    <style type="text/css">
        ...
        <td width="100%" colspan="2" valign="top" align= "center" height="90%">   
		    <table height="100%" width="99%" border="0" cellpadding="0" cellspacing="0" style="border-collapse: collapse;" class="contornotabla">
			    ...
			    %>
				    ...
				    <td align="left" width="45%"><%=rsAlumnos("alumno")%>
				        <%=rsAlumnos("nombre_cpf")%>
				        <% if rsAlumnos("inhabilitado_dma")= true then 
				            response.Write("<span style='color:Orange;'> <b>Inhabilitado</b><span>")
				           end if
				        %>
				        &nbsp;
				   </td>
				    <td align="center"  width="8%">
				    <%if rsAlumnos("estado_dma")="M" and sw = 0 and rsAlumnos("inhabilitado_dma")= false then%>
					    <input name="txtnotafinal_dma<%=i%>" type="textbox" onfocus="this.select()" onkeypress="validarnumero()" onblur="if(this.value==''){this.value=0}" value="<%=rsAlumnos("notafinal_dma")%>" onkeyup="validarnota(this,'<%=notaminima_cac%>',txtcondicion_dma<%=i%>,'<%=notamax%>')" size="2" maxlength="2" <%=estadoControl%> />
				    <%end if%>
				    </td>
