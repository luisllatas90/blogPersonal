...
usuario=session("Usuario_bit")
	...
	if accion="agregarcursoprogramado" then
		...
		Set objProgramar=Server.CreateObject("PryUSAT.clsAccesoDatos")
			...
			for i=0 to ubound(arrCodigo_cur)
				...
				mensaje=objProgramar.Ejecutar("AgregarCursoProgramado",true,numGrupos,date,codigo_cac,codigo_pes,codigo_cur,"F",vacantes_cup,"C",fechainicio_cup,fechafin_cup,fecharetiro_cup,arrcodigo_ceq,session("codigo_usu"),null)
			next
	...
	if accion="modificarcursoprogramado" then
		...
		Set objProgramar=Server.CreateObject("PryUSAT.clsAccesoDatos")
			...
			mensaje=objProgramar.Ejecutar("ModificarCursoProgramado",true,codigo_cup,grupohor_cup,estado_cup,vacantes_cup,fechainicio_cup,fechafin_cup,fecharetiro_cup,session("codigo_usu"),null)
	...
	if accion="agruparcursos" then
		...
		Set ObjCarga= Server.CreateObject("PryUSAT.clsAccesoDatos")
			...
			response.redirect "frmagruparcursos.asp?codigo_cpf=" & codigo_cpf & "&codigo_cac=" & codigo_cac
	end if
