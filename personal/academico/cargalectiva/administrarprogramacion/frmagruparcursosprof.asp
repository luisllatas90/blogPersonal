...
obj.AbrirConexion
	...
	if codigo_tfu=1 OR codigo_tfu=7 OR codigo_tfu=16 OR codigo_tfu=85 OR codigo_tfu=181 then
	    if(request.QueryString("mod") = 10) then
	        ...
	        Set rsEscuela=obj.Consultar("ACAD_BuscaEscuelaProfesional","FO",0,10,"")
	    else	    
	        Set rsEscuela=obj.Consultar("ConsultarCarreraProfesional","FO","PZ",0)
	    end if	    
	else	    
	...
	if codigo_cpf<>"" and codigo_cpf<>"-2" and Not(rsEscuela.BOF and rsEscuela.EOF) then	    
		...
		Set rsCursos=Obj.Consultar("ACAD_ConsultarCursoProgramado","FO",codigo_cac,codigo_cpf,"%",request.QueryString("mod"))
...
var Contador=0
	...
	function AgruparCursos()
	{
		...
		if (confirm("¿Está completamente seguro que desea Agrupar los cursos seleccionados?")==true){		
			...
			//alert('Padre:' + Padre + '\nHijos:' + Hijos)
			location.href = "procesarprof.asp?accion=agruparcursos&codigo_cac=" + cbocodigo_cac.value + "&mod=" + test.value + "&codigo_cpf=" + cbocodigo_cpf.value + "&Padre=" + Padre + "&Hijos=" + Hijos 
			//alert("procesar.asp?accion=agruparcursos&codigo_cac=" + cbocodigo_cac.value + "&codigo_cpf=" + cbocodigo_cpf.value + "&Padre=" + Padre + "&Hijos=" + Hijos)
		}	
	}
</script>
</head>
...
<input type="hidden" id="test" value="<%=request.QueryString("mod")%>" />
<table border="0" cellpadding="2" cellspacing="0" style="border-collapse: collapse" bordercolor="#111111" width="100%" <%=alto%>>
	<tr height="5%" valign="top">
		...
		<td width="10%"><%call llenarlista("cbocodigo_cac","actualizarlista('frmagruparcursosprof.asp?mod=" & request.QueryString("mod") & "&codigo_cac='+ this.value + '&codigo_cpf=' + cbocodigo_cpf.value)",rsCiclo,"codigo_cac","descripcion_cac",codigo_cac,"","","")%></td>
		<td width="20%" align="right">Escuela Profesional</td>
		<td width="40%"><%call llenarlista("cbocodigo_cpf","actualizarlista('frmagruparcursosprof.asp?mod=" & request.QueryString("mod") & "&codigo_cac='+ cbocodigo_cac.value + '&codigo_cpf=' + cbocodigo_cpf.value)",rsEscuela,"codigo_cpf","nombre_cpf",codigo_cpf,"Seleccione la Escuela Profesional","","")%></td>
	</tr>
	<%if HayReg=true then%>
