...
Set Obj= Server.CreateObject("PryUSAT.clsAccesoDatos")
	obj.AbrirConexion
		Set rsHorario=obj.Consultar("ConsultarHorariosAmbiente","FO",19,codigo_cac,codigo_amb,0,0)
	obj.CerrarConexion
Set Obj=nothing
...
if Not(rsHorario.BOF and rsHorario.EOF) then
  if ((cint(codigo_cac) <= cint(session("codigo_cac"))) or (session("codigo_tfu")="1" or session("codigo_tfu")="15" or session("codigo_tfu")="41"  or session("codigo_tfu")="18" or session("codigo_tfu")="25" or session("codigo_tfu")="72" or session("codigo_tfu")="9" or session("codigo_tfu")="16" or session("codigo_tfu")="85" or session("codigo_tfu")="181")  ) then
	dim dia,hora
				...
				if Not(rsHorario.BOF and rsHorario.EOF) then
					...
					Do while not rsHorario.EOF
						...
						if trim(dia)=trim(diaBD) AND int(hora)>=int(inicioBD) AND int(hora)<int(finBD) then
							marcas=marcas+1
							TextoCelda=TextoCelda & rsHorario("nombre_cur") & "<br>" & rsHorario("docente")  & "<br>" & rsHorario("tipoHoraCur_Lho") & "<br><br>"
