...
<!-- <input class="excel" onClick="location.href='<%=pagina%>'" type="button" value="     Exportar..." id="exportar" />    -->
<%
...
Set rsAmbiente=obj.Consultar("ACAD_ConsultarAmbiente","FO",codigo_cac,codigo_cpf,ciclo_cur,0,codigo_tam,codigo_ube)
...
if Not(rsAmbiente.BOF and rsAmbiente.EOF) then
	...
	Do while not rsAmbiente.EOF
		...
		if rsAmbiente("codigo_amb")<>32 then
				...
				Set rsHorario=obj.Consultar("ACAD_ConsultarHorariosAmbiente","FO",codigo_cac,rsAmbiente("codigo_amb"),codigo_cpf,ciclo_cur,codigo_tam,codigo_ube)
					...
					if c=0 then
						...
						if HayReg=true then						    
							...
							Do while not rsHorario.EOF
							    cantidad = rsAmbiente("capacidad_Amb")
							    matriculados = rsAmbiente("Matriculados")
							    ...
							    if trim(dia)=trim(diaBD) AND int(hora)>=int(inicioBD) AND int(hora)<int(finBD) then
								    ...
								    'TextoCelda=TextoCelda & "class='" & estilo & "'"                                                                    								    								    
								    sw = CINT(rsHorario("Row"))
								    if(CINT(rsHorario("Row")) mod 2 = 1) then
								        TextoCelda=TextoCelda & "class='CU1'"
								    else
								        TextoCelda=TextoCelda & "class='CU'"
								    end if
							
							        while(LEN(matriculados)<2)
						                matriculados = "0" & matriculados
						            wend	    
								    '******************************************************
						...
						TextoCelda=TextoCelda & " tooltip='" &  diaBD & "|" & inicioBD & "|" &  finBD & "'><u>" & matriculados & "</u> " & cantidad & " </td>" & vbcrlf
