...
if (codigo_sem<>"" or codigo_amb<>"") then

Set Obj= Server.CreateObject("PryUSAT.clsAccesoDatos")
	obj.AbrirConexion
		Set rsHorario=obj.Consultar("HorarioPE_ConsultarHorarioxSemana","FO",codigo_amb,codigo_sem,codigo_anno)
	obj.CerrarConexion
Set Obj=nothing
...
if Not(rsHorario.BOF and rsHorario.EOF) then
 ' if ((cint(codigo_cac) <= cint(session("codigo_cac"))) or (session("codigo_tfu")="1" or session("codigo_tfu")="15" or session("codigo_tfu")="41"  or session("codigo_tfu")="18" or session("codigo_tfu")="25" or session("codigo_tfu")="72" or session("codigo_tfu")="9" or session("codigo_tfu")="16"))   then
	...
	response.write vbtab & "<th width='15%'>Domingo</th>" & vbcrlf
	...
	for f=1 to 16
		...
		for c=0 to 7
			if c=0 then		
				...
				'============================================================================================================================
				response.write vbtab & "<td width='15%' class='etiquetaTabla'>" & f+6 & ":00 - " & f+7 & ":00</td>"
				'============================================================================================================================
			else
				...
				TextoCelda=vbtab & "<td style=""background-color:$color$;"" width='15%' id='" & dia & "' hora='" & hora & "'>" & vbcrlf
				...
				if Not(rsHorario.BOF and rsHorario.EOF) then
					...
					Do while not rsHorario.EOF
						...
						if trim(dia)=trim(diaBD) AND int(hora)>=int(inicioBD) AND int(hora)<int(finBD) then
							...
							TextoCelda= Replace(TextoCelda,"$color$", rsHorario("color"))
						end if
						rsHorario.movenext
					Loop
				end if
