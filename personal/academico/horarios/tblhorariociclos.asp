...
if (codigo_pes<>"" or codigo_cac<>"") then

Set Obj= Server.CreateObject("PryUSAT.clsAccesoDatos")
	obj.AbrirConexion
		Set rsHorario=obj.Consultar("ConsultarHorariosAmbiente","FO",modo,codigo_cac,codigo_pes,ciclo_cur,0)
	obj.CerrarConexion
Set Obj=nothing
...
if Not(rsHorario.BOF and rsHorario.EOF) then
  if ((cint(codigo_cac) <= cint(session("codigo_cac"))) or (session("codigo_tfu")="1" or session("codigo_tfu")="15" _
        or session("codigo_tfu")="41" or session("codigo_tfu")="18" or session("codigo_tfu")="9" _
        or session("codigo_tfu")="116") or session("codigo_tfu")="23") then
	titulopag="<p class='usattitulo'>"
		if modo=6 then titulopag=titulopag & descripcion_pes
		if ciclo_cur>0 then titulopag=titulopag & " - " & ConvRomano(ciclo_cur) & " CICLO"
	titulopag=titulopag & "</p>"
	...
	for f=1 to 16
		...
		for c=0 to 6
			if c=0 then		
				'response.write vbtab & "<td width='15%' class='etiquetaTabla'>" & f+6 & ":10 - " & f+1+6 & ":00</td>"  'linea con los 10 min
				
				'=================================================================================================================
				response.write vbtab & "<td width='15%' class='etiquetaTabla'>" & f+6 & ":00 - " & f+1+6 & ":00</td>"
				'=================================================================================================================
			else
					...
					Do while not rsHorario.EOF
						...
						if trim(dia)=trim(diaBD) AND int(hora)>=int(inicioBD) AND int(hora)<int(finBD) then
							...
							if rsHorario("estadoHorario_lho") = "A" then
							    TextoCelda=TextoCelda & rsHorario("nombre_cur") & "«<font color='blue'>" & rsHorario("ambiente") & "</font>»<br>" & rsHorario("docente") & "<br><br>"
							else
							    TextoCelda=TextoCelda & rsHorario("nombre_cur") & "«<font color='red'>" & rsHorario("ambiente") & "</font>»<br>" & rsHorario("docente") & "<br><br>"
							end if 
						end if
						rsHorario.movenext
					Loop
