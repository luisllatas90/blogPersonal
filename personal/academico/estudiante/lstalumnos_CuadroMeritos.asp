...
if (resultado="S" and trim(param)<>"") then
	...
	pagina="misdatos_CuadroMeritos.asp?tipo=" & tipoResp
...
<table width="100%" <%=alto%> border="0" cellspacing="0" cellpadding="3" style="border-collapse: collapse" bordercolor="#111111">
    <tr> 
      <td height="5%" colspan="4" valign="top" width="100%"><span class="usatTitulo">Búsqueda de estudiantes de Pregrado</span>
	</td>
    </tr>
    <tr> 
      ...
      <input name="txtParam" type="text" id="txtParam" size="30" class="cajas2" onkeyup="if(event.keyCode==13){BuscarAlumno_CuadroMeritos('<%=modulo%>')}"></td>
      <td valign="top" width="30%" height="5%">
      <input name="cmdBuscar" type="button" id="cmdBuscar" value="Buscar" class="usatbuscar" onClick="BuscarAlumno_CuadroMeritos('<%=modulo%>')">
	 </td>
    </tr>
    <%if resultado="S" then%>
    <tr id="trLista">
	...
	<tr id="trDetalle" >
  		<td width="100%" height="65%" colspan="4">
			<table cellspacing="0" cellpadding="0" style="border-collapse: collapse" bordercolor="#111111" width="100%" height="100%">
				<tr height="8%">
					<td class="pestanaresaltada" id="tab" align="center" width="18%" onclick="ResaltarPestana2('0','','');AbrirFicha('misdatos_cuadroMeritos.asp',this)">
		   ...
		   <img border="0" src="../../../images/imprimir.gif" style="cursor:hand" ALT="Imprimir Ficha" onClick="ImprimirFicha()">
		    <%if session("codigo_tfu")=1 or session("codigo_tfu")=16 or session("codigo_tfu")=33 or session("codigo_tfu")=7 or session("codigo_tfu")=2815  then%>
