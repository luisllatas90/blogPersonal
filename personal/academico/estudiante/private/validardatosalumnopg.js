...
function EnviarFicha(frm,navegar)
{
   ...
   if (modo=="P"){
	...
	if (!validarAnio(frm.anio,'Por favor ingrese correctamente el Año de Nacimiento',15)){
		return(false)
	}

	if (!validarDocIdentidad(frm.tipodocident_alu,frm.nrodocident_alu)){
		return(false)
	}
...
/*
	if (!validarCorreo(frm.email2_alu)){
		return(false)
	}
*/
	if (frm.email_alu.value.length>5){
		if (!validarCorreo(frm.email_alu,'Por favor debe ingresar correctamente su correo electrónico')){
			return(false)
		}
	}
	...
	if (frm.urbanizacion_dal.value.length<5){
		alert("Por favor debe especificar el lugar de ubicación (Urb./Residencial, etc) donde reside actualmente")
   ...
   if (modo=="A"){
	if (!validarAnio(frm.anioegresosec_dal,'Por favor ingrese correctamente el año en el que Egresó sus Estudios Universitarios',-1)){
		return(false)
	}

	if (frm.nombrecolegio_dal.value.length<5){
		alert("Por favor especifique el Nombre de Centro de Estudios Universitarios")
