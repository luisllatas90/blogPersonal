...
var tipo=document.all.cbotipoBD.value
if (tipo=='C'){
//BuscarBibliografia(frmbuscarCatalogo)
//alert('buscar catalogo')
BuscarBibliografia(frmbuscarLibros)
}
else{
//alert(tipo)
BuscarBibliografia(frmbuscarLibros)
...
<table width="100%">
	<tr>
		<td style="width: 70%"><span class="usatTitulo">Búsqueda de 
		bibliografía a solicitar</span></td>
...
<table width="100%" class="contornotabla" align="center">
	<tr>
		...
		<td style="width: 15%">
		<select name="cbotipoBD" class="Cajas2" style="display:true">
		...
		<img src="../../../../images/menus/buscar_small.gif" onClick="buscarLibros()">
		</td>
	</tr>
</table>
</form>
	...
	<tr height="95%">
		...
		<table width="100%">
			...
			<tr>
				...
				<td style="width: 48%" valign="top"><%'' Response.Write(tipoBD)%>
					<table border="0" width="100%" 	<%
					if trim(tipoBD)="L" then
						Response.Write(clasetabla(rsLibros("codigo_dpe")))
					else
						Response.Write(clasetabla(rsLibros("codigo_dpe")))
					end if	
					%> id="tblLibro
					<%
					if trim(tipoBD)="L" then
						Response.Write(rsLibros("idIngreso"))
					else
						Response.Write(rsLibros("idIngreso"))
					end if	
						%>
					...
					<tr class="piepagina" valign="top" >
						...
						<span class="etiqueta">Fecha de actualización: <%=rsLibros("FechaRegActual")%></span>
						</td>
					</tr>
					<tr class="piepagina" valign="top" >
					...
					<tr class="piepagina" valign="top" >
						...
						<td style="width: 85%"><%=rsLibros("preciounitario")%>&nbsp;</td>
					</tr>
					<tr class="piepagina" valign="top" >
						<td valign="middle" class="usatCeldaGris" style="width: 15%">Proveedor</td>
						<td style="width: 85%" align="right"><%if rsLibros("codigo_dpe")=0 then%>
						  <input name="cmdDetalle" type="button" value="    Ver más..." class="buscar2" onClick="AbrirDetallePedido('L','<%=rslibros("idIngreso")%>')">
						  <input name="cmdSolicitar" type="button" value="    Solicitar" class="solicitar" 
						  <%if tipoBD ='C' then%>						  
							onClick="AbrirDetalleDeCatalogo('<%=codigo_cco%>','<%=titulo%>','<%=rsLibros("preciounit")%>','<%=rsLibros("moneda")%>')"
						  <%else%>
	  						onClick="AbrirDetallePedido('A','<%=rslibros("idIngreso")%>')"
						  <%end if%>
						  >
						  <%end if%>
					  </td></tr>
				...
				<table border="0" width="100%" <%=clasetabla(rsLibros("codigo_dpe"))%> id="tblLibro<%=rsLibros("idIngreso")%>">
				<tr class="piepagina" valign="top" >
					<td class="usatCeldaGris" colspan="2" align="right">
					<span class="etiqueta">Fecha de actualización: <%=rsLibros("FechaRegActual")%></span>
					</td>
				...
				<tr class="piepagina" valign="top" >
					<td valign="middle" class="usatCeldaGris" style="width: 15%"><span class="usatCeldaGris" style="width: 15%">Proveedor</span></td>
					<td style="width: 85%" align="right"><input name="cmdDetalle" type="button" value="    Ver más..." class="buscar2" onClick="AbrirDetallePedido('L','<%=rslibros("idIngreso")%>')">
					<input name="cmdSolicitar" type="button" value="    Solicitar" class="solicitar" onClick="AbrirDetallePedido('A','<%=rslibros("idIngreso")%>')">
				  </td>
				</tr>
				</table>
				</td>
