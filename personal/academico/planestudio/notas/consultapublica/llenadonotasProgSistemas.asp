...
Set obj=Server.CreateObject("PryUSAT.clsAccesoDatos")
	...
	if codigo_tfu=1 or codigo_tfu=7 or codigo_tfu=16 then
		Set rsEscuela= obj.Consultar("ConsultarCarreraProfesional","FO","PS",0)
	else
		Set rsEscuela= obj.Consultar("consultaracceso","FO","ESC","Silabo",codigo_usu)
	end if

	if codigo_cac<>"-2" and codigo_cpf<>"-2" then
		Set rsProfesor= obj.Consultar("ConsultarNotas","FO","PS",codigo_cac,codigo_cpf,estadonota)
...
<% if lcase(session("Usuario_bit"))="usat\hzelada"  or lcase(session("Usuario_bit"))="usat\lotake"  or lcase(session("Usuario_bit"))="usat\ealonso" or lcase(session("Usuario_bit"))="usat\gvillalobos"  or lcase(session("Usuario_bit"))="usat\hreyes" then%>
...
<table border="0" cellpadding="3" cellspacing="0" style="border-collapse: collapse" bordercolor="#111111" width="100%" <%=alto%>>
 ...
 <tr>
    ...
    <input type="button" value="     Consultar..." name="cmdBuscar" class="buscar2" onClick="mensaje.innerHTML='<b>Espere un momento...</b>';actualizarlista('llenadonotasProgSistemas.asp?codigo_cac=' + cbocodigo_cac.value + '&codigo_cpf=' + cbocodigo_cpf.value + '&estadonota=' + cboestadonota.value)"></td>
...
<%
    ...
    response.write ("USUARIO NO AUTORIZADO PARA VER ESTE REPORTE!!!")
