...
Partial Class medicina_actividades
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Me.CmdNuevo.Attributes.Add("OnClick", "AbrirPopUp('frmactividades.aspx?e=n&pa=0&txtact=Actividades&syl=" & Request.QueryString("codigo_syl") & "&act=0',330,470,0,0,0,'Actividades'); return false;")

        Me.DataList1.DataBind()

    End Sub

    Private Sub CargarMenu(ByVal Nodo As TreeNode, ByVal codigo_act As Integer)
        Dim Tabla As New Data.DataTable
        Dim ObjDatos As New ClsSqlServer(ConfigurationManager.ConnectionStrings("CNXBDUSAT").ConnectionString)
        Tabla = ObjDatos.TraerDataTable("MED_COnsultarActividades", codigo_act, Request.QueryString("codigo_syl"), "A")
    ...
    Protected Sub TreeActividad_SelectedNodeChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles TreeActividad.SelectedNodeChanged
        ...
        If Me.TreeActividad.SelectedNode.Value <> 0 Then
            Me.CmdModificar.Attributes.Add("OnClick", "AbrirPopUp('frmactividades.aspx?e=m&syl=" & Request.QueryString("codigo_syl") & "&act=" & Me.TreeActividad.SelectedNode.Value & "',330,470,0,0,0,'Actividades'); return false;")
            Me.CmdNuevo.Attributes.Add("OnClick", "AbrirPopUp('frmactividades.aspx?e=n&pa=" & Me.TreeActividad.SelectedValue & "&txtact=" & Me.TreeActividad.SelectedNode.Text & "&syl=" & Request.QueryString("codigo_syl") & "&act=" & Me.TreeActividad.SelectedNode.Value & "',330,470,0,0,0,'Actividades'); return false;")
        Else
    ...
    Protected Sub CmdElminar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles CmdElminar.Click
        ...
        strEstado = objDatos.TraerDataTable("MED_ConsultarEstadoActividad", Me.HidenTree.Value).Rows(0).Item("estadorealizado_act").ToString
        If strEstado = "S" Then
            Response.Write("<script>alert('No se puede eliminar una actividad ya realizada.')</script>")
            objDatos = Nothing
        ElseIf strEstado = "D" Then
            Response.Write("<script>alert('El Item seleccionado tienen Actividades dependientes, elija primero las de rango inferior.')</script>")
            objDatos = Nothing
        Else
            Try
    ...
    Private Sub CargarTree()
        ...
        Dim ObjDatos As New ClsSqlServer(ConfigurationManager.ConnectionStrings("CNXBDUSAT").ConnectionString)
        datos = ObjDatos.TraerDataTable("MED_COnsultarActividades", 0, Request.QueryString("codigo_syl"), "A")
