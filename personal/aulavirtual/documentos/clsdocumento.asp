...
Class clsdocumento
  ...
  Public Property LET Cerrar(strModo)
  	Select case strModo
  		case "PL" 'Enviar al frmpermisos para especificar los usuarios del recurso
  			response.redirect "../usuario/frmagregarusuariosrecurso.asp?accion=agregarpermisos&idtabla=" & iddocumento & "&nombretabla=documento&tipodoc=L"
  		case "R"
  		...
  		case "MD" 'Modificar documento y actualizar lista y remarcarla
  			response.write "<script>window.opener.parent.location='listadocumentos.asp?idcarpeta=" & xidcarpeta & "&IdDocMarcado=" & iddocumento & "&numfila=" & filamodificada & "';window.close()</script>"
 		case "ED" 'Eliminar documento y actualizara lista
  		...
  		case "MOV" 'Mover carpeta de archivos
  			response.redirect "carpetas.asp"
  	End select
  End Property
  ...
  Function AbrirEnlace(ByVal Comentario,ByVal Link,ByVal Texto,ByVal IdDoc)
		AbrirEnlace="<a title='" & comentario & "' TARGET=""_blank"" href=" & Link & ">" & Texto & "</a>"
  End Function
  ...
  Public Function Modificar( byVal iddoc, byVal nombrearchivo, byVal titulodocumento, byVal idusuario, byVal fechainicio, byVal fechafin, byVal descripcion, byVal estado,byval escrit)
  	Set Obj= Server.CreateObject("AulaVirtual.clsdocumento")
  		Call Obj.Modificar(iddoc,nombrearchivo,titulodocumento,idusuario,fechainicio,fechafin,descripcion,estado,escrit)
  	Set Obj=Nothing
  End Function

  Public Function Eliminar(byVal tipodoc,byVal iddoc,ByVal arch)
	Set Obj= Server.CreateObject("AulaVirtual.clsdocumento")
  		Eliminar=Obj.Eliminar(tipodoc,iddoc)
  	Set Obj=Nothing
  	if tipodoc="A" then
  		call BorrarArchivoReg(arch)
  	end if
  End Function
  
  Public Function Mover(byVal tipodoc,byVal iddoc,ByVal nuevoiddoc)
	Set Obj= Server.CreateObject("AulaVirtual.clsdocumento")
  		call Obj.Mover(tipodoc,iddoc,nuevoiddoc)
  	Set Obj=Nothing
  End Function
  ...
  Public Function Modificarlink(byVal idlink, byVal iddoc, byVal tituloLink, byVal tipoLink, byVal URL )
  	Set Obj= Server.CreateObject("AulaVirtual.clsdocumento")
  		Call Obj.Modificarlink(idlink,iddoc,titulolink,tipolink,URL)
  	Set Obj=Nothing	
  End Function

  Public Function Eliminarlink( byVal idlink)
	Set Obj= Server.CreateObject("AulaVirtual.clsdocumento")
  		Call Obj.Eliminarlink(idlink)
  	Set Obj=Nothing
  End Function
  
  'Agregar versión se ha procesado en guardarversion.asp
  	  
  Public Function ModificarVersion(byval idver,tituloversion,byval publica,byval bloqueada)
    Set Obj= Server.CreateObject("AulaVirtual.clsdocumento")
  		Call Obj.ModificarVersion(idver,tituloversion,publica,bloqueada)
  	Set Obj=Nothing
  End Function
  
  Public Function EliminarVersion(Byval idversion,byval arch)
  	Set Obj= Server.CreateObject("AulaVirtual.clsdocumento")
  		EliminarVersion=Obj.EliminarVersion(idversion)
  	Set Obj=Nothing
  	if EliminarVersion="" then
  		call BorrarArchivoReg(arch)
  		response.write "<script>parent.location.reload()</script>"
  	else
  		response.write "<script>alert('" & EliminarVersion & "');history.back(-1)</script>"
  	end if
  End Function
