...
function AbrirTarea(modo,numfila,pagina)
{
   var idtarea=txtelegido.value
	switch(modo)
	{
		...
		case "M":
			AbrirPopUp(pagina + "&accion=modificartarea&idtarea=" + idtarea + "&numfila=" + numfila,"350","620")
			break

		case "E":
			var pagina="procesar.asp?accion=eliminartarea&idtarea=" + idtarea
			Eliminar("La acción a realizar es irreversible\n¿Está seguro completamente seguro que desea eliminar la Tarea?",pagina)
			break

		case "P":
			var ruta="../usuario/frmcompartirrecurso.asp"			
			var titulotarea=txttitulotarea.innerText
			var descripcion=""
			
			AbrirPopUp(ruta + "?titulo=Tarea: " + titulotarea + "&idtabla=" + idtarea + "&nombretabla=tarea&descripcion=" + descripcion,"500","600")
			break
	}
}
...
function IntegrarRecurso(modo,idtarea,idtarearec)
{
	switch(modo) 
	{
		case "A":
			...
			break
		case "E":
			var pagina="procesar.asp?accion=eliminartarearecurso&idtarea=" + idtarea + "&idtarearecurso=" + idtarearec
			Eliminar("La acción a realizar es irreversible\n¿Está seguro completamente seguro que desea quitar el recurso de la Tarea?",pagina)
			break
	}
}
...
function AbrirVersionTarea(modo,idtarea,idtareausuario,arch)
{
	switch (modo){
		case "A":
			...
			AbrirPopUp("generacadena.asp?pagina=frmenviararchivo.aspx&idtarea="+ idtarea + "&idtareausuario=" + idtareausuario,"300","550")
			break
		case "M":
			location.href="detalleversion.asp?modo=modificarversion&idtarea=" + idtarea + "&idtareausuario=" + idtareausuario
			break
		case "E":
			var pagina="procesar.asp?accion=eliminartareausuario&idtarea=" + idtarea + "&idtareausuario=" + idtareausuario + "&archivo=" + arch
		...
		case "G":
			var bloqueada=0
			if (chkbloqueada.checked==true)
				{bloqueada=1}
			location.href="procesar.asp?accion=modificartareausuario&idtarea=" + idtarea + "&idtareausuario=" + idtareausuario  + "&obs=" + obs.value + "&bloqueada=" + bloqueada
			break
		case "R":
