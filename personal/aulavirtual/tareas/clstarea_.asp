  ...
  Public function AbrirTarea(byval idfuncion,byval idusuario,byval idestadorecurso,byval idtarea,byval titulotarea,byval idtipotarea,byval descripciontipotarea,byval permitirreenvio)
  	...
  	'if (idestadorecurso=1 OR idfuncion=1) then
  		select case idtipotarea
        	...
        	case "4":
        		if permitirrenvio=1 then
        			pagina=iif(idfuncion=3,"listaversiones.asp","listatareausuario.asp")
				else
					pagina="listatareausuario.asp"
				end if
        end select%>
		<a target="_parent" href="<%=pagina%>?idtarea=<%=idtarea%>&titulotarea=<%=titulotarea%>&idtipotarea=<%=idtipotarea%>&descripciontipotarea=<%=descripciontipotarea%>&idusuario=<%=idusuario%>&permitirreenvio=<%=permitirreenvio%>&idestadotarea=<%=idestadorecurso%>">
