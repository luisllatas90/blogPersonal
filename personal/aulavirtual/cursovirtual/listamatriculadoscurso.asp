...
<table border="0" cellpadding="0" cellspacing="0" style="border-collapse: collapse" bordercolor="#111111" width="100%" class="barraherramientas">
    <tr>
      ...
      <%if codigo_apl<>4 then
  		...
  		<input type="button" onclick="Javacript:location.href='../usuario/frmbuscarusuario.asp?idtabla=<%=idcursovirtual%>&nombretabla=cursovirtual&codigo_apl=<%=codigo_apl%>&codigo_tfu=<%=codigo_tfu%>'" value="   Agregar participantes" class="agregar3" style="width: 180" name="cmdGuardar">
		<input type="submit" value="Eliminar" name="cmdEliminar" class="eliminar3">
		<%if trim(lcase(session("codigo_usu")))="usat\gchunga" then%>
		<input type="button" onclick="Javacript:location.href='frmusuario.asp?accion=agregarusuario'" value="     Nuevo Participante" class="agregar3" style="width: 150">
	  	<%end if
  		end if%>&nbsp;
