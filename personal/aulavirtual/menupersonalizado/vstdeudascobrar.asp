...
if fechainicio="" then fechainicio=date
if fechafin="" then fechafin=date

Set obj= Server.CreateObject("AulaVirtual.clsAccesoDatos")
	obj.AbrirConexion
		Set rsServicio=obj.Consultar("ConsultarCursoVirtual","FO",8,session("idcursovirtual"),0,0)
		
		codigo_sco=rsServicio("codigo_sco")
	obj.CerrarConexion
Set obj=nothing
...
<script type="text/javascript" language="javascript">
	...
	{
		pagina="../aulavirtual/menupersonalizado/vstdeudascobrar.asp?codigo_sco=<%=codigo_sco%>&fechainicio=" + txtFechaInicio.value + "&fechafin=" + txtFechaFin.value + "&moneda=S"
...
if codigo_sco<>"-2" then
	...
	<table bgcolor="white" bordercolor="silver" border="1" cellpadding="3" style="width: 100%;border-collapse:collapse">
		...
		<tr <%=clase%> onMouseOver="Resaltar(1,this,'S')" onMouseOut="Resaltar(0,this,'S')">
			...
			<td align="right"><%=rsClientes("cargos")%>&nbsp;</td>
			<td align="right"><%=rsClientes("abonos")%>&nbsp;</td>
			<td align="right"><%=rsClientes("diferencia")%>&nbsp;</td>
		</tr>
		<%rsClientes.movenext
		Loop
		'rsClientes.close
