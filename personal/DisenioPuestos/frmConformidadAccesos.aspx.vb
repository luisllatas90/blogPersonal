...
Partial Class DisenioPuestos_frmConformidadAccesos
...
#Region "Eventos"
    ...
    '    Try
    '        Session("frmConformidadAccesos-codigo_pue") = 0
    ...
    Protected Sub grwLista_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles grwLista.RowCommand
        Try
            ...
            'Session("frmConformidadAccesos-codigo_pue") = Me.grwLista.DataKeys(index).Values("codigo_pue").ToString

            Select Case e.CommandName
                Case "GestionarFicha"
                    Call mt_CargarDatosTarea(Session("frmConformidadAccesos-codigo_pue"))
                    Call mt_CargarDatosT1(Session("frmConformidadAccesos-codigo_pue"))
                    Call mt_CargarDatosT2(Session("frmConformidadAccesos-codigo_pue"))
                    Call mt_CargarDatosT3(Session("frmConformidadAccesos-codigo_pue"))
                    Call mt_CargarDatosT4(Session("frmConformidadAccesos-codigo_pue"))
                    Call mt_CargarDatosCompetencia(Session("frmConformidadAccesos-codigo_pue"))
                    Call mt_CargarDatosRecomendacion(Session("frmConformidadAccesos-codigo_pue"))
                ...
                Case "Editar"
                    ...
                    If Not mt_CargarFormularioRegistro(CInt(Session("frmConformidadAccesos-codigo_pue"))) Then Exit Sub
                ...
                Case "Eliminar"
                    ...
                    If Not mt_EliminarNotificacion(CInt(Session("frmConformidadAccesos-codigo_pue"))) Then Exit Sub
    ...
    Protected Sub btnGuardar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnGuardar.Click
        Try
            If mt_RegistrarNotificacion(CInt(Session("frmConformidadAccesos-codigo_pue"))) Then
                Call btnListar_Click(Nothing, Nothing)
                Call mt_FlujoTabs("Listado")
                Exit Sub
            End If
...
#Region "Metodos"
    ...
    Private Sub mt_LimpiarVariablesSession()
        Try
            'Session("frmConformidadAccesos-codigo_pue") = Nothing
        Catch ex As Exception
            Call mt_ShowMessage(ex.Message.Replace("'", " "), MessageType.error)
        End Try
    End Sub
    ...
    Private Function fu_ValidarCargarFormularioRegistro() As Boolean
        Try
            'If Session("frmConformidadAccesos-codigo_pue") Is Nothing OrElse String.IsNullOrEmpty(Session("frmConformidadAccesos-codigo_pue")) Then mt_ShowMessage("El código de notificación no ha sido encontrado.", MessageType.warning) : Return False
    ...
    Private Function fu_ValidarRegistrarNotificacion() As Boolean
        Try
            ...
            'If dt.Rows.Count > 0 AndAlso CInt(Session("frmConformidadAccesos-codigo_pue")) = 0 Then mt_ShowMessage("Existe un registro con este tipo, clasificación, abreviatura y versión.", MessageType.warning) : Return False

            'For Each fila As DataRow In dt.Rows
            '    If CInt(Session("frmConformidadAccesos-codigo_pue")) <> CInt(fila("codigo_pue")) Then mt_ShowMessage("Existe un registro con este tipo, clasificación, abreviatura y versión.", MessageType.warning) : Return False
            'Next
