...
Partial Class DisenioPuestos_frmGeneralidades
...
#Region "Eventos"
    ...
    Protected Sub btnNuevo_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnNuevo.Click
        Try
            Session("frmGeneralidades-codigo_not") = 0
    ...
    Protected Sub grwLista_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles grwLista.RowCommand
        Try
            ...
            Session("frmGeneralidades-codigo_gen") = Me.grwLista.DataKeys(index).Values("codigo_gen").ToString

            Select Case e.CommandName
                Case "Editar"
                    'If Not fu_ValidarCargarFormularioRegistro() Then Exit Sub

                    'If Not mt_CargarFormularioRegistro(CInt(Session("frmGeneralidades-codigo_not"))) Then Exit Sub
                ...
                Case "Eliminar"
                    ...
                    If Not mt_EliminarNotificacion(CInt(Session("frmGeneralidades-codigo_not"))) Then Exit Sub

                    Call btnListar_Click(Nothing, Nothing)

                Case "GestionarEstructura"
                    Call mt_CargarDatosEstructura(Session("frmGeneralidades-codigo_gen"))

                    Call mt_FlujoTabs("Estructura")

                Case "GestionarArea"
                    Call mt_CargarDatosArea(Session("frmGeneralidades-codigo_gen"))

                    Call mt_FlujoTabs("Area")
    ...
    Protected Sub btnGuardar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnGuardar.Click
        Try
            If mt_RegistrarNotificacion(CInt(Session("frmGeneralidades-codigo_not"))) Then
                Call btnListar_Click(Nothing, Nothing)
                Call mt_FlujoTabs("Listado")
                Exit Sub
            End If
    ...
    Protected Sub btnNuevoEstructura_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnNuevoEstructura.Click
        Try
            Session("frmEmpresa-codigo_est") = 0

            'Call mt_LimpiarControlesContacto()

            Call mt_UpdatePanel("RegistroEstructura")

            Call mt_FlujoModal("RegistrarEstructura", "open")
        Catch ex As Exception
            Call mt_ShowMessage(ex.Message.Replace("'", " "), MessageType.error)
        End Try
    End Sub

    Protected Sub btnNuevoArea_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnNuevoArea.Click
        Try
            Session("frmEmpresa-codigo_est") = 0

            'Call mt_LimpiarControlesContacto()

            Call mt_UpdatePanel("RegistroArea")

            Call mt_FlujoModal("RegistrarArea", "open")
        Catch ex As Exception
            Call mt_ShowMessage(ex.Message.Replace("'", " "), MessageType.error)
        End Try
    End Sub

    Protected Sub btnSalirEstructura_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSalirEstructura.Click
        Try
            Call mt_FlujoTabs("Listado")
        Catch ex As Exception
            Call mt_ShowMessage(ex.Message.Replace("'", " "), MessageType.error)
    ...
    Protected Sub btnSalirArea_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSalirArea.Click
        Try
            Call mt_FlujoTabs("Listado")
        Catch ex As Exception
            Call mt_ShowMessage(ex.Message.Replace("'", " "), MessageType.error)
...
#Region "Metodos"
    ...
    Private Sub mt_UpdatePanel(ByVal ls_panel As String)
        Try
            Select Case ls_panel
                ...
                Case "Estructura"
                    Me.udpEstructura.Update()
                    ScriptManager.RegisterStartupScript(Me, Me.Page.GetType, "udpEstructuraUpdate", "udpEstructuraUpdate();", True)

                Case "Area"
                    Me.udpArea.Update()
                    ScriptManager.RegisterStartupScript(Me, Me.Page.GetType, "udpAreaUpdate", "udpAreaUpdate();", True)
    ...
    Private Sub mt_FlujoTabs(ByVal ls_tab As String)
        Try
            Select Case ls_tab
                ...
                Case "Estructura"
                    Me.udpScripts.Update()
                    ScriptManager.RegisterStartupScript(Me, Me.Page.GetType, "flujoTabs", "flujoTabs('estructura-tab');", True)

                Case "Area"
                    Me.udpScripts.Update()
                    ScriptManager.RegisterStartupScript(Me, Me.Page.GetType, "flujoTabs", "flujoTabs('area-tab');", True)
    ...
    Private Sub mt_LimpiarVariablesSession()
        Try
            'Session("frmGeneralidades-codigo_not") = Nothing
        Catch ex As Exception
            Call mt_ShowMessage(ex.Message.Replace("'", " "), MessageType.error)
        End Try
    End Sub
    ...
    Private Sub mt_CargarDatos()
        Try
            ...
            dt.Columns.Add("codigo_gen")
            dt.Columns.Add("decreto")
    ...
    Private Function fu_ValidarCargarFormularioRegistro() As Boolean
        Try
            'If Session("frmGeneralidades-codigo_not") Is Nothing OrElse String.IsNullOrEmpty(Session("frmGeneralidades-codigo_not")) Then mt_ShowMessage("El código de notificación no ha sido encontrado.", MessageType.warning) : Return False
    ...
    Private Function fu_ValidarRegistrarNotificacion() As Boolean
        Try
            ...
            'If dt.Rows.Count > 0 AndAlso CInt(Session("frmGeneralidades-codigo_not")) = 0 Then mt_ShowMessage("Existe un registro con este tipo, clasificación, abreviatura y versión.", MessageType.warning) : Return False

            'For Each fila As DataRow In dt.Rows
            '    If CInt(Session("frmGeneralidades-codigo_not")) <> CInt(fila("codigo_not")) Then mt_ShowMessage("Existe un registro con este tipo, clasificación, abreviatura y versión.", MessageType.warning) : Return False
            'Next
    ...
    Private Sub mt_CargarDatosEstructura(ByVal codigo_emp As Integer)
        Try
            Dim dt As New DataTable

            If Me.grwEstructura.Rows.Count > 0 Then Me.grwEstructura.DataSource = Nothing : Me.grwEstructura.DataBind()

            dt.Columns.Add("codigo_est")
            dt.Columns.Add("organo")
            ...
            fila = dt.NewRow()
            fila("codigo_est") = 1
            fila("organo") = "Órganos de dirección"
            fila("nombre") = "Decano"
            dt.Rows.Add(fila)

            fila = dt.NewRow()
            fila("codigo_est") = 2
            fila("organo") = "Órganos de asesoría"
            fila("nombre") = "Director del Departamento Académico"
            dt.Rows.Add(fila)

            fila = dt.NewRow()
            fila("codigo_est") = 3
            fila("organo") = "Órganos de apoyo"
            fila("nombre") = "Director de la Escuela de Medicina Humana"
            dt.Rows.Add(fila)

            Me.grwEstructura.DataSource = dt
            Me.grwEstructura.DataBind()

            Call md_Funciones.AgregarHearders(grwEstructura)

            Call mt_UpdatePanel("Estructura")
        Catch ex As Exception
            Call mt_ShowMessage(ex.Message.Replace("'", " "), MessageType.error)
        End Try
    End Sub

    Private Sub mt_CargarDatosArea(ByVal codigo_emp As Integer)
        Try
            Dim dt As New DataTable

            If Me.grwArea.Rows.Count > 0 Then Me.grwArea.DataSource = Nothing : Me.grwArea.DataBind()
            ...
            fila = dt.NewRow()
            fila("codigo_est") = 1
            fila("area") = "Facultad de Medicina"
            dt.Rows.Add(fila)

            fila = dt.NewRow()
            fila("codigo_est") = 2
            fila("area") = "Facultad de Derecho"
            dt.Rows.Add(fila)

            fila = dt.NewRow()
            fila("codigo_est") = 3
            fila("area") = "Facultad de Ingeniería"
            dt.Rows.Add(fila)

            Me.grwArea.DataSource = dt
            Me.grwArea.DataBind()

            Call md_Funciones.AgregarHearders(grwArea)

            Call mt_UpdatePanel("Area")
        Catch ex As Exception
            Call mt_ShowMessage(ex.Message.Replace("'", " "), MessageType.error)
        End Try
    End Sub
