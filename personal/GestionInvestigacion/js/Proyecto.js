...
$(document).ready(function() {
    ...
    if (rpta == true) {
        var dtP = fnCreateDataTableBasic('tProyectos', 1, 'asc', 15);
...
function cPersonal(ctf) {
    ...
    for (i = 0; i < n; i++) {
        if (arr[i].nombre == 'TODOS' || n == 1) {
            sel = "selected='selected'";
        }
        str += '<option value="' + arr[i].cod + '" ' + sel + ' >' + arr[i].nombre + '</option>';
    }
...
function fnGuardarProyecto() {
    ...
    $("#MensajeGuardar").html('<b>Guardando Proyecto...</b>');
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data[0].rpta == 1) {
                        ...
                        fnConfirmarEnviar(data[0].cod, 99);
                        fnListarProyecto();
                        fnLoading(false);
                ...
                success: function(data) {
                    ...
                    if (filas > 0) {
                        for (i = 0; i < filas; i++) {
                            ...
                            tb += '<td style="text-align:center">';
                            if (data[i].estado == "EN EVALUACIÓN" || data[i].estado == "APROBADO") {
                                clase_icono = "ion-eye";
                            } else {
                            ...
                            if ((data[i].estado == 'REGISTRO' || data[i].estado == 'OBSERVADO') && (ObtenerValorGET("ctf") == 1 || ObtenerValorGET("ctf") == 13 || ObtenerValorGET("ctf") == 65)) {
                                ...
                                tb += '<button type="button" id="btnEnviar" name="btnEnviar" class="btn btn-sm btn-orange" onclick="fnConfirmarEnviar(\'' + data[i].cod + '\',99)" title="Enviar a Evaluación" ><i class="ion-arrow-right-a"></i></button>';
                            }
                    ...
                    $('#tbProyectos').html(tb);
                    fnResetDataTableBasic('tProyectos', 1, 'asc', 15);
    ...
    if (rpta == true) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (data[0].estado == 'APROBADO' || data[0].estado == 'EN EVALUACIÓN') {
                    $("#btnA").attr("style", "display:none");
                    $("#frmRegistro input").attr("disabled", "disabled");
                    $("#hdcod").removeAttr("disabled");
                } else {
                    $("#btnA").removeAttr("style");
                    $("#frmRegistro input").removeAttr("disabled");
                }
                if (data[0].cod_dis != "0") {
...
function fnEnviar(cod, vered) {
    ...
    if (rpta == true) {
        ...
        $('form#frmbuscar').append('<input type="hidden" id="txtobservacion" name="txtobservacion" value="" />');
        var form = $("#frmbuscar").serializeArray();
        $("form#frmbuscar input[id=action]").remove();
        $("form#frmbuscar input[id=ctf]").remove();
        $("form#frmbuscar input[id=cod]").remove();
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (filas > 0) {
                    ...
                    for (i = 0; i < filas; i++) {
                        if (data[i].sol == 0) {
                            div += "<label style='font-size:12px;line-height:10px;margin-botton:2px;color:red'>" + (i + 1) + " .- " + data[i].fec + " - " + data[i].des + "</label></br>";
                        } else {
                            div += "<label style='font-size:12px;line-height:10px;margin-botton:2px;color:green'>" + (i + 1) + " .- " + data[i].fec + " - " + data[i].des + "</label></br>";
                        }
                    }
                    div += "</div>";
                } else {
...
function fnValidarProyecto() {
    ...
    if (banderaG == 0) {
        fnMensaje("warning", "Proyecto Debe Contar con un Objetivo General.");
        return false;
    }
    ...
    if (fecini > fecfin) {
        fnMensaje("warning", "Fecha de Fin de Proyecto no Puede ser Menor a la Fecha de Inicio.");
        return false;
    }
    ...
    if ($("#txtpresupuesto").val() == "") {
        fnMensaje("warning", "Ingrese el Presupuesto del Proyecto.");
        return false;
    }
    var RE = /^\d*(\.\d{1})?\d{0,1}$/;
    if (!RE.test($("#txtpresupuesto").val())) {
        fnMensaje("warning", "Ingrese Correctanente el Presupuesto, puede colocar hasta 2 decimales después del punto decimal.");
        return false;
    }
    if ($("#txtavance").val() == "") {
    ...
    if ($("#cboAvance").val() == "") {
        fnMensaje("warning", "Seleccione el Estado de Avance.");
        return false;
    }
    /*if ($("#filepto").val() != '') {
    archivo = $("#filepto").val();
    ...
    if ($("#fileinforme").val() != '') {
        ...
        if (permitida == false) {
            ...
            fnMensaje("warning", "Solo puede Adjuntar Archivos de Informe en Formato de PDF");
            return false;
        }
    }
...
function fnConfirmarEnviar(cod, veredicto) {
    fnMensajeConfirmarEliminar('top', '¿Desea Enviar Proyecto a Evaluación.?', 'fnEnviar', cod, veredicto);
}
...
function fnConfirmarEliminar(cod) {
    fnMensajeConfirmarEliminar('top', '¿Desea Enviar Proyecto a Evaluación.?', 'fnEliminarProyecto', cod);
}

function fnEliminarProyecto(cod) {
    rpta = fnvalidaSession()
    if (rpta == true) {
        fnLoading(true)
        $("form#frmbuscar input[id=cod]").remove();
