...
//                nap++;
    ...
    function fnValidaCursoDesaprobadoJSON(ccur) {
        var credSel = parseInt($('#h6CreditosSeleccionados').html());
        var i = 0;
        var sw = true;
        for (var idx in lstCursoD) {
            key = lstCursoD[idx];
            i++;
        }
        ...
        if (i > 0) {
            if (credSel > 15) {
                ...
                for (var idx in lstCursoD) {
                    key = lstCursoD[idx];
                    t += '<li><i class="ion-android-checkbox-outline"></i> ' + key.nom + '</li>';
                    i++;
                }
                t += '<ul>';
                sw = false;
                fnMensajeModal('alert', 'Ha seleccionado cursos que ha desaprobado 2 veces. Sólo puede matricularse hasta 15 créditos: <br>' + t);
            } 
        }
    ...
    function fnSeleccionar(ccup, ccur) {             
        ...
        if (sw) {
            ...
            rpta4 = fnValidaCursoDesaprobadoJSON(ccur);
           // console.log(rpta4);
            if (!rpta4) {
    ...
    function fnCrdSituacion(sumcredSel) {
        var className = $('#lblsituacion').attr('class');
        if (sumcredSel >= aDataCnf[0].ncrdSituacion) {
            //alert('Regular');
            $('#lblsituacion').html('Regular');
            $('#lblsituacion').removeClass(className).addClass('label label-success');
            

        } else {
            $('#lblsituacion').html('No Regular');
            $('#lblsituacion').removeClass(className).addClass('label label-warning');
        }
...
//        return false;
        ...
        $.ajax({
            ...
            contentType: "application/json; charset=utf-8",
            url: "procesar.aspx",
            data: { "param0": "regMat", "param1": obj, "M": m, "O": o, "T": t,"st": st },
            dataType: "json",
            success: function(data) {
           // console.log(data);
                if (data[0].R == "C") {
                ...
                if (data[0].URLCARTA != "") window.open('ImprimirCarta.aspx');
                    fnMensaje("success", data[0].Mensaje);
