Partial Class frmpersona
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load        
        If IsPostBack = False Then
            If (Request.QueryString("sou") = "A") Then
                Me.btnRegresar.Visible = True
                Me.btnRegresar.Attributes.Add("onclick", "history.back(); return false;")
            Else
            ...
            'Asignamos Valor por defecto en situacion laboral
            Me.rblSituacionLaboral.SelectedValue = "N"
            Me.txtNumMeses.Text = 0
        ...
        Try
            If (validaIngresoForm() = True) Then                
                ...
                If (Me.fileCV.HasFile = True) Then
                    strRutaCV = Server.MapPath("..\..\librerianet\Egresado\curriculum")
                ...
                If (Me.fileFoto.HasFile = True) Then
                    strRutaFoto = Server.MapPath("..\..\librerianet\Egresado\fotos")
                ...
                If ValidarNroIdentidad() = True Then
                    ...
                    If (Request.QueryString("sou") = "A") Then
                        ...
                        obj.Ejecutar("ALUMNI_RegistraPreEgresado", Me.hdcodigo_pso.Value, _
                                    ...
                                    Me.chkMostrar.Checked, Me.dpNivel.SelectedItem.Text, 0, "1", _
                        ...
                        strClave = GeneraPassword()
                        obj.Ejecutar("ALUMNI_ActivaEgresado", Me.hdcodigo_pso.Value, strClave)
                        obj.CerrarConexion()
                        ...
                        strMensaje = "Estimado(a) " & Me.txtAPaterno.Text & " " & Me.txtAMaterno.Text & ", " & Me.txtNombres.Text & ": <br />"
                        strMensaje = strMensaje & "Ud. acaba de ser activado en la lista de Egresados de la universidad. <br />"
                        strMensaje = strMensaje & "Usuario: " & Me.txtdni.Text & "<br />"
                        strMensaje = strMensaje & "Clave: " & strClave & "<br /><br />"

                        Dim strError As String = ""
                        'strError = cls.EnviarUno("Confirmación de Egresado ALUMNI", strMensaje, dtDirector.Rows(0).Item(0), Me.hdcodigo_pso.Value, True)

                        Me.lblmensaje.Text = "Egresado registrado"
                        If (strError <> "") Then
                            Me.lblmensaje.Text = strError
                        End If
    ...
    Protected Sub lnkComprobarDNI_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkComprobarDNI.Click
        ...
        If ValidarNroIdentidad() = True Then
            ...
            If (dtEgresado.Rows(0).Item(0).ToString.Trim <> "") Then
                lnkComprobarNombres.Text = "Clic aquí para buscar coincidencias"
                Me.lblEgresado.Text = dtEgresado.Rows(0).Item("Egresado").ToString.ToLower
                If (Me.dpTipoDoc.SelectedIndex = 2) Then
                    BuscarPersona("DNIE", dtEgresado.Rows(0).Item(1).ToString)  'Nro Documento
                Else
                    BuscarPersona("DNIE", Me.txtdni.Text.Trim)
                End If

                If (dtEgresado.Rows(0).Item(0).ToString = "BACHILLER") Then
                    Me.dpNivel.SelectedValue = "BACHILLER"
                ElseIf (dtEgresado.Rows(0).Item(0).ToString = "EGRESADO") Then
    ...
    Private Function ValidarNroIdentidad(Optional ByVal IrAlFoco As Boolean = True) As Boolean
        ...
        ElseIf Me.dpTipoDoc.SelectedIndex = 1 And Me.txtdni.Text.Length < 9 Then
            ...
            If IrAlFoco = True Then Me.txtdni.Focus()
            'Response.Write(3)
            'ElseIf Me.dpTipoDoc.SelectedIndex = 2 And Me.txtdni.Text.Length < 11 Then
            '    Me.lblmensaje.Text = "El Cod. Universitario es incorrecto. Mínimo 10 caracteres"
            '    Me.lblmensaje.Visible = True
            '    ValidarNroIdentidad = False
        Else
            ValidarNroIdentidad = True
    ...
    Private Function GeneraPassword() As String
        Dim rnd As New Random
        Dim ubicacion As Integer
        Dim strNumeros As String = "0123456789"
        Dim strLetraMin As String = "abcdefghijklmnopqrstuvwxyz"
