...
Partial Class PlanProyecto_lstCalendario
    ...
    Private Sub CargaCalendarioAnual()
        ...
        obj.AbrirConexion()
        dtAnio = obj.TraerDataTable("PLAN_RetornaAnioSeleccion", strTipoPro, codigo_pro)
        obj.CerrarConexion()
        obj = Nothing
    ...
    Private Function fn_actividades() As String
        ...
        'Capturamos el codigo del proyecto 
        If (Right(Request.QueryString("pro").ToString, 1) = "T") Then
            codigo_pro = Request.QueryString("pro").ToString.Substring(0, Request.QueryString("pro").ToString.Length - 1)

            'Verifica si el calendario esta finalizado
        ...
        If (blnFinalizado = False) Then            
            ...
            Else
                'Capturamos todos los proyectos relacionado al TIPO de Proyecto seleccionado                
        ...
        For i As Integer = 0 To dt.Rows.Count - 1            
            ...
            If (dt.Rows(i).Item("proceso").ToString = "") Then 'SI ES PROCESO
                If (dt.Rows(i).Item(0).ToString = "12") Then
                    strAct = strAct & "<td colspan='2' style='width:175px; background:#FFFF66; font-size:11px; border-bottom-width:1px; border-bottom-color:#FFFFFF; border-bottom-style:solid; border-right-style:solid; border-right-color:#D8DADB; border-right-width:1px;'><b> " & dt.Rows(i).Item(1) & "</b></td>"
                Else
            ...
            Else    'SI NO ES PROCESO                               
                If (dt.Rows(i).Item("proceso").ToString = "S") Then 'SI ES PROCESO
                    If (dt.Rows(i).Item(0).ToString = "12") Then  'Ubicacion 12 se encuentr
                        If (dt.Rows(i).Item("feriado_apr") = False) Then
                            strAct = strAct & "<td colspan='2' style='width:175px; background:#FFFF66; font-size:11px; border-bottom-width:1px; border-bottom-color:#FFFFFF; border-bottom-style:solid; border-right-style:solid; border-right-color:#D8DADB; border-right-width:1px;'><b> " & dt.Rows(i).Item(1) & "</b></td>"
                        Else
                            strAct = strAct & "<td colspan='2' style='width:175px; background:#F7FE2E; font-size:11px; border-bottom-width:1px; border-bottom-color:#FFFFFF; border-bottom-style:solid; border-right-style:solid; border-right-color:#D8DADB; border-right-width:1px;'><b> " & dt.Rows(i).Item(1) & "</b></td>"
                        End If
                    Else
                        If (dt.Rows(i).Item("feriado_apr") = False) Then
                            strAct = strAct & "<td colspan='2' style='width:175px; background:#CEE3F6; font-size:11px; border-bottom-width:1px; border-bottom-color:#FFFFFF; border-bottom-style:solid; border-right-style:solid; border-right-color:#D8DADB; border-right-width:1px;'><b> " & dt.Rows(i).Item(1) & "</b></td>"
                    ...
                    If (j < 54) Then
                        ...
                        Else    'Tiene semanas
                            ...
                            If (dt.Rows(i).Item(j - 1).ToString = "") Then
                                If (dt.Rows(i).Item(j + 1).ToString.Trim = "") Then
                                    strAct = strAct & "<td title='Actividad' align='center'  style='width:13.5px; padding-left:0.3px; background: " & colorFeriado & "; color:" & dt.Rows(i).Item("color").ToString & "; cursor:hand; border-bottom-width:1px; border-bottom-color:#0B0B61; border-bottom-style:solid; border-right-style:solid; border-right-color:#D8DADB; border-right-width:1px;'>" & _
                                    "<a href='frmResumenActividad.aspx?apr=" & dt.Rows(i).Item(j) & "&KeepThis=true&TB_iframe=true&height=450&width=550' title='Actividades' class='thickbox' style='color:black'>" & IIf(dt.Rows(i).Item("mostrarDias_apr"), "<u>" & diaIni, "&nbsp;")
                                    strAct = strAct & IIf(dt.Rows(i).Item("mostrarDias_apr"), "</u><br/>" & diaFin, "&nbsp;")
                                    strAct = strAct & "</a>" & "</td>"
                                Else
                                    strAct = strAct & "<td title='Actividad' align='center'  style='width:13.5px; padding-left:0.3px; background: " & colorFeriado & "; color:" & dt.Rows(i).Item("color").ToString & "; cursor:hand; border-bottom-width:1px; border-bottom-color:#0B0B61; border-bottom-style:solid; border-right-style:solid; border-right-color:#D8DADB; border-right-width:1px;'>" & _
                                    "<a href='frmResumenActividad.aspx?apr=" & dt.Rows(i).Item(j) & "&KeepThis=true&TB_iframe=true&height=450&width=550' title='Actividades' class='thickbox' style='color:black'>" & IIf(dt.Rows(i).Item("mostrarDias_apr"), diaIni, "&nbsp;")
                                    strAct = strAct & "</a>" & "</td>"
                                End If
                            ElseIf (dt.Rows(i).Item(j + 1).ToString = "") Then  'Indica el final de esa actividad
                                strAct = strAct & "<td title='Actividad' align='center'  style='width:13.5px; background: " & colorFeriado & "; color:" & dt.Rows(i).Item("color").ToString & "; cursor:hand; border-bottom-width:1px; border-bottom-color:#0B0B61; border-bottom-style:solid; border-right-style:solid; border-right-color:#D8DADB; border-right-width:1px;'>" & _
                                       "<a href='frmResumenActividad.aspx?apr=" & dt.Rows(i).Item(j) & "&KeepThis=true&TB_iframe=true&height=450&width=550' title='Actividades' class='thickbox' style='color:black'>" & IIf(dt.Rows(i).Item("mostrarDias_apr"), diaFin, "&nbsp;")
                                strAct = strAct & "</a>" & "</td>"
                            ElseIf (j - 1 = 1) Then 'Indica el Inicio de esa actividad
                                strAct = strAct & "<td title='Actividad' align='center'  style='width:13.5px; background: " & colorFeriado & "; color:" & dt.Rows(i).Item("color").ToString & "; cursor:hand; border-bottom-width:1px; border-bottom-color:#0B0B61; border-bottom-style:solid; border-right-style:solid; border-right-color:#D8DADB; border-right-width:1px;'>" & _
                                       "<a href='frmResumenActividad.aspx?apr=" & dt.Rows(i).Item(j) & "&KeepThis=true&TB_iframe=true&height=450&width=550' title='Actividades' class='thickbox' style='color:black'>" & IIf(dt.Rows(i).Item("mostrarDias_apr"), diaIni, "&nbsp;")
                            Else
                                If (j = 53) Then
                                    strAct = strAct & "<td title='Actividad' align='center'  style='width:13.5px; background: " & colorFeriado & "; color:" & dt.Rows(i).Item("color").ToString & "; cursor:hand; border-bottom-width:1px; border-bottom-color:#0B0B61; border-bottom-style:solid; border-right-style:solid; border-right-color:#D8DADB; border-right-width:1px;'>" & _
                                        "<a href='frmResumenActividad.aspx?x=d&apr=" & dt.Rows(i).Item(j) & "&KeepThis=true&TB_iframe=true&height=450&width=550' title='Actividades' class='thickbox' style='color:black'>" & IIf(dt.Rows(i).Item("mostrarDias_apr"), diaFin, "&nbsp;")
                                    strAct = strAct & "</a>" & "</td>"
                                Else
                                    strAct = strAct & "<td title='Actividad' align='center'  style='width:13.5px; background: " & colorFeriado & "; color:" & dt.Rows(i).Item("color").ToString & "; cursor:hand; border-bottom-width:1px; border-bottom-color:#0B0B61; border-bottom-style:solid; border-right-style:solid; border-right-color:#D8DADB; border-right-width:1px;'>" & _
                                       "<a href='frmResumenActividad.aspx?apr=" & dt.Rows(i).Item(j) & "&KeepThis=true&TB_iframe=true&height=450&width=550' title='Actividades' class='thickbox'>&nbsp;"
                                    strAct = strAct & "</a>" & "</td>"
                                End If
