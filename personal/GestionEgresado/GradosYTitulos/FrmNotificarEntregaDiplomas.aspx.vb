...
Partial Class GradosYTitulos_FrmNotificarEntregaDiplomas
    ...
    Function Validar() As Boolean
        If Me.cboSesion.SelectedValue = "" And Me.cboTipoDenominacion.SelectedValue = "" And Me.cboEstado.SelectedValue = "" Then
    ...
    Protected Sub btnBuscar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnBuscar.Click
        ...
        If Validar() = True Then
            ...
            Dim obj As New ClsGradosyTitulos
            Dim dt As New Data.DataTable
            dt = obj.ConsultarEntregaDiplomasEstado(Me.cboSesion.SelectedValue, Me.cboTipoDenominacion.SelectedValue, Me.txtBusqueda.Text, Me.cboEstado.SelectedValue)
    ...
    Sub EnviarEmail()
        Try
            ...
            dt.Columns.Add("egresado")
            dt.Columns.Add("codigo_Alu")
            ...
            For Each row As GridViewRow In Me.gvAlumnos.Rows
                ...
                If chckrw.Checked Then
                    ...
                    dataRow("denominacion") = Me.gvAlumnos.DataKeys(row.RowIndex).Values("descripcion_tdg").ToString
                    dataRow("codigos") = Me.gvAlumnos.DataKeys(row.RowIndex).Values("codigo_egr").ToString
                    dataRow("egresado") = Me.gvAlumnos.DataKeys(row.RowIndex).Values("egresado").ToString
                    dataRow("codigo_Alu") = Me.gvAlumnos.DataKeys(row.RowIndex).Values("codigo_Alu")
                    dataRow("correos") = Me.gvAlumnos.DataKeys(row.RowIndex).Values("email_Alu")
            ...
            If dt.Rows.Count > 0 Then
                ...
                For k As Integer = 0 To dt.Rows.Count - 1
                    ...
                    If ConfigurationManager.AppSettings("CorreoUsatActivo") = "1" Then
                        emailAlumno = dt.Rows(k).Item("correos")
                    Else
                    ...
                    If dt.Rows(k).Item("denominacion") = "BACHILLER" Then
                        ClsComunicacionInstitucional.EnviarNotificacionEmail(codigoEnvio, "GRAD", "GFED", "1", codigo_usu, "codigo_alu", dt.Rows(k).Item("codigo_Alu"), "47", emailAlumno, "", "", "", _
                        dt.Rows(k).Item("egresado"))
                    ElseIf dt.Rows(k).Item("denominacion") = "TÍTULO PROFESIONAL" Then
                        ClsComunicacionInstitucional.EnviarNotificacionEmail(codigoEnvio, "TITU", "TFED", "1", codigo_usu, "codigo_alu", dt.Rows(k).Item("codigo_Alu"), "47", emailAlumno, "", "", "", _
                      dt.Rows(k).Item("egresado"))
                    End If

                    ''-----------------Actualiza el campo Informoentregadiploma_egr de la  tabla egresado_GyT ------------------------------------------------------------------------------------------------------------------------------
                ...
                dtc = objGyt.ConsultarEntregaDiplomasEstado(Me.cboSesion.SelectedValue, Me.cboTipoDenominacion.SelectedValue, Me.txtBusqueda.Text, Me.cboEstado.SelectedValue)
        ...
        Catch ex As Exception
            Me.divMensaje.InnerHtml = ex.ToString()
            Me.divMensaje.Attributes.Add("Class", "alert alert-danger")
        End Try
