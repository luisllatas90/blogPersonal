    ...
    if (rpta == true) {
        ...
        var cellsrenderer = function(row, column, value) {
            ...
            if (dataRecord.entregado == 1) {
                if (dataRecord.estado_trl == 'E')  {
                    return '<button class="btn btn-success" title="Click para Retirar Entrega" disabled="true" onclick="ActualizarEntrega(\'' + dataRecord.cod + '\',0,\'' + dataRecord.cod_dta + '\');"><i class="ion-close-round"></i></button>';
                }
                else {
                ...
                if (dataRecord.estado_trl == "E") {
                    return '<button class="btn btn-warning" title="Click para Entregar" disabled="true" onclick="ActualizarEntrega(\'' + dataRecord.cod + '\',1,\'' + dataRecord.cod_dta + '\');"><i class="ion-checkmark"></i></button>';
                }
                else {
...
function ActualizarEntrega(cod, entregado,cod_dta) {
    ...
    /* cambio olluen 10/09/2020 */
    /*$('#FrmRegistro').append('<input type="hidden" id="action" name="action" value="' + ope.edip + '" />');*/
    $('#FrmRegistro').append('<input type="hidden" id="action" name="action" value="' + ope.entd + '" />');
    /*fin cambio olluen*/
    ...
    $('#FrmRegistro').append('<input type="hidden" id="cod_tfu" name="cod_tfu" value="178" />');
    var form = $("#FrmRegistro").serializeArray();
    console.log(form);
    $.ajax({
        type: "POST",
