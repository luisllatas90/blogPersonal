    ...
    if (rpta == true) {
        if (fnValidarBusqueda() == true) {
            ...
            var source =
            {
                ...
                root: 'rows',
                url: "../../DataJson/GradosyTitulos/Egresado.aspx",
                data: { action: operacion, hdscu: $("#cboSesion").val(), hdTest: $("#cboTipoEstudio").val(), hdCarrera: $("#cboCarrera").val(), txtbuscar: $("#txtbusqueda").val(), hdTipo: $("#cboTipoDenominacion").val(), estado: $("#cboEstado").val() }
...
function fnValidarBusqueda() {
    if ($("#cboEstado").val() == "") {
        $("#cboEstado").focus();
        fnMensaje("error", "Seleccione un estado.")
        return false;
    }
    if ($("#cboSesion").val() == "") {
        $("#cboSesion").focus();
