...
Partial Class operaciones
    ...
    Private Sub enviarEmailEvaluadorExterno()
        ...
        For i As Integer = 0 To tb.Rows.Count - 1
            ...
            mensaje = mensaje + "<a href= '../../../librerianet/EvaluadorExternoGI/FrmEvaluacionEvaluadorExterno.aspx?EVE=" + obj1.EncrytedString64(tb.Rows(i).Item("codigo_eve").ToString()) + "&CON=" + obj1.EncrytedString64(tb.Rows(i).Item("codigo_con").ToString()) + "' style='background-color:#F1132A;color:white;border-radius:10px;font-size:15px;padding:10px 10px;width:200px;height:20px;text-decoration:none;font-weight:200px;display:block;margin:0 auto;text-align:center' target='_blank'>Revisión de Postulaciones</a>"
    ...
    Sub SubirRubricaNew()
        Try
            ...
            list.Add("Fecha", Fecha)
            list.Add("Extencion", System.IO.Path.GetExtension(post.FileName))
            list.Add("Nombre", System.IO.Path.GetFileName(post.FileName.Replace("&", "_").Replace("'", "_").Replace("*", "_").Replace("á", "a").Replace("é", "e").Replace("í", "i").Replace("ó", "o").Replace("ú", "u").Replace("Á", "A").Replace("É", "E").Replace("Í", "I").Replace("Ó", "O").Replace("Ú", "U").Replace("Ñ", "N").Replace("ñ", "n")))
            list.Add("TransaccionId", codigo)
            list.Add("TablaId", "9")
    ...
    Sub ListarPostulacionesPorEvaluar()
        ...
        For i As Integer = 0 To tb.Rows.Count - 1
            ...
            Data.Add("cod_con", tb.Rows(i).Item("codigo_con"))
            Data.Add("tit_pos", tb.Rows(i).Item("titulo_pos"))
            Data.Add("fecha_eva", tb.Rows(i).Item("fecha_finEve"))
    ...
    Sub EnviarEmail()
        ...
        'receptor = "freddy.seclen@usat.edu.pe,freddy.seclen@usat.edu.pe"
        'receptor = "yperez@usat.edu.pe"
        receptor = email
    ...
    Sub ActualizarBonoPublicacion()
        ...
        obj.CadenaConexion = ConfigurationManager.ConnectionStrings("CNXBDUSAT").ConnectionString
        obj.AbrirConexion()
        tb = obj.TraerDataTable("INV_actualizarBonoPublicacion", Codigo_bon, Tipo_act, Codigo_use, Rechazo)
        obj.CerrarConexion()
    ...
    Sub RegistrarBonoPublicacion()
        ...
        If (Accion = "R") Then
            ...
            Dim Codigo_user As Integer = obj1.DecrytedString64(Request("hdUser"))
            tb = obj.TraerDataTable("INV_registrarBonoPublicacion", 0, titulo_bon, fecha_bon, BDrevista_bon, revista_bon, URLPubli_bon, Codigo_user, Codigo_user)
        Else
            ...
            Dim Codigo_user As Integer = obj1.DecrytedString64(Request("hdUser"))
            tb = obj.TraerDataTable("INV_registrarBonoPublicacion", codigo_bon, titulo_bon, fecha_bon, BDrevista_bon, revista_bon, URLPubli_bon, Codigo_user, Codigo_user)
        End If
        obj.CerrarConexion()
    ...
    Sub ListarRolInvestigador()
        ...
        Try
            ...
            obj.CadenaConexion = ConfigurationManager.ConnectionStrings("CNXBDUSAT").ConnectionString
            obj.AbrirConexion()
            tb = obj.TraerDataTable("INV_listarRolInvestigador")
            obj.CerrarConexion()
    ...
    Sub RegistrarInvestigador()
        ...
        Try
            ...
            obj.AbrirConexion()
            If (Accion = "R") Then
                tb = obj.TraerDataTable("INV_registrarInvestigador", Codigo_user, Url_dina, dina, regina, Codigo_user)
            Else
                If (Accion = "A") Then
                    tb = obj.TraerDataTable("INV_actualizarInvestigadorHistorial", Codigo_user, Url_dina, dina, regina, Codigo_user)
                End If
            End If
            obj.CerrarConexion()
    ...
    Private Sub ConsultarRolInvestigador()
        Dim serializer As System.Web.Script.Serialization.JavaScriptSerializer = New System.Web.Script.Serialization.JavaScriptSerializer()
        Dim JSONresult As String = ""
        Dim list As New List(Of Dictionary(Of String, Object))()
    ...
    Sub DescargarArchivo()
        Try
            ...
            obj.CadenaConexion = ConfigurationManager.ConnectionStrings("CNXBDUSAT").ConnectionString
            obj.AbrirConexion()
            tb = obj.TraerDataTable("dbo.USP_LISTARARCHIVOSCOMPARTIDOS", 2, 9, IdArchivo)
            obj.CerrarConexion()
    ...
    Sub DescargarArchivo3()
        Try
            'Dim usuario_session_ As String() = Session("perlogin").ToString.Split(New Char() {"\"c})
            Dim usuario_session As String = "USAT\esaavedra"
            Dim IdArchivo As String = Request("IdArchivo")
            ...
            list.Add("Usuario", usuario_session)
            Dim envelope As String = wsCloud.SoapEnvelopeDescarga(list)
            Dim result As String = wsCloud.PeticionRequestSoap("http://localhost/campusvirtual/ArchivosCompartidos/SharedFiles.asmx", envelope, "http://usat.edu.pe/DownloadFile", usuario_session)
            'Dim result As String = wsCloud.PeticionRequestSoap("http://localhost/campusvirtual/ArchivosCompartidos/SharedFiles.asmx", envelope, "http://usat.edu.pe/DownloadFile", usuario_session)
            Dim imagen As String = ResultFile(result)
            For i As Integer = 0 To tb.Rows.Count - 1
                Dim Data As New Dictionary(Of String, Object)()
