...
Partial Class logistica_Default
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Session("id_per") Is Nothing Then
            Response.Redirect("..\..\librerianet\ErrorSistema.aspx")
        End If

        If Not IsPostBack Then
            Dim log As New ClsLogistica
            Dim fun As New ClsFunciones
            fun.CargarListas(cboInstancia, log.ConsultarInstancias("TT"), "codigo_Ipl", "nombreInstancia_Ipl")
            '''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
            ...
            datos = objpre.ConsultarProcesoContable_V1() 'dflores
            ...
            'Hcano 30-11-16 : Cambio Lista de Centros de Costo
    ...
    Protected Sub gvPedidos_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles gvPedidos.SelectedIndexChanged
        Try
            ...
            'Inicia HCano 11-07-2019
            'If Me.gvPedidos.Rows(Me.gvPedidos.SelectedIndex).Cells(5).Text = "Eliminado" Then                
            '    Me.cmdEnviar.Visible = False
            ...
            '    If Me.gvPedidos.Rows(Me.gvPedidos.SelectedIndex).Cells(5).Text = "Rechazado" Then
            '        Me.cmdEnviar.Visible = False
            '    End If                
            'End If
            Dim estado As String = Me.gvPedidos.Rows(Me.gvPedidos.SelectedIndex).Cells(5).Text
            ...
            Else
                Me.cmdEnviar.Visible = False
                Me.cmdEliminar.Visible = False
                If estado = "Rechazado" Or estado = "Observado" Then
                    Me.cmdEliminar.Visible = True
                End If
                pnlDatos.Visible = False
                Me.cmdGuardar.Visible = False
            End If
            'fin HCano 11-07-2019
        Catch ex As Exception
            'Response.Write(ex.Message)

            ScriptManager1.RegisterStartupScript(Me, Me.GetType, "ErrorLoad", "alert('ocurrió un error al procesar los datos " & ex.Message & "')", True)
        End Try
    End Sub

    Protected Sub lnkDatos_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles lnkDatos.Click
        If Me.txtPedido.Text <> "" And Me.cboInstancia.SelectedValue = 1 Then
            ConsultarDatosPedido(Me.txtPedido.Text)
        End If
    End Sub
    Private Sub CargarDetalle()
        Dim objLog As New ClsLogistica
        gvDetallePedido.DataSource = objLog.ConsultarDetalle_V1(Me.txtPedido.Text)
    ...
    Protected Sub cmdEnviar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdEnviar.Click
        'ScriptManager1.RegisterStartupScript(Me, Me.GetType, "Alerta", "alert('zado');", True)
        'ScriptManager1.RegisterStartupScript(Me, Me.GetType, "DisableEnviar", "DisableEnviar()", True)
        Dim estado As String
        If Me.txtPedido.Text <> "" Then
            estado = Me.gvPedidos.SelectedRow.Cells(5).Text
            If gvDetallePedido.Rows.Count > 0 Then
                If estado = "Rechazado" Then
                    ScriptManager1.RegisterStartupScript(Me, Me.GetType, "Alerta", "alert('El pedido no se puede enviar porque se ecuentra como: Rechazado');", True)
                Else
                    Dim log As New ClsLogistica
                    ...
                    ScriptManager1.RegisterStartupScript(Me, Me.GetType, "siguientepagina", "location.href='frmMonitorPedidosSolicitante_V1.aspx?id=" & Request.QueryString("id") & "';", True)
            ...
            End If
        Else
            ScriptManager1.RegisterStartupScript(Me, Me.GetType, "Alerta", "alert('Seleccione un pedido de la lista para enviar');", True)
        End If
        Me.gvPedidos.SelectedIndex = -1
    End Sub
    Private Sub LimpiarDetalle()
        Me.cboProgramaPresu.SelectedIndex = -1
        Me.cboDetalleActividadPOA.SelectedIndex = -1
        txtConcepto.Text = ""
        txtCodItem.Text = ""
        txtPrecioUnit.Text = ""
        txtCantidad.Text = ""
    ...
    Protected Sub gvDetallePedido_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gvDetallePedido.RowDataBound
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            End If
            'ScriptManager1.RegisterStartupScript(Me, Me.GetType, "Alerta", "alert('" & Me.gvPedidos.SelectedRow.Cells(5).Text & "');", True)

            'If Me.gvPedidos.SelectedRow.Cells(5).Text = "Registrado" Then
            e.Row.Cells(11).Attributes.Add("OnClick", "javascript:ModalAdjuntar2('" & objL.EncrytedString64(DataBinder.Eval(e.Row.DataItem, "codigo_dpe").ToString()) & "','" & objL.EncrytedString64(Me.txtPedido.Text.ToString()) & "','" & Me.gvPedidos.SelectedRow.Cells(5).Text & "')")
            'Dim a As String = DataBinder.Eval(e.Row.DataItem, "codigo_dpe").ToString()
    ...
    Protected Sub gvDetallePedido_RowEditing(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewEditEventArgs) Handles gvDetallePedido.RowEditing
        ...
        Try
            ...
            If estado = "Rechazado" Then
                ScriptManager1.RegisterStartupScript(Me, Me.GetType, "Alerta", "alert('El item no se puede editar porque el Pedido se ecuentra como: Rechazado');", True)
            Else
                ...
                Me.cboDetalleActividadPOA.SelectedValue = Me.gvDetallePedido.DataKeys.Item(e.NewEditIndex).Values(2)
                ...
                spAdjunto.Visible = False
                If substrings.Length > 1 Then ' verifica si tiene archivo adjunto
                ...
                If Me.gvDetallePedido.DataKeys.Item(e.NewEditIndex).Values(3) = True Then
    ...
    Private Sub EliminarItemDetalle(ByVal codigo_dpe As Integer)
        ...
        If rpta = 2 Then
            ScriptManager1.RegisterStartupScript(Me, Me.GetType, "Alerta", "alert('No se puede eliminar el Ítem ya que posee una Solicitud de Anticipo relacionada.');", True)
        End If
    ...
    Protected Sub cmdGuardar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdGuardar.Click
        ...
        Dim ruta As String
        Try
            'Response.Write("<script>alert('" & Me.txtDetPresup.Text & "')</script>")
            'Response.Write("<script>alert('" & Me.txtCodigoDpe.Text & "')</script>")
            ...
            '        ScriptManager1.RegisterStartupScript(Me.GetType, "CorregirDatos", "alert('El PRECIO X CANTIDAD a SOLICITAR no debe superar lo DISPONIBLE presupuestado: S/. " & Me.hdDisponible.Value.ToString & "')", True)
            '        Exit Sub
            '    End If
            'End If

            If codDetPresup > 0 Then
                Dim objPresu As New ClsPresupuesto
                Dim dt As New Data.DataTable
                dt = objPresu.ValidaMontoPresupuestadoDetalle(codDetPresup, CInt(Me.txtCodigoDpe.Text))
                If dt.Rows.Count > 0 Then
                    If (CDbl(Me.txtPrecioUnit.Text) * CDbl(Me.txtCantidad.Text)) > CDbl(dt.Rows(0).Item("disponible")) Then
                        ScriptManager1.RegisterStartupScript(Me, Me.GetType, "CorregirDatos", "alert('El PRECIO X CANTIDAD a SOLICITAR no debe superar lo DISPONIBLE presupuestado: S/. " & dt.Rows(0).Item("disponible").ToString & "')", True)
                        Exit Sub
                    End If
                End If
            End If
            ...
            If Me.cboProgramaPresu.SelectedValue = -1 Then
                cboProgramaPresu.SelectedValue = 1
            End If

            Dim codigo_Dpe As Integer
            If Me.txtCodigoDpe.Text = "" Then
                codigo_Dpe = objLog.AgregarDetallePedido_VPOA(Me.txtPedido.Text, txtCodItem.Text, cboCecos.SelectedValue, Me.txtPrecioUnit.Text, Me.txtCantidad.Text, Me.txtComentarioReq.Text, Me.TxtFechaEsperada.Text, 1, "S", rblModoDistribucion.SelectedValue, Me.cboProgramaPresu.SelectedValue, codDetPresup, Me.txtEspecificaciones.Text, Me.cboDetalleActividadPOA.SelectedValue)

                If codigo_Dpe <> 0 Then
                    ScriptManager1.RegisterStartupScript(Me, Me.GetType, "FaltanDatosCco", "SubirArchivo2('" & objLog.EncrytedString64(codigo_Dpe) & "','" & objLog.EncrytedString64(Me.txtPedido.Text) & "')", True)
                End If
            Else
                objLog.ActualizarDetallePedido_VPOA(CInt(Me.txtPedido.Text), CInt(txtCodItem.Text), CInt(cboCecos.SelectedValue), CDbl(Me.txtPrecioUnit.Text), CDbl(Me.txtCantidad.Text), Me.txtComentarioReq.Text, Me.TxtFechaEsperada.Text, 1, "S", rblModoDistribucion.SelectedValue, CInt(Me.cboProgramaPresu.SelectedValue), CInt(codDetPresup), CInt(Me.txtCodigoDpe.Text), Me.txtEspecificaciones.Text, Me.cboDetalleActividadPOA.SelectedValue)
            End If
            'Hcano
        ...
        Catch ex As Exception
            ...
            ScriptManager1.RegisterStartupScript(Me, Me.GetType, "error", "alert('ocurrió un error al procesar los datos')", True)
        End Try
    End Sub
    ...
    Protected Sub cmdEliminar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdEliminar.Click
        Try
            If Me.txtPedido.Text <> "" Then
                Dim log As New ClsLogistica
                log.ElmininarPedido(Me.txtPedido.Text, Session("id_per"))
                ScriptManager1.RegisterStartupScript(Me, Me.GetType, "Alerta", "alert('Se eliminó el pedido satisfactoriamente');", True)
                ScriptManager1.RegisterStartupScript(Me, Me.GetType, "siguientepagina", "location.href='frmMonitorPedidosSolicitante_V1.aspx?id=" & Request.QueryString("id") & "';", True)
            Else
                ScriptManager1.RegisterStartupScript(Me, Me.GetType, "Alerta2", "alert('Seleccione un pedido de la lista para eliminar');", True)
            End If
        Catch ex As Exception
            ScriptManager1.RegisterStartupScript(Me, Me.GetType, "Error", "alert('" & ex.Message.ToString & "');", True)
        End Try
        Me.gvPedidos.SelectedIndex = -1
    End Sub
    ...
    '            ScriptManager1.RegisterStartupScript(Me,Me.GetType, "Alerta", "alert('Se duplicó el pedido satisfactoriamente');", True)
    '            ScriptManager1.RegisterStartupScript(Me,Me.GetType, "siguientepagina", "location.href='frmMonitorPedidosSolicitante.aspx?id=" & Request.QueryString("id") & "';", True)
    '        Else
    '            ScriptManager1.RegisterStartupScript(Me,Me.GetType, "Alerta", "alert('Seleccione un pedido de la lista para duplicar');", True)
    '        End If
    '    Catch ex As Exception
    '        ScriptManager1.RegisterStartupScript(Me,Me.GetType, "Alerta", "alert('Error: " & ex.Message & "');", True)
    '    End Try        
    'End Sub

    Sub Inhabilitar_registro()
        'Me.txtDetPresup.Text = ""
        'Me.txtCodigoDpe.Text = ""
        Me.chkNoPresupuestado.Checked = False
        Me.txtConcepto.Enabled = False
        Me.txtConcepto.Text = ""
        Me.cboDetalleActividadPOA.Enabled = False
        ...
        Me.File1.Disabled = True
    End Sub

    Sub Habilitar_registro()
        Me.txtConcepto.Enabled = True
        Me.cboDetalleActividadPOA.Enabled = True
    ...
    Protected Sub chkNoPresupuestado_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles chkNoPresupuestado.CheckedChanged
        If Me.chkNoPresupuestado.Checked = True Then
            ...
            Me.LinkButton3.Visible = False
            Me.TxtFechaEsperada.Enabled = False
            Me.TxtFechaEsperada.Text = Date.Now.ToString("dd/MM/yyyy")
            Me.txtComentarioReq.Enabled = False
            Me.txtComentarioReq.Text = ""
            Me.txtEspecificaciones.Enabled = False
            Me.txtEspecificaciones.Text = ""
            'Me.fuCargarArchivo.Enabled = False
            spAdjunto.Visible = True
            Habilitar_registro()
        Else
            Inhabilitar_registro()
        End If
    ...
    '    Dim codigo_Dren As String = "164563"
    '    Dim NroRend As String = "96969"
    '    Dim Fecha As String = "10-04-2017"
    '    Dim objA As New ClsArchivosCompartidos
    '    Dim list As New Dictionary(Of String, String)

    '    list.Add("Fecha", Fecha)
    '    list.Add("Extencion", ".jpg")
    '    list.Add("Nombre", "a.jpg")
    '    list.Add("TransaccionId", codigo_Dren)
    '    list.Add("TablaId", "2")
    '    list.Add("NroOperacion", NroRend)
    '    list.Add("Archivo", ConvertFileToBase64("c:\a.jpg"))
    '    list.Add("Usuario", Usuario)
    '    list.Add("Equipo", "DSISTEMAS09")
    '    list.Add("Ip", "10.10.1.5")
    '    list.Add("param8", Usuario)

    '    Dim envelope As String = objA.SoapEnvelope(list)
    '    Dim result As String = objA.PeticionRequestSoap("http://serverdev/campusvirtual/ArchivosCompartidos/SharedFiles.asmx", envelope, "http://usat.edu.pe/UploadFile", Session("perlogin").ToString)
