...
Partial Class logistica_Default
        ...
        If Me.txtPedido.Text <> "" Then
            ...
            If gvDetallePedido.Rows.Count > 0 Then
                ...
                Else
                    ...
                    ClientScript.RegisterStartupScript(Me.GetType, "siguientepagina", "location.href='frmMonitorPedidosSolicitante_V1.aspx?id=" & Request.QueryString("id") & "';", True)
                End If
            Else
            ...
            If codDetPresup > 0 Then
                ...
                If dt.Rows.Count > 0 Then
                    If (CDbl(Me.txtPrecioUnit.Text) * CDbl(Me.txtCantidad.Text)) > CDbl(dt.Rows(0).Item("disponible")) Then
                        ClientScript.RegisterStartupScript(Me.GetType, "CorregirDatos", "alert('El PRECIO X CANTIDAD a SOLICITAR no debe superar lo DISPONIBLE presupuestado: S/. " & dt.Rows(0).Item("disponible").ToString & "')", True)
                        Exit Sub
                    End If
                End If
            End If
    ...
    Sub Habilitar_registro()
        ...
        Me.fuCargarArchivo.Enabled = True
    End Sub
    ...
    Protected Sub Button1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Button1.Click
        Try
            Dim usuario_session_ As String()
            usuario_session_ = Session("perlogin").ToString.Split(New Char() {"\"c})
            ...
            list.Add("param8", Session("perlogin"))
            Dim envelope As String = objA.SoapEnvelope(list)

            'Response.Write(envelope)
            Dim result As String = objA.PeticionRequestSoap("http://serverdev/campusvirtual/ArchivosCompartidos/SharedFiles.asmx", envelope, "http://usat.edu.pe/UploadFile", Session("perlogin"))
