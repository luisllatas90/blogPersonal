...
Partial Class personal_academico_estudiante_historial
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            If Tbl.Rows.Count > 0 Then
                ...
                '    Request.QueryString("ctf") <> 19 And Request.QueryString("ctf") <> 20 And _
                '    Request.QueryString("ctf") <> 90 Then
                '    If (Request.QueryString("param") = 1) Then
                '        CargaCiclosMatriculados(codigo_alu)
                '    Else
    ...
    Private Sub CargaCiclosMatriculados(ByVal codigo_alu As Integer)
        Dim Obj As New ClsConectarDatos
        Obj.CadenaConexion = ConfigurationManager.ConnectionStrings("cnxBDUSAT").ConnectionString
        Try
            ...
            End If
        Catch ex As Exception
            Me.lblMensaje.Text = "Error al cargar ciclos: " & ex.Message
        End Try                
    End Sub
    ...
    Protected Sub dlstCiclos_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataListItemEventArgs) Handles dlstCiclos.ItemDataBound
        ...
        If e.Item.DataItem("tipo_cac").ToString = "N" Then
            ...
            'Response.Write(e.Item.DataItem("descripcion_cac").ToString & " " & e.Item.DataItem("codigo_sco") & "<br>")
            If e.Item.DataItem("codigo_sco") = 103 Then 'Reserva: NO HA ESTUDIADO EN EL CICLO Y PAGÓ
                ...
                CType(e.Item.FindControl("lblciclo"), Label).Text = e.Item.DataItem("descripcion_cac") & " - RESERVA DE MATRICULA (" & e.Item.DataItem("estadodeuda").ToString & ")"
                EsReserva = True
        ...
        If EsReserva = False Then
            ...
            Obj.CadenaConexion = ConfigurationManager.ConnectionStrings("cnxBDUSAT").ConnectionString
            Obj.AbrirConexion()
            gr.DataSource = obj.TraerDataTable("ConsultarHistorialAlumnoReservas", 2, Request.QueryString("id"), Me.dlstCiclos.DataKeys(e.Item.ItemIndex))
            Obj.CerrarConexion()
            gr.DataBind()
            Obj = Nothing
        End If
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            Else
                If fila.Item("inhabilitado_dma") = True Then
                    'yperez 23.07.19
                    e.Row.Cells(7).Text = "-" 'nota

                    e.Row.Cells(8).Text = "Inhabilitado"
                End If
            End If
            ...
            '*******************************************  
            '*******************************************
            ' Edgard: fila.Item("codigo_Pes") <> 593: se agrego para no tomnar en cuenta en promedios cursos con plan de estudio PLAN DE NIVELACIÓN DE COMPETENCIAS
            '*******************************************
            If fila.Item("estado_dma") <> "R" And fila.Item("codigo_Pes") <> 593 And fila.Item("inhabilitado_dma") = False And _
            ...
            ' Edgard: fila.Item("codigo_Pes") <> 593: se agrego para no tomnar en cuenta en promedios cursos con plan de estudio PLAN DE NIVELACIÓN DE COMPETENCIAS
            '*******************************************
            If fila.Item("condicion_dma") = "A" And fila.Item("codigo_Pes") <> 593 And fila.Item("inhabilitado_dma") = False Then
                ...
                ElseIf fila.Item("tipomatricula_dma") = "C" And fila.Item("estado_dma") <> "R" And fila.Item("inhabilitado_dma") = False Then
