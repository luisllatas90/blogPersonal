...
#Region "Metodos"
    ...
    Private Function TablaManifiesto() As PdfPTable
        ...
        Dim p5 As New Paragraph("La carrera se abrirá sólo si se cumple con un número mínimo de estudiantes matriculados. Para el cumplimiento de esta condición, el número mínimo equivale al número de vacantes abiertas en la carrera profesional. En caso no se llegue a éste, se cerrará la convocatoria a la carrera profesional y se les invitará a los ingresantes a migrar a otra carrera o a realizar el trámite correspondiente para la devolución de su dinero.", mo_Font) 'andy.diaz 18/08/2020
        ...
        'andy.diaz 14/10/2020
        Dim p11 As New Paragraph("El postulante, sus padres y/o tutor garantizan su conexión a internet para rendir el examen de admisión de la Universidad, aceptando que:", mo_Font)
        p11.SetLeading(yLeading, 0)
        p11.SpacingAfter = ySpacing

        Dim p111 As New Paragraph("Si el postulante no mantiene encendida su cámara web durante el control de asistencia o durante el tiempo que dura la evaluación o", mo_Font)
        p111.SetLeading(yLeading, 0)
        p111.SpacingAfter = ySpacing

        Dim p112 As New Paragraph("Si se le detecta copiando o consultando las respuestas del examen a terceros.", mo_Font)
        p112.SetLeading(yLeading, 0)
        p112.SpacingAfter = ySpacing

        Dim p11p As New Paragraph("La Universidad podrá inhabilitarlos del examen, publicando su condición como no ingresante, así haya obtenido una nota aprobatoria. Siendo esta decisión inimpugnable y no corresponderá ni la devolución del pago realizado ni la reprogramación del examen.", mo_Font)
        p11p.SetLeading(yLeading, 0)
        p11p.SpacingAfter = ySpacing

        Dim uoList11 As New iTextSharp.text.List
        uoList11.SetListSymbol("o   ")
        uoList11.IndentationLeft = 10
        uoList11.Add(New ListItem(p111))
        uoList11.Add(New ListItem(p112))

        Dim li11p As New ListItem(p11p)
        li11p.ListSymbol = New Chunk("    ")
        '/andy.diaz 14/10/2020
    ...
    Public Shared Function TablaFirma() As PdfPTable
        ...
        Dim cellFirma As New PdfPCell(New Phrase("Firma del Apoderado", lo_FontSubTitulo)) 'andy.diaz 09/09/2020
        With cellFirma
            .Colspan = 100
            .HorizontalAlignment = PdfPCell.ALIGN_CENTER
