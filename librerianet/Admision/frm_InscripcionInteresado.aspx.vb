...
Partial Class frm_InscripcionInteresado
...
#Region "Eventos"
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            If Not IsPostBack Then
                ...
                hddTipo.Value = IIf(Request.Form("tipo") Is Nothing, "0", Request.Form("tipo"))
                ...
                hddFechaHoraLimiteInscripcion.Value = IIf(Request.Form("fechaHoraLimiteInscripcion") Is Nothing, "", Request.Form("fechaHoraLimiteInscripcion"))
                ...
                hddTextoBtnInscripcion.Value = IIf(Request.Form("textoBtnInscripcion") Is Nothing, "", Request.Form("textoBtnInscripcion"))
                ...
                Dim ls_MsgConfInscripcion As String = IIf(Request.Form("msgConfInscripcion") Is Nothing, "", Request.Form("msgConfInscripcion"))
                ls_MsgConfInscripcion = DesencriptaTexto(ls_MsgConfInscripcion)

                Dim ls_MsgExitoInscripcion As String = IIf(Request.Form("msgExitoInscripcion") Is Nothing, "", Request.Form("msgExitoInscripcion"))
                ViewState("msgExitoInscripcion") = ls_MsgExitoInscripcion 'Mantengo el mensaje encriptado en el estado sino da error, lo desencripto cuando lo uso
                ...
                hddInscripcionCampus.Value = IIf(Request.Form("inscripcionCampus") Is Nothing, "0", Request.Form("inscripcionCampus"))

                If hddInscripcionCampus.Value = "1" Then
                    frmInscripcionInteresado.Attributes.Item("style") = "background: #f9b457"
                End If

                hTitulo.InnerHtml = hddTitulo.Value
                btnFakeInformarme.InnerHtml = hddTextoBtnInformacion.Value
                btnFakeInscribirme.InnerHtml = hddTextoBtnInscripcion.Value
                mensajeConfirmacion.InnerHtml = ls_MsgConfInscripcion
                ...
                lr_MostrarOcultarBotonInscripcion()
                lr_CargarCombos()
            Else
    ...
    Protected Sub cmbAnioEstudio_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbAnioEstudio.SelectedIndexChanged
        lr_MostrarOcultarBotonInscripcion()
        'lr_ObtenerCalculosPension(cmbInstitucionEducativa.SelectedValue, cmbCarreraProfesional.SelectedValue)
        'udpCostos.Update()
    End Sub
...
#Region "Metodos"
    ...
    Private Sub lr_MostrarOcultarBotonInscripcion()
        Dim lb_MostrarBotonInscripcion As Boolean = hddTipo.Value = "1" AndAlso (cmbAnioEstudio.SelectedValue = "Q" OrElse cmbAnioEstudio.SelectedValue = "E")

        'Si ya venció la fecha límite de inscripción, oculto el botón correspondiente
        If IsDate(hddFechaHoraLimiteInscripcion.Value) Then
            Dim ld_FechaHoraLimiteInscripcion As DateTime = DateTime.Parse(hddFechaHoraLimiteInscripcion.Value)
            If ld_FechaHoraLimiteInscripcion < DateTime.Now Then
                lb_MostrarBotonInscripcion = False
            End If
        End If

        divRequisitosAdmision.Visible = lb_MostrarBotonInscripcion
        udpConsideracionesAdmision.Update()

        btnInscribirme.Enabled = lb_MostrarBotonInscripcion
        btnFakeInscribirme.Visible = lb_MostrarBotonInscripcion
        udpBotonesSubmit.Update()
    End Sub
    ...
    Private Sub lr_ObtenerCalculosPension(ByVal ls_codigoIed As String, ByVal ls_codigoCpf As String)
        If ls_codigoIed <> "-1" AndAlso ls_codigoCpf <> "-1" AndAlso (cmbAnioEstudio.SelectedValue = "E" OrElse cmbAnioEstudio.SelectedValue = "5") Then
            Dim lo_Datos As New Dictionary(Of String, String) : lo_Datos.Add("codigoIed", ls_codigoIed) : lo_Datos.Add("codigoCpf", ls_codigoCpf)
            Dim lo_RespuestaSOAP As New XmlDocument : lo_RespuestaSOAP.LoadXml(mo_SOAP.lr_RealizarPeticionSOAP(ObtenerVariableGlobal("servicioUrl"), "CalcularCategoriaEstudiante", lo_Datos))

            Dim lo_Namespace As XmlNamespaceManager = New XmlNamespaceManager(lo_RespuestaSOAP.NameTable) : lo_Namespace.AddNamespace("ns", "http://tempuri.org/")
            Dim ls_RutaNodos As String = "//ns:CalcularCategoriaEstudianteResponse/ns:CalcularCategoriaEstudianteResult"
            Dim lst_Nodo As XmlNodeList = lo_RespuestaSOAP.DocumentElement.SelectNodes(ls_RutaNodos, lo_Namespace)

            For Each _Nodo As XmlNode In lst_Nodo
                spnCostoMes.InnerText = "S/" & Decimal.Parse(_Nodo.Item("costoMes").InnerText).ToString("N2")
                spnCostoCiclo.InnerText = "S/" & Decimal.Parse(_Nodo.Item("costoCiclo").InnerText).ToString("N2")
            Next
    ...
    Private Sub lr_RegistrarInteresado()
        Try
            Dim lo_ValidacionFormulario As Dictionary(Of String, String) = lf_ValidarFormulario(False)
            ...
            Else
                Dim corigoOri As Integer
                If hddInscripcionCampus.Value = "1" Then
                    corigoOri = ObtenerVariableGlobal("codigoOrigenCampus")
                Else
                    corigoOri = ObtenerVariableGlobal("codigoOrigenWeb")
                End If
                ...
                Dim ls_NombreNodo As String = ""
                If hddInscripcionCampus.Value = "1" Then
                    ls_NombreNodo = "GuardarInteresadoCampus"
                Else
                    ls_NombreNodo = "GuardarInteresado"
                End If
    ...
    Private Sub lr_InscribirInteresado()
        Try
            Dim lo_ValidacionFormulario As Dictionary(Of String, String) = lf_ValidarFormulario(True)
            If lo_ValidacionFormulario.Item("rpta") <> "1" Then
                lr_GenerarMensajesValidacion(lo_ValidacionFormulario.Item("rpta"), lo_ValidacionFormulario.Item("msg"), lo_ValidacionFormulario.Item("control"))
            Else
                Dim corigoOri As Integer
                ...
                With lo_Datos
                    ...
                    .Add("direccionDin", UCase(txtDireccion.Text.Trim))
                    .Add("tokenCco", hddTokenCco.Value)
                    .Add("codigoCac", "0") 'El procedimiento busca el codigo_cac
                    .Add("codigoMin", hddCodigoMin.Value)
                    .Add("codigoTest", mn_codigoTest)
                    ...
                    .Add("codigoTpar", "0") 'Este valor se utiliza en educación continua
                    .Add("numerodocFin", UCase(txtDNIApoderado.Text.Trim))
                    .Add("apepaternoFin", UCase(txtApePatApoderado.Text.Trim))
                    .Add("apematernoFin", UCase(txtApeMatApoderado.Text.Trim))
                    .Add("nombresFin", UCase(txtNombresApoderado.Text.Trim))
                    ...
                    .Add("emailFin", UCase(txtEmailApoderado.Text.Trim))
                    .Add("consulta", UCase(txtConsultas.Text.Trim))
                    .Add("validaDeuda", "1") 'Valida si el alumno tiene deudas pendientes
                    .Add("generaCargo", True) 'Genera el cargo (deuda) automáticamente
                    .Add("usuarioReg", mn_Usuario)
                    .Add("accion", "N") 'Accion "Normal"?
                End With

                Dim lo_RespuestaSOAP As New XmlDocument : lo_RespuestaSOAP.LoadXml(mo_SOAP.lr_RealizarPeticionSOAP(ObtenerVariableGlobal("servicioUrl"), "InscribirInteresado", lo_Datos))
                ...
                Select Case lo_Dict.Item("rpta")
                    Case "-1"
                        ls_Mensaje = "No se pudo registrar la información"
                    Case "1"
                        ls_Mensaje = DesencriptaTexto(ViewState("msgExitoInscripcion"))
    ...
    Private Function lf_ValidarFormulario(ByVal lb_Inscripcion As Boolean) As Dictionary(Of String, String)
        Try
            ...
            If hddTipo.Value = "1" Then
                ...
                If lb_Inscripcion Then
                    'Verifico si el formulario está habilitado
                    If Not String.IsNullOrEmpty(hddFechaHoraLimiteInscripcion.Value) Then
                        Dim ld_FechaHoraLimiteInscripcion As DateTime = DateTime.Parse(hddFechaHoraLimiteInscripcion.Value)

                        If ld_FechaHoraLimiteInscripcion < DateTime.Now Then
                            lo_Resultado.Item("rpta") = "-1"
                            lo_Resultado.Item("msg") = "Se ha superado la fecha límite para realizar la inscripción."
                            Return lo_Resultado
                        End If
                    End If

                    'Verifico que el postulante esté en 5to grado o sea egresado
                    If cmbAnioEstudio.SelectedValue <> "Q" AndAlso cmbAnioEstudio.SelectedValue <> "E" Then
                        lo_Resultado.Item("rpta") = "-1"
                        lo_Resultado.Item("msg") = "El año de estudios seleccionado no permite realizar una inscripción"
                        Return lo_Resultado
                    End If

                    If Not chkConsideracionesAdmision.Checked Then
                        lo_Resultado.Item("rpta") = "0"
                        lo_Resultado.Item("msg") = "Debe aceptar las consideraciones del proceso de admisión"
                        lo_Resultado.Item("control") = "chkConsideracionesAdmision"
                        Return lo_Resultado
                    End If
                End If
            End If
    ...
    Private Sub lr_LimpiarFormulario()
        ...
        cmbAnioEstudio.SelectedValue = "-1" : cmbAnioEstudio_SelectedIndexChanged(Nothing, Nothing)
        'Datos laborales
