...
'----------------------------------------------------------------------
Partial Class frmTestRevisaHorario
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            If ddlPersonal.SelectedIndex > 0 Then
    ...
    Private Sub ListaPeriodoLaborable()
        Dim obj As New clsPersonal
        Dim dts As New Data.DataTable
        Try
            dts = obj.ListaPeriodoLaboral(0, "")
            If dts.Rows.Count > 0 Then
                cboPeriodo.DataSource = dts
                cboPeriodo.DataTextField = "descripcion_Pel"
                cboPeriodo.DataValueField = "codigo_Pel"
                cboPeriodo.DataBind()
    ...
    Private Sub cargaPersonal()
        ...
        idPer = Request.QueryString("id") 'del usuario que inicio sesion
        ' idPer = 684
        ...
        'Response.Write("<br />")
        'Response.Write(estado_hop)

        dts = obj.ConsultarPersonalDirectorDepartamento_v4(idPer, Request.QueryString("ctf"), estado_hop)
    ...
    'AGREGADO EL 02/12/2011 dguevara
    ...
    Protected Sub ddlTipoActividad_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlTipoActividad.SelectedIndexChanged
        ...
        If ddlTipoActividad.SelectedIndex <> -1 Then
            ...
            ElseIf ddlTipoActividad.SelectedValue = 5 Or ddlTipoActividad.SelectedValue = 24 Then  '5 Investigacion   24 Gestión en Investigación  --15/04/2019 MNeciosup
                CargarDepartamento()
                HabilitarEsFacuDep()
                Call MostrarOpciones(False)
        ...
        Try
            ...
            If ddlTipoActividad.SelectedValue = 7 Then
                ...
                If txtObservacion.Text = "" Then
                    vMensaje = vMensaje & "Especificar el centro de prácticas. "
                    Dim myscript As String = "alert('" & vMensaje.Trim & "')"
                    Page.ClientScript.RegisterStartupScript(Me.GetType(), "myscript", myscript, True)
                    Exit Sub
                End If
            ...
            'Ojo: es solo para informar al usuario, la insercion en la bd ya se realizo                

            If (obj.VerificarLimiteHoras("", ddlHoraInicio.Text, ddlHoraFin.Text, codigo_per, 0, abreviatura_td, 0, 0, "", "", 0, 0, 0, txtObservacion.Text.Trim, 0, "n")) = True Then
            ...
            If vMensaje = "" Then
                ...
                mensajeAlerta = obj.ValidarCrucesFinal(codigo_per, codigo_pel, ddlSemana.SelectedValue, ddlHoraInicio.Text, ddlHoraFin.Text, ddlDia.SelectedValue, abreviatura, cc, Me.txtObservacion.Text.Trim.ToString)
    ...
    Protected Sub btnEnviar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnEnviar.Click
        Try
            ...
            If ddlPersonal.SelectedValue = 0 Then
                lblMensaje.Text = "Por favor seleccione un trabajador."
                Exit Sub
            End If

            If ddlEstadoHorario.SelectedValue = "CALIFICAR" Then
                lblMensaje.Text = "Por favor seleccione una opción para calificar el horario."
                Exit Sub
            End If
            ...
            If ddlEstadoHorario.SelectedValue = "O" Then
                If txtObservacionHorario.Text = "" And ddlEstadoHorario.SelectedValue = "O" Then
                    lblMensaje.Text = "Por favor ingrese un comentario sobre el horario observado."
                    lblObservacionHorario.Focus()
                    Exit Sub
                End If
                ...
                obj.ObservarHorario(codigo_per, txtObservacionHorario.Text, Me.lblNombre.Text, txtId.Text, ddlEstadoHorario.SelectedValue, Request.QueryString("id"))
            ...
            If ddlEstadoHorario.SelectedValue = "C" Then
                ...
                '                lblMensaje.Text = "La carga horaria del día Lunes excede el límite permitido (10 horas), por favor verifique."
                '                lblMensaje.Visible = True
                '                Exit Sub
                '            End If
                '        Case 2
                ...
                '            If horasdiarias > 12 Then
                '                lblMensaje.Text = "La carga horaria del día Martes excede el límite permitido (10 horas), por favor verifique."
                '                lblMensaje.Visible = True
                '                Exit Sub
                '            End If
                '        Case 3
                ...
                '            If horasdiarias > 12 Then
                '                lblMensaje.Text = "La carga horaria del día Miércoles excede el límite permitido (10 horas), por favor verifique."
                '                lblMensaje.Visible = True
                '                Exit Sub
                '            End If
                '        Case 4
                ...
                '            If horasdiarias > 12 Then
                '                lblMensaje.Text = "La carga horaria del día Jueves excede el límite permitido (10 horas), por favor verifique."
                '                lblMensaje.Visible = True
                '                Exit Sub
                '            End If
                '        Case 5
                ...
                '            If horasdiarias > 12 Then
                '                lblMensaje.Text = "La carga horaria del día Viernes excede el límite permitido (10 horas), por favor verifique."
                '                lblMensaje.Visible = True
                '                Exit Sub
                '            End If
                '        Case 6
                ...
                '            If horasdiarias > 12 Then
                '                lblMensaje.Text = "La carga horaria del día Sábado excede el límite permitido (10 horas), por favor verifique."
                '                lblMensaje.Visible = True
                '                Exit Sub
                '            End If
                '    End Select
                    ...
                    Select Case j
                        Case 1
                            ...
                            If horasdiarias > 12 Then
                                ...
                                Exit Sub
                            End If
                            'If horasdiarias >= 8 Then    ''MNeciosup 24/06/2019
                ...
                Else
                    ...
                    'Envia Horario.   
                    'Registra una observacion vacia:
    ...
    Private Sub ConsultarListaCambiosHorarios()
        Try
            Dim obj As New clsPersonal
            Dim dts As New Data.DataTable
            If (Me.cboPeriodo.Items.Count > 0) Then
                dts = obj.ConsultarListaCambiosHorarios(codigo_per, Me.cboPeriodo.SelectedValue)
            Else
    ...
    Protected Sub ddlPersonal_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlPersonal.SelectedIndexChanged
        Try
            ...
            '##########  agregado  ###############################
            If lblTotalHorasLectivas.Text <> 0 Then
                Hr.Visible = True
                ModificarHrsLectivas(Me.ddlSemana.SelectedValue)
            Else
                Hr.Visible = False
    ...
    Protected Sub gvListaCambios_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gvListaCambios.RowDataBound
        Try
            If e.Row.RowType = DataControlRowType.DataRow Then
                ...
                Else
                    e.Row.Cells(5).Text = "<a href='frmVerHorarios.aspx?codigo_pel=" & Me.cboPeriodo.SelectedValue & "&codigo_per=" & codigo_per & "&registradopor=" & registradopor & "&fecha=" & fecha & "&hora=" & hora & "&mes_vigente=" & mes_vigente & "&KeepThis=true&TB_iframe=true&height=600&width=750&modal=true' title='Ver horarios' class='thickbox'><img src='../../images/resultados.gif' border=0 /><a/>"
                End If
    ...
    Protected Sub cboPeriodo_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cboPeriodo.SelectedIndexChanged
        ConsultarListaCambiosHorarios()
    End Sub
