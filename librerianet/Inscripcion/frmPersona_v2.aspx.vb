...
Partial Class Inscripcion_frmPersona
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            If IsPostBack = False Then                
                ...
                obj.AbrirConexion()
                'Cargar paises excepto Peru = 156
                ...
                If (Request.QueryString("token") <> Nothing) Then
                    ...
                    If (dtPreIns.Rows.Count > 0) Then
                        If (dtPreIns.Rows(0).Item("estado_pins") = "S") Then
                            ...
                            Me.hdcodigo_cco.Value = dtPreIns.Rows(0).Item("codigo_cco") 'lo descomenté. Se usa para buscar datos del evento, y del preinscrito, en CargaDatosEvento("P")
                ...
                If (Request.Form("Evento") IsNot Nothing) Then
                    'If (1 = 1) Then
                    Try
                        ...
                        'strToken = token
                        strToken = Request.Form("Evento").ToString
                        ...
                        Me.lblHeader.Text = "Evento: " & dtEvento.Rows(0).Item("descripcion_Cco")
                        obj.CerrarConexion()

                        Me.lblmensaje.Text = "Test:" & dtEvento.Rows(0).Item("codigo_test")
                        If (dtEvento.Rows(0).Item("codigo_test") <> "") Then
                            Me.hdTest.Value = dtEvento.Rows(0).Item("codigo_test")
                        End If

                        Me.dpTipoInscrito.SelectedValue = "PROFESIONAL"
                        'Me.dpTipoInscrito.Enabled = False
                        'If (dtEvento.Rows(0).Item("codigo_test") = 5) Then
                        '    Me.dpTipoInscrito.SelectedValue = "PROFESIONAL"
                        '    Me.dpTipoInscrito.Enabled = False
                        'Else
                        '    Me.dpTipoInscrito.Enabled = True
                        'End If
                    ...
                    End Try
                Else
                    If (Request.Form("token") Is Nothing _
                            Or Request.Form("Evento") Is Nothing) Then
                        'Response.Redirect("https://intranet.usat.edu.pe/usat/")
                    End If
                End If
            End If

            If (Me.hdcodigo_cco.Value <> 3022) Then
                Me.trConcidencias.Visible = False
                Me.tableformacion.Visible = False
                Me.tableformacion_1.Visible = False
                Me.trCargo.Visible = False
                Me.trArea.Visible = False
                Me.trCiudad.Visible = False
            Else
                Me.tableformacion.Visible = True
                Me.tableformacion_1.Visible = True
                Me.trConcidencias.Visible = True
                Me.trCargo.Visible = True
                Me.trArea.Visible = True
                Me.trCiudad.Visible = True
            End If

            Me.lblmensaje.Text = ""
        Catch ex As Exception
            Me.lblmensaje.Text = "Error al cargar formulario " & ex.Message
        End Try
    End Sub

    Private Sub CargaModalidad()
        Try
            Dim codigoMin As Integer = 0
            Dim obj As New ClsConectarDatos
            obj.CadenaConexion = ConfigurationManager.ConnectionStrings("CNXBDUSAT").ConnectionString
            If (Request.Form("mod") IsNot Nothing) Then
                codigoMin = Integer.Parse(Request.Form("mod"))
            End If

            obj.AbrirConexion()
            Dim dtModalidad As New Data.DataTable
            dtModalidad = obj.TraerDataTable("ALUMNI_BuscaModalidad", codigoMin)
            ...
            dtModalidad.Dispose()
        Catch ex As Exception
            Me.lblmensaje.Text = "Error al cargar las modalidades"
        End Try        
    End Sub
    ...
    Private Sub CargaDatosEvento(ByVal Envia As String)
        ...
        Try
            ...
            If tbl.Rows.Count > 0 Then
                ...
                Me.lblHeader.Text = tbl.Rows(0).Item("nombre_dev")
            ...
            If (Envia = "E" Or Envia = "P") Then
                If (Envia = "P") Then
                    ...
                    'Modificado por mvillavicencio. Al sp se le envia también el token asignado a un preinscrito, el cual es recibido por la url, con el fin de consultar los datos del preinscrito.
                    dtToken = obj.TraerDataTable("EVE_RetornaPreInscripcion_v2", Me.hdcodigo_pso.Value, Me.hdcodigo_cco.Value, Request.QueryString("token"))
                    If (dtToken.Rows.Count > 0) Then
                        If (dtToken.Rows(0).Item("estado_pins").ToString.Trim = "C") Then
                            ...
                            'cls.EnviarMail("campusvirtual@usat.edu.pe", "Campus Virtual USAT", "mvillavicencio@usat.edu.pe", "Confirmación Evento", strMensaje, True)
                        End If
                    End If
                End If
            End If
        ...
        Try
            ...
            If tbl.Rows.Count > 0 Then
                ...
                If tbl.Rows(0).Item("codigo_dep").ToString <> "" Then
                    Me.dpdepartamento.SelectedValue = tbl.Rows(0).Item("codigo_dep").ToString
    ...
    Private Function ValidaForm() As Boolean
        If (Me.txtAMaterno.Text.Trim = "") Then            
            Me.lblmensaje.Text = "Debe ingresar el apellido materno"
        ...
        If (Me.txtAPaterno.Text.Trim = "") Then            
            Me.txtAPaterno.Focus()
            Me.lblmensaje.Text = "Debe ingresar el apellido paterno"
            Return False
        End If

        If (Me.txtNombres.Text.Trim = "") Then            
            Me.txtNombres.Focus()
            Me.lblmensaje.Text = "Debe ingresar un nombre"
            Return False
        End If

        If (Me.txtemail1.Text.Trim = "") Then
            Me.txtemail1.Focus()            
            Me.lblmensaje.Text = "Debe ingresar un correo electrónico"
            Return False
        End If
    ...
    Protected Sub cmdGuardar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdGuardar.Click
        ...
        If ValidarNroIdentidad() = True Then
            If (ValidaForm() = True) Then                
                ...
                ''Response.Write("ACCION: " & Me.hdAccionForm.Value.Trim)
                '' Me.hdAccionForm.Value = "N"
                If (Me.hdAccionForm.Value.Trim = "N") Then
                    Try


                        'Response.Write("EVE_RegistraPreInscrito " & Me.hdcodigo_pso.Value & ", " & _
                        ...
                        '            strInstituto & ", " & strCentroLaboral & ", " & Me.dpTipoInscrito.SelectedValue & ", " & _
                        '            Me.dppais.SelectedValue)
                        obj.AbrirConexion()
                        Dim ruta As String = ConfigurationManager.ConnectionStrings("CNXBDUSAT").ConnectionString
                        obj.Ejecutar("EVE_RegistraPreInscrito_v3", Me.hdcodigo_pso.Value, _
                                    Me.hdcodigo_cco.Value, Me.txtObservacion.Text, Date.Today, _
                                    0, 0, 0, IIf(Me.hdAccionForm.Value.Trim = "N", "M", "N"), _
                                    "S", Me.dpTipoDoc.Text, Me.txtdni.Text.Trim, Me.txtAPaterno.Text, _
                                    Me.txtAMaterno.Text, Me.txtNombres.Text, Me.txtFechaNac.Text, _
                                    ...
                                    Me.txtemail1.Text, Me.txtemail2.Text, Me.txtdireccion.Text, _
                                    Me.dpdistrito.SelectedValue, Me.txttelefono.Text, _
                                    Me.txtcelular.Text, Me.txtruc.Text, _
                                    IIf(Me.dpModalidad.SelectedValue = "", Me.hdTest.Value, Me.dpModalidad.SelectedValue), _
                                    strInstituto, strCentroLaboral, Me.dpTipoInscrito.SelectedValue, _
                                    Me.dppais.SelectedValue, Me.txtPregadoUniv.Text, Me.txtPreGradoCiclo.Text, _
                                    Me.txtEspeArea.Text, Me.cboEspeEstado.SelectedItem.Value, Me.txtEspeUniversidad.Text, _
                                    Me.txtMaestArea.Text, Me.cboMaestEstado.SelectedItem.Value, Me.txtMaestUniv.Text, _
                                    Me.txtDoctorArea.Text, Me.cboDoctorArea.SelectedItem.Value, Me.txtDoctorUniversidad.Text, _
                                    Me.txtCargoActual.Text, Me.txtAreaTrabajo.Text, Me.txtCiudad.Text, _
                                    Me.txtemailinstitucion.Text, Me.txtTelefInstitucion.Text, _
                                    Me.chkreqfact.Checked)

                        Me.lblmensaje.Text = "Ud. ha sido registrado. Muchas Gracias"
                        LimpiarCajas()
                        Me.HdBusqueda.Value = "N"

                        Dim dtToken As DataTable
                        dtToken = obj.TraerDataTable("EVE_RetornaPreInscripcion_v2", Me.hdcodigo_pso.Value, Me.hdcodigo_cco.Value, "")
                        If (dtToken.Rows.Count > 0) Then
                            'Enviamos al correo electronico 
                            Dim strMensaje As String
                            Dim cls As New ClsMail
                            ...
                            '"<a href='http://www.usat.edu.pe/campusvirtual/librerianet/Inscripcion/frmpersona.aspx?mod=0&token=" & dtToken.Rows(0).Item("token_pins").ToString.Trim & "'>" & _
                            '"Confirmo mi asistencia</a>"

                             strMensaje = "Haga clic para confirmar su asistencia. " & _
                            "<a href='//intranet.usat.edu.pe/campusvirtual/librerianet/Inscripcion/frmpersona.aspx?mod=0&token=" & dtToken.Rows(0).Item("token_pins").ToString.Trim & "'>" & _
                            "Confirmo mi asistencia</a>"
                            cls.EnviarMail("campusvirtual@usat.edu.pe", "Campus Virtual USAT", Me.txtemail1.Text.Trim, "Confirmación Evento", strMensaje, True)

                            'strMensaje = "Haga clic para confirmar su asistencia. " & _
                            '"<a href='http://server-test/campusvirtual/librerianet/Inscripcion/frmpersona.aspx?mod=0&token=" & dtToken.Rows(0).Item("token_pins").ToString.Trim & "'>" & _
                            '"Confirmo mi asis"
                            'cls.EnviarMail("campusvirtual@usat.edu.pe", "Campus Virtual USAT", "csenmache@usat.edu.pe", "Confirmación Evento", strMensaje, True)

                            Response.Write("<script>alert('Revise su correo y confirme su asistencia. Gracias')</script>")
                            Response.Write("<script>window.close()</script>")
                        End If
                        Response.Write("<script>alert('Ud. ha sido registrado')</script>")
                    Catch ex As Exception
                        obj.CerrarConexion()
                        Me.lblmensaje.Text = "Error al registrar: Comuniquese con el administrador de la página" & ex.Message
                        'obj.AbortarTransaccion()
                    End Try
                ElseIf (Me.hdAccionForm.Value = "M" And Me.hdcodigo_pso.Value <> 0) Then
                    Response.Write("TIPO M")
                    ...
                    obj.Ejecutar("PERSON_Agregarpersona", pso, _
                        ...
                        id, Me.dppais.SelectedValue, "0").copyto(codigo_psoNuevo, 0)
                    ...
                    If CBool(Me.hdgestionanotas.Value) = True Then
                        ...
                        obj.Ejecutar("EVE_AgregarParticipanteEventoGestionaNotas", Me.hdcodigo_cco.Value, Me.dpModalidad.SelectedValue, _
                                 ...
                                 Me.GeneraClave, id, codigo_psoNuevo(0), Me.dppais.SelectedValue, "0").copyto(codigo_cliNuevo, 0)
    ...
    'Private Sub MostrarDatosPrivados(ByVal sw As Boolean)        
    '    'Verificar si es usuario nuevo
    '    Me.fila2.Visible = False
    '    Me.fila7.Visible = False
