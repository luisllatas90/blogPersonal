...
$(document).ready(function() {
            ...
            if (data.length > 0) {
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    if ((parseInt(aDataCnf[0].codMat) > 0) && data[i].chk == true && data.length >1) {
                        ...
                        tabla += '<td style="text-align:center;background:#E33439;"> <a href="#" class="btn" style="cursor: pointer;font-size:small;" onclick="fnEliminarConfirmar('+data[i].cod_Dmat+','+data[i].codigo_cur+',\''+data[i].nombre_Cur+'\',\'¿Deseas retirarte del curso \''+')"><i class="ion-android-cancel"></i></a></td>';
    ...
    function fnEliminarConfirmar(cdm, cu, registro, mensaje) {
        fnMensajeConfirmarEliminar('top', mensaje + '[' + registro + ']?', 'fnRetiro', cdm, cu);
    }
    ...
    function fnValidaCursoInferiorJSON(c, e) {
        ...
        if (!fnValidaSeleccionElectivo(c)) {
            ...
            if (!fnValidaSeleccionComplementario(c)) {
                ...
                fnMensaje('warning', 'Debes seleccionar un curso complementario');
                return false;
            } else {
   ...
   function fnValidaRetiroComplementario(cur) {
       ...
       var vtc = parseInt(fnBuscarData(cur, "tc", ""));
   ...
   function fnValidaSeleccionComplementario(cur) {
        ...
        if (elect == "False") {
            for (i = 0; i < aData.length; i++) {
                ...
                sThisVal = (aData[i].chk ? "1" : "0");
                if (aData[i].electivo_cur == 'False' && parseInt(aData[i].vCic) < parseInt(cicSel) && parseInt(aData[i].stt)==1) { // && sThisVal == "0" && ) {
                ...
                if (_sw) {
                    ...
                    if (_swtc) {
                        ...
                        if (aData[i].vCic < cicCur ){ //&& aData[i].codigo_cur == cur) {
                            ...
                            if (sThisVal == "0" && st == 0) {
                                _sw = true;
                                console.log("curso: " + aData[i].codigo_cur);
    ...
    function fnSeleccionar(ccup, ccur) {
        ...
        if (sw) {
            ...
            if (sumcredSel > lmtcrd) {
                fnMensaje('warning', 'No puede llevar mas de ' + lmtcrd + ' cr&eacute;ditos');
