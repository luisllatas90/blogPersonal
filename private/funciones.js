...
//Funcion utilizada para pintar las celdas de la tabla de la vista del horario por docente.
	function PintarCeldasHorarioDocente(ocultar)
	{
		var i=0
		var ArrFilas=document.all.tblHorario.getElementsByTagName('tr')
		var total=0
...
function AbrirPopUp2(pagina,alto,ancho,ajustable,bestado,barras,variable)
{
   var izq = (screen.width-ancho)/2
   var arriba= (screen.height-alto)/2
   if (variable=="" || variable==undefined)
   ...
   window.open(pagina,variable,"height="+alto+",width="+ancho+",statusbar="+bestado+",scrollbars="+barras+",top=" + arriba + ",left=" + izq + ",resizable="+ajustable+",toolbar=no,menubar=no");
...
function FormatoFecha(dateVal){ 
	...
	FormatoFecha= DayVal + eval(MonthVal + 1) + "/" + "/" + YearVal;	
	return FormatoFecha;
}
...
function ContarTextArea(campo,limite,obj)
{
	if (campo.value.length >limite){
	    ...
	    alert( 'Este campo s�lo aceptar hasta '  + limite + ' caracteres');
	    return false;
	}
	else{
		if (obj==undefined){
			lblcontador.innerHTML=eval(limite-campo.value.length) + ' caracteres.'
			/*1obj=document.createElement('span')
			obj.id="lblcontador"
			obj.className="rojo"
...
function ResaltarfilaDetalle_net(tbl,filaseleccionada,pagina)
{
...
function DetallarComentario(pagina)
{
	...
	if (pagina!="" || pagina!=undefined){
		form1.txtelegido.value=filaseleccionada.id
		mensajedetalle.style.display="none"
		fradetalle.location.href=pagina
	}
...
function ocultar() {
    //Mostrar/Ocultar laborum, YPEREZ: 22/11/11   
    
    if (hd == true) {
        top.document.getElementById("grupo").setAttribute("cols", "19%,81%,0%");
        hd = false;
    } else {
        top.document.getElementById("grupo").setAttribute("cols", "19%,65%,16%");
        hd = true;
    }
